...
if(!in_array($result['transactionStatus'], [1,2,3,4,5])){
	...
	$url = '/screens/feedback?s=' . base64_encode( implode(',', [enc(COMPANY_ID), enc($result['outletId']), enc($result['customerId']), enc($result['transactionId'])] ));
	header('location: ' . $url);
	dai();
}
...
?>
	...
	<div class="col-md-4 col-md-offset-4 col-sm-6 col-sm-offset-3 col-xs-12 no-bg no-padder text-center momentumit" style="top:0;position:absolute;z-index:2;">
		...
		<div class="col-xs-12 no-padder m-t hidden showOnDone">
			<a href="<?=validateHttp('src') ? validateHttp('src') : '#'?>">
				<img height="40" width="40" src="/assets/150-150/0/<?=enc(COMPANY_ID)?>.jpg" class="img-circle m-r m-b animated bounceIn" id="logo" style="display:none;" />
		...
		<div class="panel no-border col-xs-12 no-padder r-24x md-whiteframe-16dp clear animated fadeInUp speed-2x hidden showOnDone">
			...
			<div class="col-xs-12 wrapper-xs m-t-sm" style="background: url(/assets/images/modal_handle_dk.png) no-repeat center top"></div>
	...
	<?php
	loadCDNFiles(['/assets/vendor/js/sweetalert2-7.33.1.min.js','https://cdnjs.cloudflare.com/ajax/libs/jquery-touch-events/2.0.3/jquery.mobile-events.min.js'],'js');
	?>
	...
	<script type="text/javascript">
		...
		var setMap = function(height){
			...
			?>
				...
				var url 		= '/screens/mapIframe?height=' + mapHeight + '&lat=<?=$oLat?>&lng=<?=$oLng?>&theme=' + colorMode + '&zoom=15&blockZoom=1&padding=20&key=0VM_W3i3Uqi5E9P57uSsA_Q2-06swWpChj24kCv9WJ8&icon=store';
			...
			?>
				...
				if(oLat && oLng && cLat && cLng){
					...
					var url 		= '/screens/mapIframe?height=' + mapHeight + '&lat=' + cLat + '&lng=' + cLng + '&theme=' + colorMode + '&zoom=15&blockZoom=1&padding=20&key=0VM_W3i3Uqi5E9P57uSsA_Q2-06swWpChj24kCv9WJ8&icon=person';
