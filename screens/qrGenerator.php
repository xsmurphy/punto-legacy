...
if(TYPE != 'qrOnly'){
  ...
  $setting      = ncmExecute("SELECT * FROM company WHERE companyId = ? LIMIT 1",array(COMPANY_ID));
  $companyData  = ncmExecute("SELECT * FROM company WHERE companyId = ? LIMIT 1",[COMPANY_ID]);
  define('SMS_CREDIT', $companyData['smsCredit']);
...
<body class="<?=(validateHttp('darkMode') ? 'bg-dark dker' : 'bg-white')?> col-xs-12 no-padder clear">

<div class="col-xs-12 no-padder text-center">
  <div class="m-b-lg hidden hideOnQROnly">
    <a href="#" class="thumb-md animated fadeInDown"> 
      <img src="/assets/150-150/0/<?=enc(COMPANY_ID)?>.jpg" class="img-circle companyImg"> 
    </a>
  </div>

  <div id="card" class="col-xs-12 no-padder r-24x clear text-center">
    
    <div id="action">

      <div class="h3 col-xs-12 no-padder font-bold m-t hideOnQROnly">Por favor escanee el código QR</div>
      
      <div class="col-xs-12 no-padder text-center" id="qrcode">
  ...
  <div class="col-xs-12 text-muted text-center text-sm animated bounceInUp hideOnQROnly" id="policy">
    
    <div class="m-t-md col-xs-12">
      <span class="text-muted">Usamos</span> <br>
      <img src="/images/incomeLogoLgGray.png" width="80">
      <div class="text-muted"><?= APP_URL ?></div>
    </div>
...
<script>
    $(document).ready(function(){
      ...
      var isLandscape = (winWidth > winHeight) ? true : false;
      var wH          = (isLandscape) ? winHeight : winWidth;
      wH              = wH - (wH / 6);
      ...
      if(validateHttp('width')){
      ?>
        wH = <?=validateHttp('width')?>;
      <?php
      }
      ?>

      var logoWidth = wH / 5; //110

      $('#qrcode').css({'margin-top' : (logoWidth / 5) + 'px'});
      
      <?php
      $txt = '';
      if(TYPE == 'inHouseAttendance'){
        $txt = '/screens/customerInHouseAttendance?s=' . base64_encode( enc(COMPANY_ID) . ',' . enc(OUTLET_ID) . ',' . $oneHFuture );
      }else if(TYPE == 'userAttendance'){
        $txt = md5( enc(COMPANY_ID) . enc(OUTLET_ID) );
      }else if(TYPE == 'qrOnly'){
        $txt = base64_decode( validateHttp('c') );
      }
      ?>
      ...
      var options = {
          ...
          logo        : "/assets/assets/images/Calcomania%205x5.png", 
          logoWidth   : logoWidth, // width. default is automatic width
          logoHeight  : logoWidth, // height. default is automatic height
          logoBackgroundColor:'#ffffff', // Logo backgroud color, Invalid when `logBgTransparent` is true; default is '#ffffff'
          logoBackgroundTransparent:true
      }
      // Create QRCode Object
      new QRCode($("#qrcode")[0], options);

      //$('#qrcode').height(wH * 1.50);

      setInterval(function(){
        location.reload();
      },30 * 60 * 1000);//actualiza cada 30 min
