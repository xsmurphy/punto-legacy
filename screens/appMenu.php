		...
		<div class="r-24x md-whiteframe-16dp col-xs-12 no-padder clear" id="list">
			...
			<div class="panel no-border col-xs-12 no-padder m-n text-left">
				...
				<ul class="list-group list-group-lg no-bg auto no-border col-xs-12 no-padder">
					...
					<a href="/screens/userOrders?s=<?=base64_encode( enc(COMPANY_ID) . ',' . enc(USER_ID) )?>" class="list-group-item clearfix no-bg myMenuLoadPage" data-container="#externalSource">
					...
					<a href="/screens/userDayAgenda?s=<?=base64_encode( enc(COMPANY_ID) . ',' . enc(USER_ID) )?>" class="list-group-item clearfix no-bg myMenuLoadPage myMenuScheduleLink" data-container="#externalSource">
						<span class="pull-left m-t-sm m-r text-md text-info">
					...
					<a href="/login?token=<?=validateHttp('token')?>&email=<?=$result['contactEmail']?>&from=app" class="list-group-item clearfix no-bg clickeable" data-type="link">
						<span class="pull-left m-t-sm m-r text-md text-info">
	...
	<script type="text/javascript">
		...
		var savePosition = function(position,callback){
	        ...
	        if($('.myMenuMyLocation').hasClass('hidden')){
	        	var ifr = '/screens/mapIframe?height=200&lat=' + myLat + '&lng=' + myLng + '&theme=' + (ncmUIX.isDarkMode ? 'dark' : 'light') + '&zoom=15&key=yVdtZeQfaXQhMKzdbjz5onZGWBSVcLaUEiY9KzVab-Q&icon=person';

	        	//$('.myMenuMyLocation').removeClass('hidden');
	        	//$('.myMenuMyLocation iframe').attr('src',ifr);
	        }

	        let url = '/screens/appMenu?s=' + ese + '&action=setPos&lat=' + btoa(myLat) + '&lng=' + btoa(myLng);
						...
						function (result) {
							...
							if(result.cancelled == false){
							    ...
							    if(find && find == ncmTransactions.cOutletData().attendanceToken){
							    	...
							    	var url = '/screens/appMenu?s=' + ese + '&action=clockIn&outlet=' + ncmTransactions.cOutletData().outletId + '&usr=' + ncmAuth.activeUser.activeUserId + '&token=' + find;
