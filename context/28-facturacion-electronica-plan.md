...
| ~~N2~~ | **HECHA 2026-09-07** (mig 201 + merge `api/n2-reemision`): `superseded_by` + índice único PARCIAL (activo por (tx,doctype)), `EInvoiceService::reissue()` con CAS y FK diferida, `action=reissue`, dialog con el motivo ruteando al arreglo. OJO: TODA query nueva sobre `einvoice_document` que asuma un doc por venta debe filtrar `superseded_by IS NULL` — se corrigieron 3 call-sites (anulación, reportes, detalle) que cancelaban/mostraban el rechazado. | — |
...
| **F7** | Onboarding white-label: formulario legal → `CreateExternal` (admin) → vault → `PUT Tenant` → actividad → **un timbrado POR CAJA** (FC+NC, sin sucursal) → `UploadCert` → prueba SET. Campos manuales de F0 retirados. Cadena de auth admin (PhoneLogin sin contraseña del tenant) como default. **Sin duplicar config**: RUC/razón social salen de Configuración del negocio; los timbrados se administran EN LA CAJA (`register.data`, sección "Timbrados por caja" — cada caja es un punto de expedición, context/29 §1) y la emisión usa el timbrado de la caja de la venta (`stampMap` registerId → {fc,nc}, `EInvoiceService::stampForDocument`) | **Hecha** (2026-07-31) — SIN VERIFICAR contra la API real: falta la credencial admin en env. Sucursales↔outlets diferido |
