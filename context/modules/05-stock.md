...
**Ajuste manual** (`StockAdjustmentService::create`, `api/lib/services/StockAdjustmentService.php:7-107`) — filtra ítems no stockeables (`skippedItems`, no tira error), un `manageStock()` por ítem dentro de una única TX, `source='adjustment'`. El POST exige `inventory.stock.adjust` (ver regla 9).

**Conteo físico** (`InventoryCountService`, `api/lib/services/InventoryCountService.php`) — `create()` abre sesión y snapshotea los ítems que devuelve `InventoryCountScope` (`api/lib/services/InventoryCountScope.php`, mig 158), ÚNICO lugar que decide qué entra en un conteo: trackeables y activos del tenant, **con presencia en la sucursal elegida** (`EXISTS` en `stock.outletid`, o en `tolocation.locationid` si el conteo es de un depósito) y, si se pidieron, de alguna de las `categoryIds` — matcheando por los dos caminos que conviven en el schema, la FK legacy `item.categoryid` y el m2m `item_category` (mismo criterio que la mig 136). El flag `includeZeroStock` salta el filtro de presencia (primer conteo de una sucursal nueva). El alcance se persiste en `inventory_count.scope` (jsonb); las sesiones anteriores a la mig 158 traen `{}` porque se creaban con el bug: `create()` snapshoteaba TODO el catálogo del tenant y el `outletId` solo servía para resolver la cantidad esperada, así que contar una sucursal arrastraba los ítems de todas las demás en 0. El mismo predicado alimenta `action=preview` del endpoint ("vas a contar N artículos"), por construcción — no por coincidencia. `setQty`/`bulkSetQty` registran lo contado; `finish()` genera UN movimiento de ajuste por cada diferencia (`type` según signo), `source='inventory_count'` — `countedQty = NULL` al finalizar se trata como "sin diferencia" (decisión conservadora documentada en el docblock de la clase, `:11-14`), no genera movimiento. `cancel()` no revierte nada porque `finish()` es el único paso que mueve stock.
