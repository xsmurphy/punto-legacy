...
### Dinero
- ✅ `14-caja.md` — apertura, cierre, arqueo; **corregido 2026-08-17**: el `drawerid` de una venta ya se resuelve por fecha de la operación, no por el turno abierto al sincronizar; **owner 2026-08-17**: abrir/cerrar caja debe andar offline, todavía sin implementar del lado POS
...
| Numeración (arriendo offline) | POS multi-dispositivo | **Actualizado 2026-08-17**: que el lease de una caja tiene un solo tenedor — en `main`/prod hoy sigue siendo falso: `lease.php` entrega el bloque activo a cualquier device que lo pida sin noción de tenencia; dos dispositivos parados a la misma caja pueden recibir el MISMO bloque y duplicar números al sincronizar offline. El invariante ("R5", owner 2026-08-17) está garantizado a nivel de schema (F0, `register_lease` + `uq_register_lease_active`, en `main`) y la aplicación que lo hace cumplir (F2/F3) ya está escrita y verificada — pero solo en la branch `api/numeracion-exclusividad`, sin mergear. El riesgo P0 sigue activo en `main`/prod hasta ese merge. Detalle: `context/modules/17-numeracion.md` reglas 8-9 |
...
| Venta offline | Caja (arqueo) | **Corregido 2026-08-17** (commit `69a3d240`, "R6"): antes se asumía que la venta cae en la sesión de caja en la que se hizo pero `transaction.drawerid` se resolvía en el momento del SYNC (rama offline), no de la operación. Ahora `SaleService::buildTransactionRecord` resuelve `drawerId` vía `DrawerService::resolveDrawerIdForDate()` contra la MISMA `transactionDate` que las cinco dimensiones de la venta (`SaleService.php:657,670-674,684-688`) — por contención de fecha en el rango del turno, no por "qué turno está abierto ahora". Si ningún turno contiene la fecha, queda `NULL` a propósito (preferir fallar a adivinar) y lo recupera el fallback de fecha del resumen. Ver `context/modules/10-pos-venta.md` regla 10 y `context/modules/14-caja.md` regla 5 |
| Caja | Numeración (arriendo) | Cerrar la caja NO libera la tenencia de `register_lease` — dos ciclos de vida sobre el mismo `register` sin cablear entre sí. Verificado 2026-08-17: el gap es más amplio que solo el cierre de caja — `RegisterLeaseService::close('released', ...)`/`close('forced', ...)` (branch `api/numeracion-exclusividad`) no tienen NINGÚN caller todavía, ni siquiera fuera de Caja — F4 (liberar caja desde panel) no existe en ninguna branch. Ver `context/modules/17-numeracion.md` §7 |
| Caja (apertura/cierre) | Sincronización | **Owner 2026-08-17 ("R3"), sin implementar**: se asume que abrir/cerrar caja puede andar offline, encolado con la fecha real de la operación, igual que la venta. Hoy NO: `openDrawer`/`closeDrawer` tienen `offlineEligible: false` (`frontend/lib/commands/registry.ts:56-65`), sin cola en IndexedDB. El backend ya soporta recibir la fecha de operación (`api/v1/drawer.php:116`) — falta el lado POS. Ver `context/modules/14-caja.md` regla 13 y `context/modules/22-sincronizacion.md` regla 12 |
