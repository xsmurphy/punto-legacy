...
| **Órdenes/mesas (`pos_order_item`)** | `CreateOrderItemInput.selections` viaja al backend y `OrderCoreService` valida contra el catálogo y persiste una fila hija por opción (`parentorderitemid`, `addonoptionid`, `pricedelta` — mig 140). | La orden es la INTENCIÓN, no un documento de plata (regla del owner): la hija nace con `price = 0` y su recargo congelado en `pricedelta`, porque el recargo ya está adentro del `price` del padre. NINGUNA agregación de órdenes suma `pricedelta` — hacerlo cobraría el add-on dos veces. La hija hereda `stationid`/`course` del padre: el queso va a la plancha con la hamburguesa, no a la barra por estar categorizado como lácteo. |
| **Órdenes → Venta (cobro de mesa, `loadFromOrder`)** | `rebuildSelectionsFromOrder()` (`frontend/lib/cart/store.ts`) agrupa las hijas por `parentOrderItemId` y las devuelve al carrito como `CartLine.selections` del padre, despejando el precio base con el `pricedelta` congelado. De ahí en adelante el cobro es indistinguible de una venta directa. | Asume que el add-on todavía existe en el catálogo local (`PosItem.addonGroups`) y que `childQty / parentQty` da entero. Si no, devuelve `undefined` y la línea se cobra como antes — fail-safe deliberado: una selección mal reconstruida la rechaza `validateSelections` con 422 y deja la mesa INCOBRABLE. El `priceDelta` que se cobra sale del catálogo VIGENTE, no del congelado: el monto se fija al facturar. |
| Impresión — comanda de cocina vía Órdenes (`buildOrderTicketData`) | Arma `TicketItem[]` directo desde `order.items` (`pos_order_item`), sin `isAddonChild`, sin filtrar/indentar nada de add-on — `frontend/lib/orders/print-comandas.ts:29-63`. | El dato YA está persistido (mig 140), pero este builder todavía no lo indenta ni distingue padre de hija. Es lo que falta para la D4 del plan (variantes de bloque completa/facturable). |
...
- ~~**Órdenes/mesas pierden `selections` por completo**~~ ✅ **CERRADO** — la
  orden persiste las hijas (mig 140) y el cobro las re-hidrata
  (`rebuildSelectionsFromOrder`, 2026-08-23). Ver §5 y
  `context/41-addons-y-combos.md` §"El add-on cruza el flujo de orden".
- **Comanda de cocina de Órdenes sin add-ons** — vigente por otro motivo que el
  original: el dato ya está persistido, lo que falta es que
  `print-comandas.ts` distinga padre de hija e indente. Desbloquea la D4 del
  plan.
