...
|---|---|---|---|
| 2026-08-01 | `DialogContent` es **fullscreen en `max-sm:`** (app-wide, panel incluido): `top-0 left-0 h-dvh max-h-dvh w-full max-w-none translate-x-0 translate-y-0 content-start rounded-none` sumado al primitive `components/ui/dialog.tsx`. El `max-h-[85dvh]` centrado sigue vigente de `sm:` para arriba. `content-start` es obligatorio — el content es un `grid` y con alto fijo `align-content: stretch` estiraría header/footer. Un dialog que necesite fullscreen por encima de 640px (ej. módulos del POS, que cortan en el breakpoint 768 de `useIsMobile`) declara las clases explícitas con `!`, no delega en el `max-sm:` | — | Modales flotantes inutilizables en viewport móvil del POS — decisión owner |
