...
| `INVOICE_AUTH_NOTIFY` | ¿El job `invoice-auth-notices` MANDA los avisos de timbrado por vencer/vencido, o solo loguea? **Default ENCENDIDO**, al revés que `PLAN_LIFECYCLE_NOTIFY`: el bloqueo de facturación por timbrado vencido ya está activo (`InvoiceAuthGate`), y un preaviso que hay que acordarse de encender deja el corte duro sin su mitad. `0`/`off`/`false` = dry-run. El volumen está acotado por las ventanas (7 y 3 días antes, y hasta 7 días después del vencimiento), así que el primer cron post-deploy no puede mailear a toda la base |
...
| `invoice-auth-notices` | diario 06:15 | `InvoiceAuthNoticeService::run()` (context/29), cross-tenant. Avisa el TIMBRADO por vencer/vencido, **por caja** — es la otra mitad de `InvoiceAuthGate`, el guard que desde 2026-09-07 impide facturar con el timbrado caído (422 `invoice_auth_expired` en `/v1/sales.php`; el POS además lo bloquea localmente, offline). Renovar un timbrado es un trámite ante la autoridad fiscal, no un botón: sin preaviso el corte deja la caja sin vender de un día para el otro. Ventanas con catch-up igual que el plan: `d7` = (3,7], `d3` = [0,3] (el día 0 todavía factura — el timbrado vence al TERMINAR su último día), `expired` = [-7,-1] (el piso evita mailear por timbrados caídos hace meses en la primera corrida). **UN mail por comercio con el detalle por caja**, no uno por caja. Idempotencia SIN tabla nueva: `register.data.invoiceAuthNotices.{d7,d3,expired}` guarda la FECHA de vencimiento que disparó el aviso, así correrlo dos veces el mismo día no duplica y una renovación vuelve a habilitar el ciclo sola. La marca se escribe solo tras un envío real (en dry-run no, o prender el switch encontraría todo "ya enviado"). Destinatario: el dueño, vía `TenantNotice::ownerEmail()` — compartido con `plan-lifecycle`. Los días se calculan en la zona de CADA comercio (`TenantClock`), no con `current_date` de la sesión |
