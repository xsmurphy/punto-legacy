...
| ~~`app/load.php` (1714 líneas)~~ → **250 líneas post-Slice 41 (2026-06-03)** | Dispatcher de proxies y APIs externas completamente vaciado. Ya NO es god node — quedan solo 5 handlers de APIs externas diferidas (Bancard ×3, ePOS ×2; money path, requieren sandbox). ✅ Cluster A 100% migrado en Slices 38-41 con el patrón "API granular + BFF compone" validado en 4 slices sucesivos. Ver `10-roadmap.md § load.php casi terminado`. |
