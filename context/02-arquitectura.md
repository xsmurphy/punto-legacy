...
|------|--------|---------------|
| **SaleService** (`bff/sales → api/v1/sales.php → api/lib/Sales/SaleService.php`) | Ventas simples type 0/3 elegibles (cashsale/creditsale puro, sin EI/parent) **+ gift card (vender y pagar — 35c ✅) + sesiones agendadas (35d ✅) + storeCredit/points/inCredit (35e ✅) + recurrente (35f ✅)** | **AUTORITATIVO** — cubre prácticamente el 100% del tráfico real de ventas cashsale/creditsale (excepción: EI 35b diferida y parentId) |
| **Legacy processData** (`app/action.php?l=processData`) | 422 del SaleService (no elegible — EI/parentId) + EI (35b, diferida por diseño). RECHAZA type 0/3 simples con **409** | Sólo retiene: EI (35b, diferida). Ya NO es safety-net de ningún path de balance ni de recurrente (todos migrados a SaleService). El cron `cronCreateRecurringInvoice.php` envía re-submisión a action.php sin JWT → 401 (deuda pre-existente, el cron necesita mintar un JWT de servicio — no causada por 35f). |
...
**Fuente única de elegibilidad (commit dbf2866, 35a.8; extendida en 35c.1+35c.2+35d+35e+35f):** `saleIsSimplePathEligible($payload, $sale): ?string` en `app/includes/functions.php` es la función COMPARTIDA entre ambos tiers: devuelve `null` si el payload es elegible; devuelve un string-motivo si no lo es. `SaleInput::assertSimplePathEligible` (API, traduce a 422) y el guard de `processData` (legacy, genera 409) la usan ambos — sin duplicar la regla. Es un god-helper más de `app/includes/functions.php`. **35c.1**: `'giftcard'` salió de los payment types rechazados. **35c.2**: `giftcardId` en items salió del rechazo; items type=`'giftcard'` saltean el check de empty-itemId. **35d**: rechazo de `item.duration > 0` eliminado. **35e**: `'points'` y `'storeCredit'` salieron de los payment types rechazados; `'inCredit'` agregado a tipos-sin-itemId válidos (junto a `discount` y `giftcard`). **35f (HITO FINAL — commit d23ead1)**: `repeat=true` salió del rechazo — era el ÚLTIMO rechazo migrado. **Solo quedan rechazados**: EI (35b, diferida), parentId, e ítems sin itemId de tipo desconocido. Para ventas type 0/3 sin EI ni parentId la elegibilidad es siempre NULL (va por SaleService).
