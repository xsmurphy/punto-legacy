...
| 3.5 | Replicar a los demás reportes + módulos (patrón YA fijado y verificado E2E con datos) | — | EN CURSO — 4 hechos: `summary`✅, `p_methods`✅, `inventory`✅, `users`✅ |
...
- **Cola de reportes "simples" (lista del usuario)**: `users`✅. Pendientes: `categories`, `brands`, `stock`, `stock_day`, `satisfaction` (este último tiene un DELETE → front→BFF→API de escritura). OJO, varios traen bugs PG propios a arreglar al migrar: `categories` hace `SELECT itemId … GROUP BY category` (válido en MySQL, **error en PG** → usar `MIN(itemId)`); `brands` usa `USE INDEX`; `stock_day` usa `DATE()`; `stock` usa `itemTrackInventory = 1` (bool=int) y `OUTLET_ID < 2` (uuid<int). Para verificar con datos hay que sembrar `itemSold` (desbloquea users/categories/brands).
