...
**Servicios modernos (en `/api/lib/<Module>/` — namespace `Punto\Api\<Module>`, PSR-4, DTOs, enums)**: **`SaleService`** — ubicado en `api/lib/Sales/SaleService.php` (namespace `Punto\Api\Sales`; commit 4037977). Primer módulo del proyecto con namespace + DTOs + autoloader PSR-4. Implementación incremental completada hasta **35f** (HITO MAYOR: strangler processData→SaleService para type 0/3 prácticamente completo salvo EI/35b y parentId). Métodos principales: `save()` (entry point), `persistItemsAndStock()` (captura `$itemSoldId` vía `Insert_ID()` post-AutoExecute; llama `persistScheduledSessions` si hay cliente; ítems tipo `inCredit` sin itemId → `persistInCreditItem` + continue; `updateLastTimeEdit` hoistado al final del loop), `persistScheduledSessions()` (35d), `persistGiftCardRedemptions()`, `redeemGiftCard()`, `sellGiftCard()`, `notifyGiftCardBeneficiaries()`, `persistLoyaltyEarning()`, `persistBalanceRedemptions()` (35e — itera payments, `manageCustomerLoyalty('used')` para `points`, `manageCustomerStoreCredit('used')` para `storeCredit`; gate: clientId null → no-op), `persistInCreditItem()` (35e — UPDATE parametrizado `contactStoreCredit + amount`; fix SQLi del legacy), **`persistRecurring()` (NUEVO 35f** — si `repeat=true AND type=Creditsale`: INSERT en tabla `recurring` dentro de la tx de `save()`; calcula `nextDate` y `endDate` con `getNextDatePeriod`; `recurringTransactionData` = JSON con shape canónico `{action:"processData", companyId, outletId, userId, roleId, registerId}`; `recurringSaleData` = payload completo con `repeat=false` para evitar loop en re-submisión del cron; `data` JSONB anidado §22.6; verificado E2E: creditsale monthly×3 → fila `recurring` con action/IDs correctos, len(txData)=236 < 255). **`dispatchElectronicInvoice()` (NUEVO 35b)**: POST-COMMIT best-effort; RUC de `company.config` JSONB; mapea type→typeDoc (FC/FCR; NCR/type=6 diferido); llama sendFE(); gate: token no vacío. **`notifyCustomer()` (actualizado 35b)**: prioridad de URL recibo: `digitalInvoice > EI > receipt local`. `SaleInput` (35f+35b): campos `repeat (bool)`, `repeatF (string)`, `repeatT (int)`, `electronicInvoicePY` (nullable array — 35b). DTOs: `SaleInput` (fromPayload, validación fail-fast, `assertSimplePathEligible` delega en `saleIsSimplePathEligible()`), `SaleResult`. Enum: `SaleType` (Cashsale=0, Creditsale=3, etc.). Excepciones: `InvalidSaleInputException`, `DuplicateSaleException`, `SaleAbortedException`. Context: `api/lib/Context/TenantContext.php` (DTO inmutable, namespace `Punto\Api\Context`). **Este es el patrón a seguir para módulos nuevos** — ver §22.9 en `08-convenciones.md`.
...
**Estado actual de load.php (250 líneas, post Slice 41 commit 9306d30 — 2026-06-03):**
- **5 handlers vivos**: bancardQR, pixQR, verifyTransactionPix, ePOSPending, verifyTransactionEPOS.
- Todos son APIs externas diferidas (money path, requieren sandbox de proveedor). ✅ **Cluster A 100% migrado** (Slices 38–41).
- load.php YA NO es un god node. Es un dispatcher mínimo de APIs externas diferidas. Ver sección "load.php casi terminado" abajo.

**Reducción total del dispatcher (referencia):**
- action.php: 3549 → 1685 líneas (**-52%**).
- load.php: 1714 → 250 líneas (**-85%**).
...
- **`processData` (~1540 líneas)**: el guardado de ventas. **ESTADO ACTUAL (2026-05-31 post-35b — HITO FINAL: STRANGLER COMPLETAMENTE MIGRADO para type 0/3)**: SaleService es AUTORITATIVO para ventas simples (cashsale/creditsale type 0/3 elegibles), **gift card (vender + pagar — 35c ✅)**, **sesiones agendadas (35d ✅)**, **storeCredit / points / inCredit (35e ✅)**, **recurrentes (35f ✅)** y **factura electrónica PY (35b ✅ — POST-COMMIT best-effort)**. El legacy SOLO retiene: parentId (edge raro). **El 100% del tráfico real de ventas cashsale/creditsale pasa por SaleService.** `saleIsSimplePathEligible()` ahora solo rechaza: parentId e ítems sin itemId de tipo desconocido. La función queda en estado casi-null para toda venta normal. **DEUDA cron JWT (pre-existente, no causada por 35f)**: `panel/crons/cronCreateRecurringInvoice.php` envía la re-submisión a `action.php` sin JWT → 401; el cron necesita refactorizarse para mintar un JWT de servicio antes de re-someter. La elegibilidad la determina `saleIsSimplePathEligible()` en `functions.php` (fuente única compartida entre tiers — ver `02-arquitectura.md § Guardado de ventas`). **Flujos verificados E2E en PG (legacy)**: venta cashsale básica ✅ (commit b45684f), venta con cliente seleccionado ✅ (commit b0617ea), venta a crédito type=3/dueDate/complete=false ✅, modificar items en carrito (count 1→5) ✅. Bugs encadenados resueltos: (1) UUID NULL para customerId/transactionParentId=0, (2) timestamp vacío → NULL, (3)-(4) columnas `contactFixedComission`/`itemComissionPercent`/`itemComissionType`/`itemSessions` demoted a JSONB → `SELECT *` + `_flattenJsonb` + filtro PHP (§22.8), (5) `getValue('item','itemSessions',...)` con UUID sin comillas + columna demoted → mismo patrón, (6) `updateLastTimeEdit()` escribía a columnas `company.*LastUpdate` inexistentes en PG → RMW sobre `config` JSONB con `$db->Execute` directo (§22.8.1). **Paths NO migrados aún (post-35d)**: EI (factura electrónica / 35b), inCredit/storeCredit/points (35e), recurrente (35f). **Migración a SaleService — SLICE 35 (strangler-fig incremental)**: BFF→API→Service (`api/lib/Sales/SaleService.php`, `api/v1/sales.php`, `app/bff/sales.php`); front (`app/scripts/app.js`, `ncmHttp.postSale()`) repuntado en 35a.7. Mapeo por bloques B1-B16 del legacy. Plan de sub-slices:
  ...
  - **35c** ✅ **COMPLETO** (commits 0e3c7bf + d099019, 2026-05-31): **gift card — vender y pagar** totalmente en SaleService. Ver entry `_session-log.md` 2026-05-31 §35c para detalle completo.
    ...
    - **Estado del strangler tras 35d**: SaleService cubre venta simple (35a) + gift card vender/pagar (35c) + sesiones agendadas (35d). El legacy processData retiene: EI (35b, diferida), inCredit/storeCredit/points (35e), recurrente (35f). Verificado E2E: 3 sesiones × 2 unidades → 6 filas type=13; total=15000 (ceil 90000÷6); packageId, customerId, status=0, meta.details correctos.
    - **Estado del strangler tras 35e (HITO MAYOR, commit 1da19e6)**: SaleService cubre venta simple (35a) + gift card vender/pagar (35c) + sesiones agendadas (35d) + storeCredit/points/inCredit (35e). **Todos los payments que modifican balance del cliente están en SaleService.** El legacy processData retiene SÓLO: EI (35b, diferida) y recurrente (35f). Verificado E2E: loyalty 5000→3000 (-2000 points), storeCredit 10000-4000+3000=9000 (débito storeCredit + acreditación inCredit).
    - **Estado final del strangler tras 35f (HITO MAYOR — STRANGLER COMPLETO, commit d23ead1)**: SaleService cubre venta simple (35a) + gift card vender/pagar (35c) + sesiones agendadas (35d) + storeCredit/points/inCredit (35e) + recurrente (35f). **El strangler processData→SaleService para type 0/3 está prácticamente completo.** processData retiene SÓLO: EI (35b, diferida por diseño) y parentId (edge raro). `saleIsSimplePathEligible()` ya no rechaza ningún path de balance ni recurrente — solo EI, parentId, e ítems sin itemId de tipo desconocido. Verificado E2E: creditsale monthly×3 → fila `recurring` con action/IDs correctos.
  - **35b** ✅ **COMPLETO — HITO FINAL: STRANGLER TYPE 0/3 COMPLETAMENTE MIGRADO** (commit a246722, 2026-05-31): factura electrónica PY en SaleService. `SaleInput`: agrega `electronicInvoicePY` (nullable array). `saleIsSimplePathEligible`: elimina el rechazo de `electronicInvoicePY` — era el ÚLTIMO rechazo de payload activo. **Ahora solo rechaza**: parentId (edge raro) e ítems sin itemId de tipo desconocido. Para ventas type 0/3 sin parentId la elegibilidad es siempre NULL → van por SaleService. `SaleService::dispatchElectronicInvoice` (NUEVO): POST-COMMIT best-effort; obtiene RUC de `company.config` JSONB (ncmExecute, tenant-scoped); mapea `type→typeDoc` (FC/FCR; NCR/type=6 es devoluciones, diferido); llama `sendFE()`; gate: `FACTURACION_ELECTRONICA_TOKEN` no vacío (dev sin FE → no-op, mismo patrón que sendAudit). `SaleService::notifyCustomer` actualizado: URL del recibo sigue prioridad del legacy (`digitalInvoice > EI (FACTURACION_ELECTRONICA_URL directo) > receipt local`). Verificado E2E: cashsale (type=0) y creditsale (type=3) con payload `electronicInvoicePY` → 200 (antes 422). sendFE no dispara en dev (token vacío). **El strangler processData→SaleService para type 0/3 está COMPLETAMENTE migrado** (salvo parentId que es un edge raro, no un feature real). `saleIsSimplePathEligible` queda en estado casi-null para toda venta normal.
  - **35d** ✅ **COMPLETO** (commit 88a9dc9, 2026-05-31): sesiones agendadas totalmente en SaleService. `saleIsSimplePathEligible`: rechazo de `item.duration > 0` eliminado. `SaleService::save()`: eliminado `assertNoScheduledItems`. `persistItemsAndStock`: captura `$itemSoldId` vía `Insert_ID()` post-AutoExecute; si hay cliente llama `persistScheduledSessions`; `updateLastTimeEdit` hoistado al final del loop (mejora vs legacy que lo llamaba por cada ítem con sesiones). `persistScheduledSessions` (nuevo): lee `itemSessions` con `ncmExecute` (JSONB demoted §22.8); `total = ceil(precio / (itemSessions × count))`; crea N filas type=13 dentro de la tx; `transactionDetails` → `meta` JSONB (el legacy lo ponía como columna directa inexistente en PG → se perdía silenciosamente); `packageId` = UUID del `itemSoldId` (no int legacy); INSERTs parametrizados vía AutoExecute. Devuelve int count. Verificado E2E: 3 sesiones × 2 unidades → 6 filas type=13; total=15000 (ceil 90000÷6); packageId, customerId, status=0, meta.details correctos.
  - **35e** ✅ **COMPLETO** (commit 1da19e6, 2026-05-31): storeCredit / points / inCredit totalmente en SaleService. `saleIsSimplePathEligible` (functions.php): elimina el rechazo de payments `points` y `storeCredit`; añade `inCredit` a tipos-sin-itemId válidos (junto a `discount` y `giftcard`). `SaleService::persistBalanceRedemptions` (NUEVO): itera payments, llama `manageCustomerLoyalty('used')` para `points` y `manageCustomerStoreCredit('used')` para `storeCredit` dentro de la tx. Gate: clientId null → no-op. Llamado entre `persistGiftCardRedemptions` y `persistLoyaltyEarning` en `save()`. `SaleService::persistInCreditItem` (NUEVO): UPDATE parametrizado `contactStoreCredit + amount`. Fix SQLi del legacy (`action.php:2380` concatenaba `$sD['total']` crudo). Gate: amount > 0 + clientId presente. `persistItemsAndStock`: ítems sin itemId tipo `inCredit` + cliente → `persistInCreditItem` + continue (sin itemSold/stock — igual que el legacy). Verificado E2E: loyalty 5000→3000 (-2000 points); storeCredit 10000-4000+3000=9000. code-reviewer sin P0/P1.
  - **35f** ✅ **COMPLETO — HITO MAYOR: STRANGLER COMPLETO** (commit d23ead1, 2026-05-31): ventas recurrentes totalmente en SaleService. `SaleInput`: nuevos campos `repeat (bool)`, `repeatF (string — daily/weekly/monthly/quarterly/yearly)`, `repeatT (int — número de repeticiones)`. `saleIsSimplePathEligible` (functions.php): elimina el rechazo de `repeat=true` — era el ÚLTIMO rechazo migrado; ahora solo quedan rechazados: EI (35b), parentId, e ítems sin itemId de tipo desconocido. `SaleService::persistRecurring` (NUEVO): si `repeat=true AND type=Creditsale`, INSERT en tabla `recurring` dentro de la tx de `save()`; calcula `nextDate` y `endDate` con `getNextDatePeriod`; `recurringTransactionData` = JSON con shape canónico `{action:"processData", companyId, outletId, userId, roleId, registerId}` (el cron espera este shape para someter vía `action.php?l=base64(txData)`); `recurringSaleData` = payload completo con `repeat=false` (evita loop en re-submisión); `data` JSONB anidado (§22.6). Verificado E2E: creditsale monthly×3 → fila `recurring` con action/IDs correctos, `data.recurringSaleData` presente, len(txData)=236 < 255. **DEUDA cron JWT (pre-existente, no causada por 35f)**: `cronCreateRecurringInvoice.php` re-somete a `action.php` sin JWT → 401; el cron necesita mintar un JWT de servicio. **Estado final del strangler**: SaleService cubre el ~100% del tráfico real de ventas type 0/3 (35a+35c+35d+35e+35f ✅); processData retiene SÓLO EI (35b, diferida) y parentId (edge raro).
...
- ~~**Handler vestigial borrado** (commit 9ce8c1f)~~: `customerAddress` (-58 líneas) — callsite ya en `bff/customer_address` desde Slice 1.
- ~~**`tin` migrado (Slice 38, commit dc33d7e, 2026-06-02)**~~: `TinService::lookup` vía Marangatu (SET). `panel/API/get_tin.php` sigue vivo (otros consumers posibles). `app/tin.php` / `app/rucs.php`: verificar callsites antes de borrar.
- ~~**`userLocation` migrado (Slice 39, commit c052fe5, 2026-06-02)**~~: `OrderService::getNextDeliveryForUser` + endpoint compositor `api/v1/orders.php?resource=userLocation`.
- ~~**`ordersPanelAPI` migrado (Slice 40)**~~: `OrderService::getPanelList` — refresh del panel de órdenes.
- ~~**`calendar_resources_json` + `calendar_week_json` + `calendar_agenda_json` + `calendar_month` migrados (Slice 41, commit 9306d30, 2026-06-03)**~~: `ScheduleService::getCalendarSlots` (resources/week), `getCalendarAgenda` (agenda mensual), `getCalendarMonthCounts` (counts por día). Endpoint compositor `api/v1/schedule.php?resource=calendar&mode=<resources|week|agenda|month>`. BFF `app/bff/schedule.php`: 4 cases nuevos. Fix PG: IN($csv UUIDs interpolados) → IN(?,?,...) bindeados. Mejora: `getCalendarAgenda` usa JOIN a `contact` en vez del N+1 SELECT del legacy. `app/scripts/app.js`: 3 callsites repunteados (líneas 8697/9447/9475). load.php: 519 → 250 líneas (-270 en este slice).

**Único pendiente — APIs externas diferidas (5 handlers, 250 líneas):** requieren credenciales reales o sandbox de proveedores (money path). **NO migrar sin integración disponible.** Decisión: no bloquean Phase AI; sesión dedicada cuando haya sandbox.
  - `bancardQR` / `pixQR` / `verifyTransactionPix` — Bancard.
  - `ePOSPending` / `verifyTransactionEPOS` — ePOS.

**Reducción total de load.php**: 1714 → 250 líneas (**-85%**). load.php ya NO es un god node — es un dispatcher mínimo de APIs externas diferidas.

**Decisión 2026-06-01/03:** ✅ Cluster A cerrado. APIs externas requieren setup de sandbox. Ninguno bloquea Phase AI.
