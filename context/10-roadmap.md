...
- **Cluster meta-JSONB (8 handlers) — DESBLOQUEADO (formato definido, 2026-05-28)**: `removeItemfromOrder`, `processOrderItems`, `processOrderItemsUpdate`, `moveOrderItems`, `setUserToOrder`, `updateSchedule`, `scheduleSession`, `checkIfUserOccupied`. Leen/escriben `transactionDetails` (en `meta` jsonb). **El formato ya está definido y probado** (commit `e7c04fb`): se guarda como JSON-string dentro de `meta` → `{"transactionDetails":"<json>","tags":"<json>"}`; `_flattenJsonb` lo re-expone y los `json_decode($row['transactionDetails'])` siguen funcionando. WRITE: rebuild de `meta` (o `jsonb_set`). Los 8 handlers ya pueden migrarse como slices limpios. NOTA: las LECTURAS multi-fila (load.php, sale, crons) NO aplican `_flattenJsonb` al iterar `$result->fields` → necesitan `SELECT meta` + decode manual.
