...
| 3.5 | Replicar a los demás reportes + módulos (patrón YA fijado y verificado E2E con datos) | — | EN CURSO — 6 hechos: `summary`✅ `p_methods`✅ `inventory`✅ `users`✅ `categories`✅ `brands`✅ |
...
- **Cola de reportes "simples" (lista del usuario)** — batch por fuente de datos (opción 2):
  ...
  - **Batch B (stock/item)**: `stock`, `stock_day` — pendiente. Bugs PG a arreglar: `DATE()` (stock_day), `itemTrackInventory = 1` (bool=int), `OUTLET_ID < 2` (uuid<int).
