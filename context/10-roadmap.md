...
**Slice 33 COMPLETO — `customerRecord` de load.php (commit b0fbec3, 2026-05-29)**: `CustomerService::getRecords(companyId, customerId)` — fichas personalizadas del cliente (tablas `customerRecord` + `cRecordField` + `cRecordValue`), devuelve datos estructurados (id, label, tipo, valor por campo). `api/v1/customers.php` GET `?resource=records&id=`. `app/bff/customers.php` handler `action=customerRecord`. Template Mustache `#customerRecordTpl` nuevo en `app/index.php` + `app/index.html` — reproduce los 7 tipos de campo del legacy (text, number, date, phone, switch, progress, image) con las mismas clases/ids que el guardado (`recordsEdit`, lee del DOM: `customerRecordValue`, `switch`, `progress<id>`, `contenteditable`). `globalv2.js` + `debug.js`: `ncmCustomer.recordsList` reescrito — antes inyectaba HTML de load.php, ahora hace fetch JSON a `bff/customers` y renderiza vía Mustache; luego conecta datePicker + uploaders Dropbox por campo imagen. `app/load.php`: eliminado el handler `customerRecord` (~300 líneas, el ÚLTIMO handler del desacople de listas/fichas). **SQL injection corregida**: el legacy concatenaba `cRecordFieldId`/`customerId` en el SQL (getValue) → queries parametrizadas; scope `companyId` agregado. **Nota QA**: requiere verificación manual del modal de fichas en browser (render + guardado + subida de imagen).
...
**PATRÓN NUEVO — datos estructurados + template Mustache (establecido Slice 33):** hasta este slice, los handlers migrados devolvían JSON y el front ya tenía lógica de renderizado. `customerRecord` era diferente: el legacy era HTML server-rendered (~300 líneas de PHP generando markup). La decisión (acordada con el usuario) fue **no portar el HTML al servidor**: la API devuelve datos estructurados (campos tipados) y el front renderiza con un template Mustache estático en `index.php`/`index.html`. Ver convención §24 en `08-convenciones.md`.
