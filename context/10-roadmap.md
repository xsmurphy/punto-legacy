...
**Slices pendientes**: ver "action.php — mapa de lo que queda" arriba (clusters meta-JSONB, ENCOM→Punto, mesa-merge, monstruos, HTML/especial, dead) + los concerns restantes de `load.php` (customerRecord, customerInfo; ambos con rotura generalizada de `transactionDetails`→`meta` esperada).
