...
| 3.5 | Replicar a los demás reportes + módulos (patrón YA fijado y verificado E2E con datos) | — | EN CURSO — 7 hechos: `summary`✅ `p_methods`✅ `inventory`✅ `users`✅ `categories`✅ `brands`✅ `stock_day`✅ |
...
- **Cola de reportes "simples" (lista del usuario)** — batch por fuente de datos (opción 2):
  ...
  - **`satisfaction`**: pendiente (tiene un DELETE → primer write front→BFF→API).
