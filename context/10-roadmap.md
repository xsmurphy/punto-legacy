...
- **`processData` (~1540 líneas)**: el guardado de ventas. **ESTADO ACTUAL (2026-05-31 post-35a.7)**: las ventas simples (cashsale/creditsale type 0/3) ahora van por `bff/sales → SaleService` con fallback automático al legacy. El legacy sigue activo como safety-net para el fallback y para los paths no migrados (EI, gift cards, sesiones, recurrentes). **Flujos verificados E2E en PG (legacy)**: venta cashsale básica ✅ (commit b45684f), venta con cliente seleccionado ✅ (commit b0617ea), venta a crédito type=3/dueDate/complete=false ✅, modificar items en carrito (count 1→5) ✅. Bugs encadenados resueltos: (1) UUID NULL para customerId/transactionParentId=0, (2) timestamp vacío → NULL, (3)-(4) columnas `contactFixedComission`/`itemComissionPercent`/`itemComissionType`/`itemSessions` demoted a JSONB → `SELECT *` + `_flattenJsonb` + filtro PHP (§22.8), (5) `getValue('item','itemSessions',...)` con UUID sin comillas + columna demoted → mismo patrón, (6) `updateLastTimeEdit()` escribía a columnas `company.*LastUpdate` inexistentes en PG → RMW sobre `config` JSONB con `$db->Execute` directo (§22.8.1). **Paths NO migrados aún**: gift card, EI (factura electrónica), schedule (citas). **Migración a SaleService — SLICE 35 (strangler-fig incremental)**: BFF→API→Service (`api/lib/Sales/SaleService.php`, `api/v1/sales.php`, `app/bff/sales.php`); front (`app/scripts/app.js`, `ncmHttp.postSale()`) repuntado en 35a.7. Mapeo por bloques B1-B16 del legacy. Plan de sub-slices:
  ...
  - **35a.8** ⏳ **DIFERIDO** — cleanup del path simple de type 0/3 en `action.php` (legacy processData). Razón: el fallback de `ncmHttp.postSale()` DEPENDE del legacy como safety-net; removerlo ahora rompería el fallback. Se hace solo tras período de estabilidad en producción observada.
