...
| 3.5 | Replicar a los demás reportes + módulos (patrón YA fijado y verificado E2E con datos) | — | EN CURSO — 8 hechos: `summary`✅ `p_methods`✅ `inventory`✅ `users`✅ `categories`✅ `brands`✅ `stock_day`✅ `satisfaction`✅ (primer WRITE) |
...
- **Cola de reportes "simples" (lista del usuario)** — batch por fuente de datos (opción 2):
  ...
  - **Batch B (stock/item)**: `stock_day`✅ (commit 75b3851; 3 fixes PG: bool=int, DATE(), y `ORDER BY stockId DESC`→`stockDate DESC` porque stockId es uuid aleatorio en PG). `stock` PENDIENTE — **NO es un clon simple**: multi-depósito (filas anidadas por depósito + `stockTrigger` por outlet), usa `getAllItemStock()` (helper que da 0 en el widget de inventory, sospechoso de bug PG), y el gate `if(OUTLET_ID < 2)` está roto en PG (UUID comparado como int → 0<2 siempre true → el reporte SIEMPRE muestra "seleccione sucursal"). Para migrarlo: arreglar el gate, verificar/arreglar getAllItemStock, y sembrar taxonomy(location)+toLocation+stockTrigger.
...
- **Cola "simples" — estado final**: `users`✅ `categories`✅ `brands`✅ `stock_day`✅ `satisfaction`✅ (5 de 6). Falta **`stock`** (el complejo: multi-depósito + stockTrigger + gate OUTLET_ID<2 roto en PG + helper getAllItemStock).
