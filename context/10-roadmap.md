...
**Slices pendientes de load.php**: queda SOLO `customerRecord` (HTML server-rendered — decisión de contrato pendiente sobre si se migra al patrón JSON o se mantiene como fragmento HTML). Los clusters restantes de `action.php` (mesa-merge, monstruos processData/sale, HTML/especial, dead) no son de `load.php`.
