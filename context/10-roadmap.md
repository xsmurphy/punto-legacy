...
| 3.5 | Replicar a los demás reportes + módulos (patrón YA fijado y verificado E2E con datos) | — | EN CURSO — 12 hechos + 1 alias: `summary`✅ `p_methods`✅ `inventory`✅ `users`✅ `categories`✅ `brands`✅ `stock_day`✅ `satisfaction`✅ (1er WRITE) `stock`✅ `recurring`✅ (2º WRITE) `summary_year`✅ `customers`✅ (núcleo; extras diferidos) + `by_brands`↪alias |
...
- **Bugs PG latentes descubiertos en customers (no arreglados — ver notas abajo)**:
  - **`getCustomerData()` / `getContactData()` interpolación sin comillas** (`functions.php:~3488`): en la branch `'uid'`/`'contactId'`, construye `WHERE contactId = $id` interpolando el UUID via `$db->Prepare($id)`, pero `$db->Prepare` devuelve UUIDs **sin comillas** (son para placeholders `?`). La query resultante es `WHERE contactId = 019e6018-...` → error de sintaxis PG → la función cae al fallback `'Sin Nombre'` vacío. **Bug activo en TODOS los call-sites de esta función** (incluyendo `ReportSatisfactionService`). El service de customers lo evita con su propio lookup por batch (`SELECT * FROM contact WHERE companyId = ? AND contactId IN (?,...)`) — mismo patrón que el reporte recurring. Candidato a fix central en `functions.php`.
...
- **Bug latente rastreado — `lessInternalTotals()` + `USE INDEX` + columna `tags`**: esta función (usada en `summary_year` y otros) tiene internamente `USE INDEX` de MySQL y referencias a la columna `tags` (movida a `meta` JSONB en Phase PG). Para tenants con `ignoreInternal = true` la función dispara queries PG-incompatibles; para tenants normales el short-circuit devuelve zeros sin ejecutar las queries → no falla en prod hoy. Si algún tenant activa `ignoreInternal`, explotará. Registrado como deuda PG pendiente; misma clase de problema que `getAllItems()` y `getAllItemStock()`.
...
- **Bug PG pendiente (god function, NO arreglado)**: `getAllItems()` (`includes/functions.php:~3354`) interpola `COMPANY_ID` y el `IN()` **sin comillas** (`companyId = 00000000-...`) → roto en PG. Lo evité con un lookup parametrizado propio en el service de inventory, pero afecta a muchos otros lugares (incl. el widget de KPIs de inventory via `getAllInventoryAndItemsModule`, que por eso da 0). Candidato a fix masivo PG aparte.
