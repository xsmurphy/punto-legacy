...
| 3.5 | Replicar a los demás reportes + módulos (patrón YA fijado y verificado E2E con datos) | — | EN CURSO — 13 hechos + 1 alias: `summary`✅ `p_methods`✅ `inventory`✅ `users`✅ `categories`✅ `brands`✅ `stock_day`✅ `satisfaction`✅ (1er WRITE) `stock`✅ `recurring`✅ (2º WRITE) `summary_year`✅ `customers`✅ (núcleo; extras diferidos) `expenses`✅ (3er WRITE) + `by_brands`↪alias |
