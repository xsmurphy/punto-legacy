...
COMMENT ON FUNCTION fn_period_guard() IS
  'E1b (mig 157) + D8 (mig 160). Genérico via TG_ARGV: [0]=columna fecha, '
...
COMMENT ON FUNCTION rollup_recompute_period(uuid, text, date) IS
  'D8 context/48 (mig 160). sales/item_sales/payments: grano día único en '
...
COMMENT ON FUNCTION period_close_run(uuid, date, uuid, text) IS
  'E1b (mig 157) + D8 (mig 160). Dominios vigentes tras el grano diario: '
...
BEGIN
  ...
  END LOOP;
  RAISE NOTICE 'mig 160: rollup_reconcile corrió % iteración(es); rollup_dirty pendiente: %',
