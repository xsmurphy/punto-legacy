#!/bin/bash
# =============================================================
# run_sale_void_test.sh — arnés de integración de la anulación de ventas
# (SaleVoidService::void(), F1+F2 de context/40-anulacion-y-nota-credito.md).
#
# Mismo patrón que run_credit_payment_void_test.sh / api/lib/Sales/
# verify_chain/run.sh (Docker Postgres descartable + schema + migraciones +
# fixtures del tenant "Verify PY") — reusa ESE seed.sql en vez de reinventar
# company/outlet/register/items, así que este script depende de ese archivo
# pero no lo modifica.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_sale_void_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_sale_void_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_sale_void_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_sale_void_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_sale_void_test.sh] esperando Postgres"
  ...
  echo "[run_sale_void_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_sale_void_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_sale_void_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_sale_void_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test INSERTA ventas de prueba contra esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    VERIFY_CHAIN_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_sale_void_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_sale_void_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
# ── 2. Test de anulación de ventas (integración, contra el Postgres de arriba) ──
echo ""
echo "[run_sale_void_test.sh] === anulación de ventas (integración) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/sale_void_test.php"

echo ""
echo "[run_sale_void_test.sh] TODO OK."
