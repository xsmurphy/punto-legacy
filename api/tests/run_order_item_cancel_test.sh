#!/bin/bash
# =============================================================
# run_order_item_cancel_test.sh — arnés de la ANULACIÓN DE ÍTEMS de comanda
# (motivo obligatorio, ventana de tiempo, atribución a la persona).
#
# Lo que verifica, en una línea: anular una línea de comanda exige motivo y
# permiso del OPERADOR del PIN, respeta la ventana configurada por el comercio
# (`settingOrderItemCancelWindowMinutes`, 0 = apagada) salvo que el operador
# tenga `pos.order.item.cancel.late`, y el evento queda registrado a nombre de
# la PERSONA y no de la tablet compartida.
# Ver el docblock de order_item_cancel_test.php.
#
...
#   bash api/tests/run_order_item_cancel_test.sh
#
# Por defecto levanta su PROPIO Postgres descartable en Docker, aplica
# schema + migraciones (necesita la 190 y la 191, que son las que propagan las
# claves nuevas a los roles) + fixtures, corre el test, y lo destruye al
...
OWN_DOCKER=0
CONTAINER_NAME="punto_order_item_cancel_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_order_item_cancel_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_order_item_cancel_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_order_item_cancel_test.sh] esperando Postgres"
  ...
  echo "[run_order_item_cancel_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_order_item_cancel_test.sh] corriendo migrate.php (incluye 190/191, backfill de las claves nuevas)..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_order_item_cancel_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${ORDER_ITEM_CANCEL_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_order_item_cancel_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test CREA órdenes, roles y contactos de prueba contra esa base," >&2
    ...
    echo "    ORDER_ITEM_CANCEL_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_order_item_cancel_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_order_item_cancel_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
echo ""
echo "[run_order_item_cancel_test.sh] === anulación de ítems (motivo, ventana, atribución) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/order_item_cancel_test.php"

echo ""
echo "[run_order_item_cancel_test.sh] TODO OK."
