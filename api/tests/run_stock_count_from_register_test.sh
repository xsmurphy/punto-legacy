#!/bin/bash
# =============================================================
# run_stock_count_from_register_test.sh — arnés del CONTEO DE STOCK DESDE LA
# CAJA (context/63 F0 + F1): que `stockCountBlind` se respete, que el permiso
# se mida contra el operador del PIN, que el mismo `opId` no duplique el
# ajuste, y que el modo "solo registro" (D9) no toque el ledger.
# Ver `api/tests/stock_count_from_register_test.php`.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_stock_count_from_register_test.sh
#
...
CONTAINER_NAME="punto_stock_count_pg_$$"
OWN_DOCKER=0

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_stock_count_from_register_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_stock_count_from_register_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_stock_count_from_register_test.sh] esperando Postgres"
  ...
  echo "[run_stock_count_from_register_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_stock_count_from_register_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_stock_count_from_register_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -q \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${STOCK_COUNT_TEST_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_stock_count_from_register_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test ESCRIBE (roles, contactos, conteos y MOVIMIENTOS DE STOCK) en esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    STOCK_COUNT_TEST_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_stock_count_from_register_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_stock_count_from_register_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
echo ""
echo "[run_stock_count_from_register_test.sh] === conteo de stock desde la caja ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
export JWT_SECRET="${JWT_SECRET:-test-secret-stock-count}"
harness_run "$SCRIPT_DIR/stock_count_from_register_test.php"

echo ""
echo "[run_stock_count_from_register_test.sh] TODO OK."
