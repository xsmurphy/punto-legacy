#!/bin/bash
# =============================================================
# run_quote_status_test.sh — arnés del ESTADO DE UNA COTIZACIÓN.
#
# Cierra el reporte del tester del 2026-08-28: la columna "Estado" de
# Cotizaciones mostraba un «1» pelado, y no había forma de saber si el
# presupuesto ya se había facturado. Cubre las dos mitades: que SaleInput lea
# `parentTransactionId` (el writer de `quote_to_sale` que la mig 115 dejó sin
# construir) y que el reporte derive los 4 estados del ciclo de vida.
# Ver el docblock de quote_status_test.php.
#
...
#   bash api/tests/run_quote_status_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_quote_status_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_quote_status_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_quote_status_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_quote_status_test.sh] esperando Postgres"
  ...
  echo "[run_quote_status_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_quote_status_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_quote_status_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_quote_status_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
  ...
  echo "[run_quote_status_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés (integración, contra el Postgres de arriba) ───────────────────
echo ""
echo "[run_quote_status_test.sh] === estado de cotizaciones (pendiente/facturada/vencida/anulada) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/quote_status_test.php"

echo ""
echo "[run_quote_status_test.sh] TODO OK."
