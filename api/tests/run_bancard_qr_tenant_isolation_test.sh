#!/bin/bash
# =============================================================
# run_bancard_qr_tenant_isolation_test.sh — arnés del AISLAMIENTO CROSS-TENANT
# del QR de Bancard (path de dinero).
#
...
#   bash api/tests/run_bancard_qr_tenant_isolation_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_bancard_qr_iso_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_bancard_qr_tenant_isolation_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_bancard_qr_tenant_isolation_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_bancard_qr_tenant_isolation_test.sh] esperando Postgres"
  ...
  echo "[run_bancard_qr_tenant_isolation_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_bancard_qr_tenant_isolation_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_bancard_qr_tenant_isolation_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_bancard_qr_tenant_isolation_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test CREA una company/QR de prueba contra esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    VERIFY_CHAIN_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_bancard_qr_tenant_isolation_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés (integración, contra el Postgres de arriba) ───────────────────
echo ""
echo "[run_bancard_qr_tenant_isolation_test.sh] === aislamiento cross-tenant del QR de Bancard ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/bancard_qr_tenant_isolation_test.php"

echo ""
echo "[run_bancard_qr_tenant_isolation_test.sh] TODO OK."
