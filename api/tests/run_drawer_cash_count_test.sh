#!/bin/bash
# =============================================================
# run_drawer_cash_count_test.sh — arnés del semáforo de cuadre del arqueo:
# esperado congelado al cerrar (mig 164), clasificación verde/rojo/amarillo
# con tolerancia por comercio (`Reports\CashCountStatus`) y cierres históricos
# sin esperado guardado. Ver `api/tests/drawer_cash_count_test.php`.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_drawer_cash_count_test.sh
#
...
CONTAINER_NAME="punto_drawer_cash_pg_$$"
NETWORK_NAME="punto_drawer_cash_net_$$"
OWN_DOCKER=0
OWN_NETWORK=0

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_drawer_cash_count_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_drawer_cash_count_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_drawer_cash_count_test.sh] esperando Postgres"
  ...
  echo "[run_drawer_cash_count_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_drawer_cash_count_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"
else
  if [ "${DRAWER_TEST_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_drawer_cash_count_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
  ...
  echo "[run_drawer_cash_count_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés (integración, contra el Postgres de arriba) ───────────────────
echo ""
echo "[run_drawer_cash_count_test.sh] === cuadre del arqueo (semáforo) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/drawer_cash_count_test.php"

echo ""
echo "[run_drawer_cash_count_test.sh] TODO OK."
