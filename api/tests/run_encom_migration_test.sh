...
#   bash api/tests/run_encom_migration_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_encom_migration_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_encom_migration_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_encom_migration_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_encom_migration_test.sh] esperando Postgres"
  ...
  echo "[run_encom_migration_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_encom_migration_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_encom_migration_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test CREA dos companies con catálogo, clientes y CAJAS CON TIMBRADO" >&2
  ...
  echo "[run_encom_migration_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés ───────────────────────────────────────────────────────────────
echo ""
echo "[run_encom_migration_test.sh] === migrador ENCOM (import, idempotencia, numeración, choque de punto) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/encom_migration_test.php"

echo ""
echo "[run_encom_migration_test.sh] TODO OK."
