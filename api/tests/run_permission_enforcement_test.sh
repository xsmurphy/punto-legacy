#!/bin/bash
# =============================================================
# run_permission_enforcement_test.sh — arnés de enforcement de permisos.
#
# Verifica que cada clave de PermissionCatalog que gatea un endpoint lo gatee
# de verdad: rol sin la clave → 403, rol con la clave → pasa, owner → pasa
# siempre; más la escalación de privilegios de /v1/users.
#
# Mismo patrón que run_sale_void_test.sh (Docker Postgres descartable +
# schema + migraciones + fixtures del tenant "Verify PY") — reusa ESE seed.sql
# en vez de reinventar company/outlet/register/items.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_permission_enforcement_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_permission_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_permission_enforcement_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_permission_enforcement_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_permission_enforcement_test.sh] esperando Postgres"
  ...
  echo "[run_permission_enforcement_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_permission_enforcement_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_permission_enforcement_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_permission_enforcement_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test CREA roles, sesiones y contactos de prueba contra esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    VERIFY_CHAIN_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_permission_enforcement_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_permission_enforcement_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
echo ""
echo "[run_permission_enforcement_test.sh] === enforcement de permisos (integración) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/permission_enforcement_test.php"

echo ""
echo "[run_permission_enforcement_test.sh] TODO OK."
