#!/bin/bash
# =============================================================
# run_drawer_closing_totals_test.sh — arnés del arqueo que devuelve el cierre
# de caja, la mitad servidor del total offline del turno
# (context/51-configuracion-offline-de-la-caja.md §4).
#
...
#   bash api/tests/run_drawer_closing_totals_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_drawer_closing_totals_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_drawer_closing_totals_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_drawer_closing_totals_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_drawer_closing_totals_test.sh] esperando Postgres"
  ...
  echo "[run_drawer_closing_totals_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_drawer_closing_totals_test.sh] corriendo migrate.php (incluye 70/147/156 — drawerid en transaction)..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_drawer_closing_totals_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_drawer_closing_totals_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test ABRE/CIERRA una caja de prueba y le inserta ventas." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    VERIFY_CHAIN_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_drawer_closing_totals_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés del arqueo (integración, contra el Postgres de arriba) ────────
echo ""
echo "[run_drawer_closing_totals_test.sh] === arqueo devuelto al cerrar la caja ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/drawer_closing_totals_test.php"

echo ""
echo "[run_drawer_closing_totals_test.sh] TODO OK."
