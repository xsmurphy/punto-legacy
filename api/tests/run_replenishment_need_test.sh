#!/bin/bash
# =============================================================
# run_replenishment_need_test.sh — arnés de la NECESIDAD DE REPOSICIÓN
# (context/70 §B.5, D6-D8, mig 228): disparo por stock mínimo dentro del
# embudo del ledger (sin duplicar, con rollback, concurrente, tolerante a
# fallos), disparo por conteo (panel y caja), cobertura por producción y
# transferencia, cierre manual y aislamiento por tenant/sucursal.
#
# Docker Postgres descartable + schema + migraciones + fixtures "Verify PY".
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_replenishment_need_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_replenishment_need_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_replenishment_need_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_replenishment_need_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_replenishment_need_test.sh] esperando Postgres"
  ...
  echo "[run_replenishment_need_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_replenishment_need_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_replenishment_need_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_replenishment_need_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
  ...
  echo "[run_replenishment_need_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_replenishment_need_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
# ── 2. Test del ledger de stock (integración, contra el Postgres de arriba) ──
echo ""
echo "[run_replenishment_need_test.sh] === Necesidad de reposición (integración) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/replenishment_need_test.php"

echo ""
echo "[run_replenishment_need_test.sh] TODO OK."
