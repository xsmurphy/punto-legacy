#!/bin/bash
# =============================================================
# run_register_tenancy_offline_test.sh — arnés de la tenencia de caja frente a
# la venta offline (incidente 2026-08-23,
...
#   bash api/tests/run_register_tenancy_offline_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_register_tenancy_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_register_tenancy_offline_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_register_tenancy_offline_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_register_tenancy_offline_test.sh] esperando Postgres"
  ...
  echo "[run_register_tenancy_offline_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_register_tenancy_offline_test.sh] corriendo migrate.php (incluye 141/144/150 — register_lease)..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_register_tenancy_offline_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_register_tenancy_offline_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test INSERTA/BORRA tenencias y dispositivos de prueba contra esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    VERIFY_CHAIN_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_register_tenancy_offline_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés de tenencia (integración, contra el Postgres de arriba) ───────
echo ""
echo "[run_register_tenancy_offline_test.sh] === tenencia de caja y venta offline ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/register_tenancy_offline_test.php"

echo ""
echo "[run_register_tenancy_offline_test.sh] TODO OK."
