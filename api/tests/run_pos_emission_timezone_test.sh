#!/bin/bash
# =============================================================
# run_pos_emission_timezone_test.sh — arnés de la HORA DE EMISIÓN de la venta
# del POS: que `apiAuthPosContext()` deje la sesión en la zona del comercio, que
# la fecha se derive del instante absoluto y que el turno de la venta se
# encuentre. Ver `api/tests/pos_emission_timezone_test.php`.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_pos_emission_timezone_test.sh
#
...
#   2. TODO EN DOCKER (`POS_TZ_TEST_IN_DOCKER=1`): además del Postgres, corre
...
#
# El arnés lanza subprocesos `php` propios (cada modo necesita un proceso y una
# conexión vírgenes — la TZ es estado de proceso). En el modo 2 esos subprocesos
# salen del `php` del contenedor, que es el correcto.
#
...
IN_DOCKER="${POS_TZ_TEST_IN_DOCKER:-0}"
PHP_IMAGE="${POS_TZ_TEST_PHP_IMAGE:-punto-php-test}"
...
CONTAINER_NAME="punto_pos_tz_pg_$$"
NETWORK_NAME="punto_pos_tz_net_$$"
OWN_DOCKER=0
OWN_NETWORK=0

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_pos_emission_timezone_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_pos_emission_timezone_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_pos_emission_timezone_test.sh] esperando Postgres"
  ...
  echo "[run_pos_emission_timezone_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_pos_emission_timezone_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"
else
  if [ "${POS_TZ_TEST_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_pos_emission_timezone_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    ...
    echo "    POS_TZ_TEST_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_pos_emission_timezone_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés (integración, contra el Postgres de arriba) ───────────────────
echo ""
echo "[run_pos_emission_timezone_test.sh] === hora de emisión de la venta (POS) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/pos_emission_timezone_test.php"

echo ""
echo "[run_pos_emission_timezone_test.sh] TODO OK."
