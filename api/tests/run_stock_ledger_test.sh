#!/bin/bash
# =============================================================
# run_stock_ledger_test.sh — arnés de integración del saneamiento de stock
# (context/52-stock-ledger-unica-fuente.md):
#   (1) combo fijo + anulación sin doble reposición (G4),
#   (2) movimiento con fecha retroactiva → SUM correcto y lectores migrados
#       de acuerdo (Reports\StockService, onHandByLocation),
#   (3) contrato de retorno de Inventory::manageStock() (G11): false solo
#       para el no-op legítimo, excepción ante un fallo real de escritura.
#
# Mismo patrón que run_return_d2_d3_test.sh / run_sale_void_test.sh (Docker
# Postgres descartable + schema + migraciones + fixtures del tenant
# "Verify PY") — reusa ESE seed.sql, que incluye el combo fijo
# VERIFY-COMBO-FIJO que este arnés necesita.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_stock_ledger_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_stock_ledger_test_$$"
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_stock_ledger_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_stock_ledger_test.sh] esperando Postgres"
  ...
  echo "[run_stock_ledger_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_stock_ledger_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_stock_ledger_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_stock_ledger_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
  ...
  echo "[run_stock_ledger_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_stock_ledger_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
echo ""
echo "[run_stock_ledger_test.sh] === Ledger de stock: única fuente de verdad (integración) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/stock_ledger_test.php"

echo ""
echo "[run_stock_ledger_test.sh] TODO OK."
