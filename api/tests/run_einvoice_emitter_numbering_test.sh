#!/bin/bash
# =============================================================
# run_einvoice_emitter_numbering_test.sh — arnés de la NUMERACIÓN DEL EMISOR
...
OWN_DOCKER=0
CONTAINER_NAME="punto_einv_numbering_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_einvoice_emitter_numbering_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_einvoice_emitter_numbering_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_einvoice_emitter_numbering_test.sh] esperando Postgres"
  ...
  echo "[run_einvoice_emitter_numbering_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_einvoice_emitter_numbering_test.sh] corriendo migrate.php (necesita la 145 —timbrado congelado— y las del módulo de FE)..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_einvoice_emitter_numbering_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${EINVOICE_NUMBERING_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_einvoice_emitter_numbering_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    ...
    echo "    EINVOICE_NUMBERING_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_einvoice_emitter_numbering_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_einvoice_emitter_numbering_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
echo ""
echo "[run_einvoice_emitter_numbering_test.sh] TODO OK."
