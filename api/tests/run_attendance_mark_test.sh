...
#   bash api/tests/run_attendance_mark_test.sh
#
...
CONTAINER_NAME="punto_attendance_pg_$$"
OWN_DOCKER=0

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_attendance_mark_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_attendance_mark_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_attendance_mark_test.sh] esperando Postgres"
  ...
  echo "[run_attendance_mark_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_attendance_mark_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_attendance_mark_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -q \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${ATTENDANCE_TEST_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_attendance_mark_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test ESCRIBE (legajos y marcaciones de asistencia) en esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    ATTENDANCE_TEST_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_attendance_mark_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_attendance_mark_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
echo ""
echo "[run_attendance_mark_test.sh] === marcación de asistencia ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
export JWT_SECRET="${JWT_SECRET:-test-secret-attendance}"
harness_run "$SCRIPT_DIR/attendance_mark_test.php"

echo ""
echo "[run_attendance_mark_test.sh] TODO OK."
