#!/bin/bash
# =============================================================
# run_psp_payment_methods_test.sh — arnés del refactor de pasarelas de pago
# (PSP): `ensurePspMethod`, `PspCatalog`, separación en el arqueo y grano del
# rollup. Ver `api/tests/psp_payment_methods_test.php`.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_psp_payment_methods_test.sh
#
# Dos modos:
#
#   1. HOST con php+pdo_pgsql (default si `php` está y el arnés arranca):
#      levanta su propio Postgres descartable en Docker y corre php local.
#      Es el modo de `run_return_d2_d3_test.sh` y los demás runners.
#
#   2. TODO EN DOCKER (`PSP_TEST_IN_DOCKER=1`): además del Postgres, corre el
#      PHP dentro de la imagen `punto-php-test` en una red propia. Es el modo
#      del servidor de Punto (167.71.165.221), donde no hay php con pdo_pgsql
#      en el host. NO toca la base de producción: levanta la suya.
#
...
IN_DOCKER="${PSP_TEST_IN_DOCKER:-0}"
PHP_IMAGE="${PSP_TEST_PHP_IMAGE:-punto-php-test}"
...
CONTAINER_NAME="punto_psp_pg_$$"
NETWORK_NAME="punto_psp_net_$$"
OWN_DOCKER=0
OWN_NETWORK=0

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_psp_payment_methods_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_psp_payment_methods_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  # sin valor, que le dice a docker "tomala del entorno". Si el export viniera
  # despues, el contenedor arrancaria sin contrasena y el arnes colgaria
  # esperando un Postgres que nunca acepta conexiones.
  ...
  echo -n "[run_psp_payment_methods_test.sh] esperando Postgres"
  ...
  echo "[run_psp_payment_methods_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_psp_payment_methods_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"
else
  if [ "${PSP_TEST_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_psp_payment_methods_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test ESCRIBE (tenant, medios de pago y ventas de prueba) en esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    PSP_TEST_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_psp_payment_methods_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés (integración, contra el Postgres de arriba) ───────────────────
echo ""
echo "[run_psp_payment_methods_test.sh] === pasarelas de pago (PSP) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/psp_payment_methods_test.php"

echo ""
echo "[run_psp_payment_methods_test.sh] TODO OK."
