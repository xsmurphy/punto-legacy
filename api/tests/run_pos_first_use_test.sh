#!/bin/bash
# =============================================================
# run_pos_first_use_test.sh — arnés del primer uso del POS (context/72 §9):
# pareo automático desde una sesión de panel (D-P1) y desbloqueo sin PIN con
# un solo usuario en la sucursal (D-P2). Ver el docblock de pos_first_use_test.php.
#
# Mismo patrón que run_device_invitation_single_use_test.sh: Postgres
# descartable + schema + migraciones + seed, corre el arnés y destruye todo.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_pos_first_use_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_pos_first_use_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_pos_first_use_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_pos_first_use_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_pos_first_use_test.sh] esperando Postgres"
  ...
  echo "[run_pos_first_use_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_pos_first_use_test.sh] corriendo migrate.php (incluye 171 — canje de un solo uso)..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_pos_first_use_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_pos_first_use_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
  ...
  echo "[run_pos_first_use_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés del canje (integración, contra el Postgres de arriba) ─────────
echo ""
echo "[run_pos_first_use_test.sh] === primer uso del POS: pareo automático y desbloqueo sin PIN ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/pos_first_use_test.php"

echo ""
echo "[run_pos_first_use_test.sh] TODO OK."
