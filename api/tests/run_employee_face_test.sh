#!/bin/bash
# =============================================================
# run_employee_face_test.sh — arnés del ROSTRO DEL EMPLEADO
# (context/83 F2, mig 231): que sin consentimiento no se pueda registrar un
# rostro, que el quiosco NO elija a quién enrola (la ventana la abre el panel,
# es de una sucursal y se consume), que los vectores de otra versión de modelo
# no se sirvan, y que el egreso —y retirar el consentimiento— borren la
# biometría dejando el legajo en pie.
# Ver `api/tests/employee_face_test.php`.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_employee_face_test.sh
#
...
CONTAINER_NAME="punto_face_pg_$$"
OWN_DOCKER=0

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_employee_face_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_employee_face_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_employee_face_test.sh] esperando Postgres"
  ...
  echo "[run_employee_face_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_employee_face_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_employee_face_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -q \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${FACE_TEST_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_employee_face_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
    echo "  este test ESCRIBE (legajos y rostros registrados) en esa base." >&2
    echo "  Si es un Postgres descartable a propósito, confirmá con:" >&2
    echo "    FACE_TEST_ALLOW_EXISTING_DB=1 bash $0" >&2
    exit 1
  fi
  echo "[run_employee_face_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
  echo "[run_employee_face_test.sh] cargando fixtures (seed.sql de verify_chain, idempotente)..."
...
echo ""
echo "[run_employee_face_test.sh] === rostro del empleado ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
export JWT_SECRET="${JWT_SECRET:-test-secret-face}"
harness_run "$SCRIPT_DIR/employee_face_test.php"

echo ""
echo "[run_employee_face_test.sh] TODO OK."
