#!/bin/bash
# =============================================================
# run_sale_tags_test.sh — arnés de las ETIQUETAS DE LA VENTA.
#
# Regresión del incidente del 2026-09-15: una etiqueta escrita a mano en la
# caja ("Venta Whatsapp") se comparaba contra una columna uuid y Postgres
# abortaba la venta entera con 22P02. Dos ventas ya cobradas quedaron trabadas
# en la cola del POS reintentándose solas. Ver el docblock de
# sale_tags_test.php.
#
# Mismo patrón que run_quote_status_test.sh: Postgres descartable +
# schema + migraciones, corre el arnés y destruye todo al terminar.
#
# Uso (un comando, desde la raíz del repo):
#   bash api/tests/run_sale_tags_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_sale_tags_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_sale_tags_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_sale_tags_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_sale_tags_test.sh] esperando Postgres"
  ...
  echo "[run_sale_tags_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_sale_tags_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_sale_tags_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_sale_tags_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
  ...
  echo "[run_sale_tags_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés (integración, contra el Postgres de arriba) ───────────────────
echo ""
echo "[run_sale_tags_test.sh] === etiquetas de la venta (toTag): nombres, uuids y tenant scope ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/sale_tags_test.php"

echo ""
echo "[run_sale_tags_test.sh] TODO OK."
