#!/bin/bash
# =============================================================
# run_mcp_key_test.sh — arnés de las API KEYS DEL MCP (M0 de context/58).
#
...
# existencia. Ver el docblock de mcp_key_test.php.
#
...
#   bash api/tests/run_mcp_key_test.sh
#
...
OWN_DOCKER=0
CONTAINER_NAME="punto_mcp_key_test_$$"

cleanup() {
  if [ "$OWN_DOCKER" = "1" ]; then
    echo "[run_mcp_key_test.sh] deteniendo Postgres descartable ($CONTAINER_NAME)..."
...
if [ -z "${POSTGRES_HOST:-}" ]; then
  OWN_DOCKER=1
  echo "[run_mcp_key_test.sh] sin POSTGRES_HOST — levantando Postgres descartable en Docker..."
  ...
  echo -n "[run_mcp_key_test.sh] esperando Postgres"
  ...
  echo "[run_mcp_key_test.sh] cargando extensiones + schema base..."
  ...
  echo "[run_mcp_key_test.sh] corriendo migrate.php..."
  php -d variables_order=EGPCS "$API_DIR/database/migrate.php"

  echo "[run_mcp_key_test.sh] cargando fixtures (seed.sql de verify_chain)..."
  docker exec -i "$CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
    < "$VERIFY_CHAIN_DIR/seed.sql" >/dev/null
else
  if [ "${VERIFY_CHAIN_ALLOW_EXISTING_DB:-}" != "1" ]; then
    echo "[run_mcp_key_test.sh] ERROR: POSTGRES_HOST=$POSTGRES_HOST está seteado, pero" >&2
  ...
  echo "[run_mcp_key_test.sh] usando Postgres existente: $POSTGRES_HOST:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-puntoDB}"
fi

# ── 2. Arnés (integración, contra el Postgres de arriba) ───────────────────
echo ""
echo "[run_mcp_key_test.sh] === API keys del MCP (realm propio, permisos heredados, revocación aislada) ==="
export POSTGRES_HOST POSTGRES_PORT POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD
harness_run "$SCRIPT_DIR/mcp_key_test.php"

echo ""
echo "[run_mcp_key_test.sh] TODO OK."
