...
| **Lotes y vencimientos** | `75-lotes-y-vencimientos.md` (análisis pedido por el owner 2026-09-10, SIN plan ni decisiones — se puede sólo en versión REFERENCIAL: si las salidas no eligen lote, el saldo por lote es una ESTIMACIÓN, y eso hay que aceptarlo antes de construir. La versión valorizada (FEFO real) toca los 23 call-sites de `manageStock()`, el COGS y las recetas. Casi toda la infra de alertas ya existe: el patrón "próximo a vencer" con ventanas disjuntas está resuelto en `InvoiceAuthNoticeService`/`PlanLifecycleService` sobre `TenantNotice`, con el cron montado. OJO: la tabla `inventory` del schema ES una tabla de lotes con `inventoryExpirationDate`, muerta y sin lectores — revivirla contradice la D2 de `context/52`, se dropea; y "lote" ya significa lote de PRODUCCIÓN (`production_batch`). Ver §6 arquitecturas a evitar antes de proponer nada) |
