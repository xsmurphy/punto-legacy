...
| **App del dueño (dashboard + Punto AI en el teléfono)** | `72-app-del-duenio.md` (plan sin implementar, escrito 2026-09-07 y AMPLIADO 2026-09-09 §7 con el alcance nuevo del owner: config de emergencia, reportes básicos y captura de facturas de compra. D1 CERRADA: es una PWA APARTE. **D2 CERRADA 2026-09-09: vive DENTRO de `frontend/`, NO un proyecto Next nuevo** — "PWA aparte" es manifest+`id`+`scope` propios, no otro repo; el sitio de marketing ya convive ahí ruteado por host. **D3 CERRADA: el preset shadcn es variación de DENSIDAD, no marca nueva** (radios/spacing/altura de control); los tokens van bajo la clase del route group, NUNCA en `:root` — `--spacing` es la escala base de Tailwind v4 y filtrada repinta el POS que factura en producción. PROHIBIDO correr `shadcn init --preset` sobre `frontend/`: reescribe `globals.css` y `components.json` del producto entero. Pendiente de dato: el preset `b2D0vQ4au` es del registro de shadcn.com y no se pudo resolver — hay que extraer sus tokens a mano. **CORRIGE a `context/71`**: su D1 dice "la PWA de Punto" pero se escribió un día ANTES de que existieran DOS; la captura de facturas va en la app del DUEÑO (realm panel, borrador por usuario), no en la del POS. D4-D6 y A0-A5 propuestas sin OK. Ver §5 y §7.6 arquitecturas rechazadas antes de proponer nada) |
