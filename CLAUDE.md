...
| **Migrador ENCOM → Punto** | `77-migrador-encom.md` (**F1 implementada 2026-09-11**, D1-D6 cerradas — soporte carga las credenciales del cliente en `/admin`, un worker CLI exporta del panel legacy e importa por los SERVICIOS REALES; `migration_map` da idempotencia. La password NO se persiste: el endpoint hace el login en el momento y guarda solo las cookies, que el worker borra al terminar. **Las cajas CONTINÚAN su serie fiscal** —`document_sequence` nace con el timbrado y el punto del legacy y `nextnumber` = último emitido + 1, porque el contador legacy guarda el ÚLTIMO y el de Punto el PRÓXIMO— y dos cajas con el mismo (timbrado, EEE-PPP) abortan el dominio ENTERO sin importar ninguna. **§4 corrige el mapa del legacy con el que se arrancó**: `get_registers.php` NO existe (las cajas salen de `get_company.php`, única fuente que las trae con su sucursal; `a_registers.php?list=true` es HTML y solo ve la sucursal activa), `registerInvoicePrefix` YA es `EEE-PPP` (el legacy le hace explode para SIFEN, `sufix` es otra cosa), `get_tags.php` no usa envelope e inyecta un tag hardcodeado, y solo `get_items.php` pagina. F2 = ventas históricas, SIN implementar — una venta importada no puede pasar por `SaleService::save()`. Falta cargar `ENCOM_MIGRATION_URL` en Coolify y validar los shapes contra el sistema vivo (§9). Ver §10 arquitecturas rechazadas antes de proponer nada) |
