...
| **Asistente IA DENTRO del POS (POS-nativo)** | `59-asistente-en-la-caja.md` (plan sin implementar 2026-08-30 — la arquitectura elegida REUSA el realm read-only del MCP: al desbloquear con PIN el backend emite una `auth_session` de turno, read-only y de vida corta, igual que una key MCP (`McpKeyService.php:76`), con el guard del embudo de `api/bootstrap.php:153`. Realm HERMANO `pos-ai`, no `mcp` reusado: el opt-in de endpoints ES el recorte del catálogo, y con `mcp` el backend no distinguiría la caja de una API key. D2 (read-only) queda cerrada por construcción y D7 (UI) la cerró el owner. **Lo único bloqueante es D3: qué tools ve el cajero** — 7 dentro, `get_report` y finanzas AFUERA. Ver §arquitecturas RECHAZADAS antes de proponer nada) |
