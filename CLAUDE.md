...
| **Adelantos y saldo a favor por concepto** | `74-saldo-a-favor-por-concepto.md` (plan sin implementar 2026-09-08 — nace de una cantina escolar pero **D1 lo convirtió en un motor genérico de ADELANTOS**. **D1 CERRADA por el owner: DOS MODOS** — A factura al recibir el adelanto (percepción del cliente, es lo que Punto ya hace) y B factura al entregar (correcto contablemente, la carga es cobranza anticipada sin venta). **Los dos modos INVIERTEN dónde se reconoce el ingreso**: en A el consumo no suma (`NonAddingSales`), en B el consumo ES la venta — tratar B como "A con un if" duplica o pierde ingresos. **Invariante crítica: el modo se congela en el SALDO, no en la config del comercio** (patrón del IVA congelado de `context/38`); leerlo del switch vigente re-factura saldos ya facturados. **Insight del owner confirmado en código: giftcard y crédito interno son el MISMO mecanismo** — se emiten en el mismo loop (`SaleService.php:2100-2112`), ambos con consumo parcial, ambos escalares mutados in-place sin historial, ambos excluidos por `NonAddingSales`; solo cambia el portador (código anónimo vs contacto nominal). Se construye el motor que debieron compartir siempre y giftcard migra después (D7). Lo que el cliente cree difícil —que el consumo no se duplique— YA ESTÁ RESUELTO. **Hallazgo que cambia el alcance: el POS nuevo no tiene NINGUNA UI de saldo a favor** (`frontend/` solo lee el campo; giftcard sí tiene flujo en `pay-dialog`). D2-D7 propuestas sin OK. Ver §7 arquitecturas rechazadas antes de proponer nada) |
