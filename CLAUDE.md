...
| **Lotes y vencimientos** | `75-lotes-y-vencimientos.md` (**PLAN con D1-D7 cerradas por el owner 2026-09-15**, sin implementar — reemplaza el análisis "referencial" del 09-10: el pedido real es TRAZABILIDAD legal (importadoras/veterinarias), así que el lote se asigna también en las SALIDAS. Lo que destraba: **lote ≠ método de costeo** — el lote es eje de CANTIDAD del ledger (como el depósito) y el costo sigue siendo el promedio ponderado del ítem; contracara aceptada: el margen por lote no es real. `stock.lotId` + invariante en `manageStock()` (ítem con lotes sin lote se rechaza; el "sin lote" es una FILA, nunca NULL), saldo por lote = `SUM(stockCount)` derivado. Venta: FEFO sugerido y el cajero corrige, UNA línea = UN lote. Dos avisos: stock propio y VENDIDO por cliente (distribuidoras); días de aviso SOLO por ítem (controla lotes + días), el aviso de vendido es UN interruptor de empresa (D6). D7: no complica la operativa — el dato se carga solo en la compra, todo lo demás lo asigna el sistema; el stock previo queda en "sin lote" y FEFO lo vende primero (no hay que contar para arrancar); producción fuera de alcance. Cambio de lote al cliente = devolución + venta nueva. La tabla `inventory` se DROPEA, no se revive; "lote" ya es `production_batch`, la tabla nueva es `stock_lot`. No habilitar a clientes antes de F2. Ver §8 arquitecturas a evitar antes de proponer nada) |
