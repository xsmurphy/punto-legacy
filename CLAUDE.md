...
| **Wallet multi-nivel (titular + sub-cuentas)** | `74-wallet-multinivel.md` (plan sin implementar 2026-09-08, renombrado desde `74-saldo-a-favor-por-concepto.md` — el owner lo elevó de "saldo a favor" a MÓDULO genérico: wallet de titular, sub-cuentas, transferencia interna, histórico y TOPES de consumo. Rubro-agnóstico: colegio, gimnasio, comedor de empresa, flota; "padre/alumno" es UI, el schema dice titular/sub-cuenta. **D1 CERRADA: DOS MODOS de facturación** — A factura al cargar (percepción del cliente, es lo que Punto ya hace) y B factura al entregar (la carga es cobranza anticipada). **Los modos INVIERTEN dónde se reconoce el ingreso** (en A el consumo no suma por `NonAddingSales`, en B el consumo ES la venta): B no es "A con un if". **Invariante: el modo se congela en el SALDO, no en la config** (patrón IVA congelado de `context/38`) — leerlo del switch vigente re-factura saldos ya facturados. **D2 CERRADA: la jerarquía es el modelo** (no "del padre o del alumno": ambos), así que el vínculo entre contactos pasa a ser el corazón y la transferencia interna es operación de primera clase que NO mueve caja. **Giftcard y crédito interno son el MISMO mecanismo** (confirmado: mismo loop `SaleService.php:2100-2112`, consumo parcial, escalares sin historial, ambos en `NonAddingSales`; solo cambia el portador) — giftcard migra al motor después (D9). **Tres hallazgos que condicionan el diseño**: (1) la credencial de cliente final YA EXISTE en el schema vivo (`contact.contactPassword CHAR(68)`+salt, `PanelAuth::checkPassword()`) porque el legacy la usaba para el login del ecommerce —corrección del owner 2026-09-08—: lo que falta es acotado, `findPhoneLogin` (`functions.php:2609`) filtra `type = 0 AND ownerRoleSql` y excluye a los clientes, no queda código del login de ecommerce, y hace falta un realm `customer` (el rewrite de `context/21` dejó realm como COLUMNA, sumar uno es el mecanismo previsto). Aplica igual el MANDATO de no mezclar realms. D5: link firmado para LECTURA, realm propio para escribir topes; (2) `contactCreditLine` NO se enforcea en ninguna venta (`CustomerService.php:121,189,238` solo lo formatea) — los topes serían el primer límite que Punto evalúa al vender, con carrera entre cajas que exige evaluación atómica server-side y bloqueo offline; (3) el POS nuevo no tiene NINGUNA UI de saldo. D3-D9 propuestas sin OK. Ver §8 arquitecturas rechazadas antes de proponer nada) |
