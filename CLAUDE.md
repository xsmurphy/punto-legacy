...
| **Base de conocimiento de Punto AI (carga de .md en /admin)** | `82-base-de-conocimiento-punto-ai.md` (plan sin implementar 2026-09-17, D1 CERRADA por el owner: es para Punto AI de los COMERCIOS (panel y caja), NO para el agente de atención que se alimenta de `content/sitio/*.md` — ese contenido es comercial y no se reusa. Hoy NO hay ninguna infraestructura de recuperación (ni pgvector, ni embeddings, ni tsvector). D2-D8 PROPUESTAS sin OK: base GLOBAL sin companyId (la escribe solo Punto; lo de cada comercio es `context/69`); recuperación por FULL-TEXT de Postgres (`spanish`+`unaccent`, GIN) y NO pgvector — la BD de prod es `postgres:16-alpine`, que no lo trae, y sumarlo cambia la imagen de una base con clientes facturando; el vector, si hace falta, entra como columna híbrida sobre los mismos fragmentos. Se consulta con tool `search_punto_help`, nunca inyectada entera en el prompt. Fragmentos por ENCABEZADO; subir el mismo archivo reemplaza en UNA transacción. OJO: `unaccent` no es IMMUTABLE — el tsvector se calcula en el servicio, no en columna generada; y la caja usa ALLOWLIST de tools, hay que sumarla a propósito. Ver §6 arquitecturas rechazadas antes de proponer nada) |
