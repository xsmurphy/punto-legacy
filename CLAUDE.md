...
| **Telegram: facturas de compra por foto** | `71-telegram-facturas-compra.md` (plan sin implementar 2026-09-06, D1-D3 cerradas por el owner, P1-P8 propuestas SIN su OK — el pipeline de OCR YA EXISTE en producción (`context/32`); Telegram es solo un canal de ingesta nuevo hacia `purchase_draft`, no un pipeline nuevo. D1: un solo bot de Punto compartido por todos los comercios, así que el `chat_id` es lo único que identifica al comercio — pareo por código de un solo uso (P1), nunca por @username (se libera y se reasigna), atado a un USUARIO no al comercio (P2, evita compras sin autor). Siempre borrador, nunca compra directa (P3). Prerequisito de seguridad: `purchases.php`/`purchase-drafts.php` no tienen permission key dedicada (`context/modules/08-compras.md:45`) — hay que cerrarlo ANTES de abrir el canal. Cómo se cobra el procesamiento queda ABIERTO para el owner. Ver §Arquitecturas rechazadas antes de proponer nada) |
