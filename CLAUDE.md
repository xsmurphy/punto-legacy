...
| **App del dueño (dashboard + Punto AI en el teléfono)** | `72-app-del-duenio.md` (plan sin implementar 2026-09-07, D1 CERRADA por el owner — es una PWA APARTE ("que sean como dos apps"), no el panel instalable ni una extensión de la PWA del POS. Tres hechos la fundamentan: el manifest actual (`frontend/app/manifest.ts`) ya anticipó la decisión con `scope: "/pos"` deliberado; el POS es token-only por MANDATO (device Bearer) contra el realm `panel` por cookie del dashboard/agente, mezclarlos reintroduce el bug de multi-realm; y el agente hoy tiene `hideOnMobile: true` en `/chat` — justo lo que el owner pide es lo único que el panel esconde en mobile. P1-P4 propuestas SIN su OK) |
