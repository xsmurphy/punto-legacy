...
| **Saldo a favor por concepto (cantina escolar)** | `74-saldo-a-favor-por-concepto.md` (plan sin implementar 2026-09-08 — caso de cantina de colegio: el padre paga y factura, el alumno consume contra saldo. **D1 es BLOQUEANTE y FISCAL, no técnica**: con qué tasa de IVA se factura un anticipo cuyo consumo futuro se desconoce; hasta cerrarla el modelo de datos no se puede fijar. Lo que el cliente cree difícil —que el consumo no se duplique como venta— YA ESTÁ RESUELTO: `NonAddingSales` excluye storeCredit del total y `FinanceLedger.php:270` no mueve caja al debitar. El trabajo real son los 3 huecos: `contactStoreCredit` es UN escalar y el caso pide dos bolsillos que no se mezclen (va a ledger patrón `ai_credit_ledger`, la columna queda como proyección derivada), no hay restricción de qué paga cada bolsillo, y no existe vínculo entre contactos (padre↔alumno). **Hallazgo que cambia el alcance: el POS nuevo no tiene NINGUNA UI para saldo a favor** — el backend lo soporta desde el legacy (`SaleService::persistInCreditItem()`, pago `type='storeCredit'`) pero `frontend/` solo lee el campo; no hay forma de cargar ni de cobrar con saldo. D2-D7 propuestas sin OK. Ver §6 arquitecturas rechazadas antes de proponer nada) |
