...
| **Módulo Wallet multi-nivel** | `74-wallet-multinivel.md` (plan sin implementar 2026-09-09 — **MÓDULO NUEVO diseñado DESDE CERO**, por pedido explícito del owner: NO se diseña alrededor de giftcard ni del crédito interno existentes, que quedan como mecanismos aparte (§11 — la convergencia se decide DESPUÉS, con el módulo funcionando). Dinero prepago con jerarquía y control: titular carga, abre sub-cuentas, transfiere, y fija topes por ventana de tiempo. Rubro-agnóstico (colegio, gimnasio, comedor de empresa, flota, hotel): el schema dice titular/sub-cuenta, "padre/alumno" es UI. **D1 CERRADA: DOS MODOS** — A factura al cargar (lo que el cliente pide), B factura al entregar (la carga es cobranza anticipada). **Son flujos INVERSOS, no variantes**: el ingreso se reconoce en puntos opuestos; B como "A con un if" duplica o pierde ingresos. **Invariante: el modo se congela en el MOVIMIENTO de carga** — leerlo de la config vigente re-factura saldos ya facturados. **D2 CERRADA: la jerarquía es el modelo** (no "del que paga o del que consume": ambos), así que la transferencia interna es operación de primera clase que NO mueve caja. **La jerarquía vive en el CONTACTO, no en la wallet** (owner 2026-09-09): el consumidor es un contacto más dado de alta como hijo de otro (`contact.parentContactId`, fuente única — el modelo NO tiene `parentWalletId`); a la cuenta padre se le factura y se le acreditan las cargas, y de ahí se distribuye. **El sujeto fiscal es SIEMPRE el padre en los dos modos** — en B la factura del consumo del hijo también sale a su nombre, un menor no es sujeto de facturación. **D7 CERRADA**: por ahora el cajero busca al chico por nombre en el POS y ve su crédito al seleccionarlo — sin credencial ni QR; buscar puede ser local pero el saldo se resuelve en línea, y la lista DEBE mostrar un dato que desambigüe (grado/foto) porque seleccionar mal debita a otro. **Modelo de datos propio, 4 entidades** (`wallet`, `wallet_concept`, `wallet_movement` append-only, `wallet_limit`): el saldo es la SUMA de movimientos, prohibido un campo `balance` (si existe alguien lo pisa), cada movimiento con autor, transferencia atómica por `transferGroupId`. **Topes: se evalúan y aplican server-side en la MISMA operación que descuenta** — dos cajas leyendo "lleva 5.000 de 10.000" aprueban 6.000 cada una; y sin conexión el consumo contra wallet se BLOQUEA (estado compartido, nunca tope contra saldo local). **D3 CERRADA: sub-cuenta por PERSONA con bolsillos por concepto adentro** (no una cuenta por persona×concepto) — encaja con D7: el cajero tipea el nombre y ve UNA entrada. Consecuencia: **el bolsillo lo elige el sistema**, así que la elegibilidad (D4) deja de ser restricción y pasa a ser el MECANISMO DE RUTEO; de ahí que una venta pueda repartirse entre varios bolsillos (almuerzo + gaseosa = un débito por bolsillo en la misma operación atómica), que haga falta precedencia explícita ante ambigüedad, y que un ítem sin bolsillo elegible NO se pague con wallet. D4-D6 y D8-D11 propuestas sin OK. Ver §13 arquitecturas rechazadas antes de proponer nada) |
