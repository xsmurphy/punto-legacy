...
| **Facturación electrónica (FE-PY/SIFEN)** | `28-facturacion-electronica-plan.md`. **FE-PY es el ÚNICO motor desde 2026-09-10** — todo rastro de Factomate se eliminó del código (mig 214 dropea sus columnas y normaliza `einvoice_account.provider` a 'fepy'; el factory FALLA CERRADO ante un motor desconocido). Las migs 95/100/206 conservan sus menciones a propósito: son el registro de lo que corrió en prod. **§F7 agregada 2026-09-06 — qué pasa cuando SIFEN RECHAZA**: R1 CERRADA por el owner (al cliente final NO se le avisa del rechazo — no tiene acción posible y expone el compliance del comercio a su propio cliente; la garantía va del lado del comercio). El rechazo hoy es VISIBLE pero no AVISADO. OJO: no existe 'reenviar' — `retry()` solo va desde `status='error'` y un rechazado es `issued`; reintentarlo emitiría el documento fiscal DOS VECES (no existe 'reemitir': cada envío crea un documento nuevo). Se corrige METADATA FISCAL, nunca montos/ítems de una venta ya cobrada. Hallazgo: el portal del cliente ofrece descargar el KuDE de un documento RECHAZADO (`kudeAvailable` mira `status`, no `sifen_status`). Plazo legal de reemisión SIN confirmar. N0-N3 pendientes) |
...
| **KuDE propio — SUPERSEDED** | `73-kude-propio.md` (**REVERTIDO 2026-09-10 por el owner**: el renderer A4 propio, su caché S3 y `INTERNAL_RENDER_KEY` se ELIMINARON; el KuDE vuelve a salir siempre del motor. El motivo del plan era no depender de un TERCERO y el motor pasó a ser propio, así que un segundo dibujo del mismo documento fiscal solo podía divergir del firmado — y divergió: salió un KuDE vacío a un cliente real. SOBREVIVEN el D4 (el envío lo hace Punto, Resend) y la K3 (XML firmado a S3, ahora en `EInvoiceService::archiveSignedXml()` y bajándolo por CDC — leía un campo que solo devolvía Factomate, o sea que no archivaba nada). El logo del comercio se le manda al motor con `syncEmitterLogo()`. Leer el bloque SUPERSEDED antes de reabrirlo) |
...
| **Anulación y nota de crédito** | `40-anulacion-y-nota-credito.md` (F1/F2/F5 implementadas 2026-08-21, D2/D3 implementadas en la devolución; **F3 implementada 2026-09-10** — la NC numera con serie propia heredando la caja de la factura que corrige, mig 215; F4/F6 —nota de crédito como doctype propio + UI en `/pos`— pendientes) |
...
| **Add-ons y combos** | `41-addons-y-combos.md` (F1-F5 implementadas, D1-D3 cerradas; F6 reportes y 2 gaps de F5 pendientes. **§Grupos por CATEGORÍA reabierto 2026-09-09**, plan sin implementar: el owner quiere elegir una CATEGORÍA en vez de ítems y que el POS liste sus ítems AL VENDER. Hecho que ordena todo: esto YA EXISTÍA (`combo_group.sourceType='category'`, mig 20) y la mig 136 lo EXPANDIÓ A SNAPSHOT a propósito —nota C— para que la venta por fin leyera esos grupos; no es feature nueva, es recuperar lo que se cambió. D5 CERRADA: al elegir la categoría se marca si suma al precio final o no (BOOLEANO por grupo, no `priceDelta` por opción — un grupo de categoría no tiene fila donde ponerlo) más min/max, que ya existen en `addon_group`. Con `addsToPrice=true` el ítem suma su precio de venta VIGENTE, así que el precio del combo se mueve al retocar el catálogo: contracara asumida de la lista viva. D6-D9 propuestas sin OK — las que muerden: el backend NO puede confiar en la lista de ítems del POS (con recargo cero se lleva cualquier cosa gratis), la categoría de un ítem vive en DOS lados (`item_category` m2m + FK legacy `item.categoryId`, trampa que la mig 136 ya pisó), y offline la lista deja de bajar en el bootstrap y pasa a resolverse contra el catálogo local. NO revivir `combo_group`. Ver §Arquitecturas rechazadas antes de proponer nada) |
...
| **App del dueño (panel móvil dentro de la PWA del POS)** | `72-app-del-duenio.md` (plan sin implementar. ⚠ **D1 REVERTIDA 2026-09-09 — LEER §8 ANTES QUE §2**: es la MISMA PWA que `/pos`, una sola app instalable; §2-§3 quedaron superseded. El argumento viejo ("no pueden ser la misma app por AUTH") era INCORRECTO: el manifest no es barrera de seguridad, panel y `/pos` YA comparten origen y los dos tokens ya conviven en un browser; lo que sostiene el mandato token-only es el código (`pos-fetch.ts` con `credentials:"omit"`, `/api/pos/*` sin cookies, el guard `pos-token-only.test.ts`) y unificar la PWA no lo toca. Lo que hace innecesaria la separación: **la credencial presente ya determina la superficie** — device pareado sin cookie = solo caja (mozo/cajero); cookie sin device = solo panel (dueño con personal); las dos = todo (unipersonal). Sin modos ni rol nuevo. Cambios: `scope` a `/`, `description`, y `start_url` con ruta de arranque que derive por credencial. **`id: "/pos"` NO SE TOCA** — es opaco, cambiarlo duplica la instalación en teléfonos que ya facturan. El service worker ya está en scope `/`. Consecuencia declarada: el panel dentro de la app NO tiene precache, sin red no carga. Sigue en pie: D3 (el preset shadcn es variación de DENSIDAD, tokens bajo la clase del route group NUNCA en `:root`, y PROHIBIDO correr `shadcn init` sobre `frontend/`), D5/D6, y que la captura de facturas de `context/71` va acá y no en el POS. **§9 (2026-09-16, en implementación)**: primer uso sin trabas — `/pos` en un navegador sin device pero con sesión de panel y `settings.device.pair` se parea solo (invitación auto-aprobada por el panel + canje normal de un solo uso; el panel NUNCA emite token de device) y toma la caja si está libre; sin PIN cuando la sucursal tiene UN solo usuario, verificado server-side al emitir la afirmación de operador. Ojo: el panel ya no es cookie, es Bearer en localStorage (context/54). Ver §8.5 arquitecturas rechazadas antes de proponer nada) |
...
| **Módulo Wallet multi-nivel** | `74-wallet-multinivel.md` (**Reversa de cargas 2026-09-18 — leer §14**: anular (`SaleVoidService`) o devolver con NC (`ReturnService`) una venta de CARGA revierte el saldo en la MISMA transacción con el tipo propio `load_reversal` (mig 236: atado a la carga por `reversesmovementid`, trigger que impide revertir más de lo cargado), bajo el lock del bolsillo; si el cliente ya usó parte, la anulación/NC ENTERA se rechaza (409 `WALLET_LOAD_USED`) — nunca negativo, nunca parcial en silencio; NC parcial de una venta con cargas en 2+ bolsillos se rechaza (va entera); la anulación legacy tipo→7 se niega sobre documentos que movieron saldo; "Cargado" del reporte es neto de devoluciones; §14.5 tiene la query de diagnóstico de cargas anuladas/devueltas antes de la mig. **Reporte de bolsillos 2026-09-18 — leer §13**: pestaña Bolsillos de Ventas (solo con el módulo), cierra el P1 "monto del consumo confiado desde el POS" como CONTROL: el consumo congela en cada línea el valor de LISTA resuelto en el servidor (`itemsold.itemsoldlisttotal`, mig 235 — no era derivable: el bruto de la línea lo elige la caja y el total es el `subtotal` del payload) y el reporte muestra lo debitado por debajo de lista, separando descuento registrado de diferencia sin explicación, por usuario y por caja; el saldo por entregar es de TODO el comercio, no se acota por sucursal. **F2 implementada 2026-09-18 — leer §12**: la carga es una VENTA con línea `walletLoad` (ítem de sistema `item.systemkey`, impuesto del bolsillo `wallet_pocket.taxid`, mig 234, `load` dentro de la transacción, offline sí); el consumo es un COMPROBANTE INTERNO `transactionType=15` por `/v1/pos-wallet` (stock+COGS+`spend` atómico, sin FE/caja/ingresos — el tipo queda fuera de toda lista blanca de reportes, D13; la venta interna por tag se descartó); si no alcanza se emite una carga de la diferencia y después el consumo entero (D14); permisos `pos.wallet.load/spend` contra el operador del PIN. **F1 implementada 2026-09-18 — leer §11**: mig 232 con las invariantes EN LA BD (CHECK no-negativo, trigger de encadenado `balanceafter`, append-only con única excepción la purga del tenant vía `punto.tenant_purge`), `WalletService` con TODAS las operaciones (load/spend/refund/adjust/transfer/setParent) bajo `pg_advisory_xact_lock` por contacto+bolsillo y anidable dentro de la venta de F2, `/v1/wallet` panel-only (bolsillos + saldos + movimientos + ajuste), permisos `wallet.view/manage`, módulo activable `wallet`; UI sin menú propio: bolsillos en Ajustes → Catálogo, saldo en la pestaña Financiero del cliente. F2 caja / F3 hijos+transferencias en UI pendientes. Plan original: v1 recortada 2026-09-16 — módulo NUEVO desde cero, NO sobre giftcard ni crédito interno. v1: cargar saldo con una VENTA (modo A, factura al titular), bolsillos por comercio (Almuerzo, Merienda), hijos como contactos (`contact.parentContactId`, fuente única de la jerarquía), transferencia titular→hijo por bolsillo, y pagar con saldo en la caja. **El tope ES el saldo del bolsillo** (owner) — no hay `wallet_limit`, y por eso un bolsillo NUNCA queda negativo; como el cajero elige el bolsillo (D4), el tope es tan firme como el cajero. Modelo: `wallet_pocket` + `wallet_movement` append-only, saldo = suma, sin campo `balance`, cada movimiento con autor, transferencia atómica. Chequeo de saldo y descuento en UNA operación server-side; sin conexión, pagar con saldo se bloquea. La v1 graba el modo de facturación en cada carga para que el modo B (factura al consumir, fase posterior) no refacture. Interfaz de lectura para titular e hijo: fase posterior. **Plan COMPLETO, todas las decisiones de la v1 cerradas**: si el bolsillo no alcanza la diferencia se cobra con otro medio en la misma venta (D6); los hijos quedan ocultos por defecto en los listados de clientes (D11)) |
| **Lotes y vencimientos** | `75-lotes-y-vencimientos.md` (**PLAN con D1-D8 cerradas por el owner 2026-09-15/16**, sin implementar — reemplaza el análisis "referencial" del 09-10: el pedido real es TRAZABILIDAD legal (importadoras/veterinarias), así que el lote se asigna también en las SALIDAS. Lo que destraba: **lote ≠ método de costeo** — el lote es eje de CANTIDAD del ledger (como el depósito) y el costo sigue siendo el promedio ponderado del ítem; contracara aceptada: el margen por lote no es real. `stock.lotId` + invariante en `manageStock()` (ítem con lotes sin lote se rechaza; el "sin lote" es una FILA, nunca NULL), saldo por lote = `SUM(stockCount)` derivado. Venta: FEFO sugerido y el cajero corrige, UNA línea = UN lote. Dos avisos: stock propio y VENDIDO por cliente (distribuidoras); días de aviso SOLO por ítem (controla lotes + días), el aviso de vendido es UN interruptor de empresa (D6). D7: no complica la operativa — el dato se carga solo en la compra, todo lo demás lo asigna el sistema; el stock previo queda en "sin lote" y FEFO lo vende primero (no hay que contar para arrancar); D8: un insumo con lotes recibe el mismo trato que cualquier producto — producción lo consume FEFO sin paso nuevo y le notifica el vencimiento igual. Cambio de lote al cliente = devolución + venta nueva. La tabla `inventory` se DROPEA, no se revive; "lote" ya es `production_batch`, la tabla nueva es `stock_lot`. No habilitar a clientes antes de F2. Ver §8 arquitecturas a evitar antes de proponer nada) |
| **Producción: KPIs de consumo, mermas y rendimiento** | `76-produccion-kpis.md` (plan 2026-09-10, D1-D5 PROPUESTAS sin OK — el reporte es básico porque la BASE no guarda lo necesario: `recipesnapshot` guarda UN qty por insumo y el teórico se PIERDE cuando hay ajuste, los lotes confirman con real = teórico siempre, la merma de insumos no es evento, y el ledger apunta a la orden solo por TEXTO en la nota. F1 (mostrar lo que el backend YA devuelve: `view=waste/detail/compound`) no depende de nada; F2 (desvío) requiere F0 y vale solo hacia adelante. Bug: el costo de unidades falladas se cuenta DOS veces (repartido en `unitCogs` y otra vez en `waste_event.cost`). Ver §6 antes de proponer nada) |
| **Migrador ENCOM → Punto** | `77-migrador-encom.md` (**F1 implementada 2026-09-11**, D1-D6 cerradas — soporte carga las credenciales del cliente en `/admin`, un worker CLI exporta del panel legacy e importa por los SERVICIOS REALES; `migration_map` da idempotencia. La password NO se persiste: el endpoint hace el login en el momento y guarda solo las cookies, que el worker borra al terminar. **Las cajas CONTINÚAN su serie fiscal** —`document_sequence` nace con el timbrado y el punto del legacy y `nextnumber` = último emitido + 1, porque el contador legacy guarda el ÚLTIMO y el de Punto el PRÓXIMO— y dos cajas con el mismo (timbrado, EEE-PPP) abortan el dominio ENTERO sin importar ninguna. **§4 es lo primero que hay que leer: el deploy VIVO es MÁS VIEJO que el snapshot de código.** Verificado contra el sistema real 2026-09-11: `/bff/*` da 404 y `/API/get_*` da "Acceso denegado" incluso con sesión válida, así que TODO el export va por `a_*.php?action=` con cookie PHPSESSID — se parsea CSV y HTML, no JSON. Los valores crudos viajan en `data-order` (contactos/transacciones) o `data-sort` (artículos), nunca en el texto formateado; el id de fila es `data-id` salvo en artículos, que usan `id`. El CSV de contactos se indexa POR NOMBRE de columna porque el vivo trae `TELEFONO 2` y el snapshot no, y la columna del RUC es `TIN_NAME`, que cambia por país. Las cajas se recorren con el switch global `?o=<outletId>`, que necesita DOS requests (redirige sin query string y la constante se fija antes), y se DEDUPLICAN por id porque no está garantizado que el listado esté acotado a la sucursal activa. **Las columnas se resuelven por ENCABEZADO, nunca por índice**: el listado vivo tiene una columna `Sucursal` que el snapshot no tiene y que corría un lugar todo lo fiscal (el nombre de la sucursal se leía como timbrado); y el prefijo viene con GUIÓN FINAL (`009-001-`), que el regex `^\d{3}-\d{3}$` habría rechazado abortando el dominio entero. **VERIFICADO contra el sistema vivo: `registerInvoiceNumber` es el ÚLTIMO emitido, así que el +1 de la continuación de serie es correcto.** **F2 (HISTÓRICO) implementada 2026-09-11 — leer §17**: ventas con sus líneas, compras y movimientos de caja como REGISTRO CONTABLE, en tres dominios propios (`sales_history`/`purchases_history`/`expenses_history`) que NO están tildados por defecto (el legacy no tiene detalle por rango de ventas: es una request POR VENTA, paceada, y un año es un job de horas). **§17.1 es la regla que manda: el histórico NO toca stock, caja, numeración fiscal ni FE**, y por eso es la ÚNICA parte del migrador que no pasa por los servicios de negocio — `SaleService::save()` haría justo esas cuatro cosas, así que hay un camino de escritura propio y acotado en `EncomHistoryImporter`. Tres hallazgos verificados contra Postgres real que cambian supuestos: **un INSERT con fecha vieja NO falla, cae en la partición DEFAULT y NUNCA se reclasifica** (`ensure_month_partitions()` se ancla en la partición más vieja ya creada y a propósito no va hacia atrás) — de ahí la mig 221, que extrae el motor a `ensure_month_partitions_range()` con tope de 120 meses; **el guard de cierre de período es BEFORE UPDATE OR DELETE únicamente, así que un INSERT en un mes cerrado entra** y el chequeo lo tiene que hacer el importador con `period_is_closed()`; y `transaction` no tiene columna de origen (la única parecida, `channel`, tiene CHECK cerrado), así que la marca de importado es `migration_map` + `meta.importedFrom`. **El COGS SÍ se escribe por línea** (corrección del owner): los reportes leen el costo CONGELADO por línea y no lo recalculan, así que con la columna vacía el margen histórico no existe. Se congela `item.itemCost` —el costo ACTUAL, no el del día de la venta, que el legacy no expone—, con el contrato de `SaleService` copiado: costo UNITARIO (no el de la línea), pasado por `flipOnReturn()`, y OMITIDO del insert cuando no se sabe (escribir null no alcanza: `flipOnReturn(null)` devuelve 0 y un 0 se lee como "costó nada" = margen 100%). Conviene cargar los costos ANTES de importar: cargárselos después NO corrige los asientos ya escritos. Falta cargar `ENCOM_MIGRATION_URL` en Coolify y una corrida real; lo que sigue asumido está en §13 y §17.10. Ver §15 y §17.11 arquitecturas rechazadas antes de proponer nada) |
| **Integración Fish (CRM): contactos + atributos para campañas** | `78-integracion-fish.md` (plan cerrado 2026-09-16, D1-D7, sin implementar — modelo Intercom: Punto pushea contactos con atributos custom, los SEGMENTOS viven en Fish, Punto no construye motor de segmentación. Push unidireccional server-side por evento vía outbox (el JS snippet quedó RECHAZADO como transporte: el cliente del POS no tiene browser y el tenant churneado es justo el que no entra). Atributos = hechos crudos tipados, PROHIBIDA ventana móvil (se pudre sin eventos). OJO type-freeze de Fish: el campo se crea con el tipo del PRIMER valor — PDO devuelve strings, el mapper castea explícito y OMITE keys null. `externalId` = contactId, teléfono E.164 CON `+`, duplicados NO se fusionan. Backfill inicial obligatorio. Contrato en §4 pendiente de confirmación de la sesión Fish. Ver §7 arquitecturas rechazadas antes de proponer nada) |
| **Órdenes a fecha futura** | `79-ordenes-a-fecha-futura.md` (plan sin implementar 2026-09-16, D1-D5 PROPUESTAS sin OK — prerequisito concreto de la etapa A/B de `context/70`: el "lote del día" asume una agrupación por fecha que NO existe. Verificado: `pos_order` no tiene fecha de entrega (solo created/sent/closed), `OrderDemandService` trae TODA la cola sin filtro temporal, y la "agenda" del POS es OTRO dominio (citas sobre `transaction.fromDate`, sin líneas de cocina — no reusar). Diseño chico: columna real `pos_order.scheduled_for` con índice (NUNCA en el JSONB `data` — se filtra/agrupa por ella), captura opcional al tomar el pedido (NULL = para ahora, nada cambia), el lote filtra por UNA fecha ("hoy" incluye las sin fecha Y las vencidas no producidas), y el KDS NO muestra lo futuro hasta su día (contracara obligatoria). Sin reserva de stock (coherente con `context/53`) y sin señas (eso es wallet/`context/74` o prepago/`context/70`). Ver §6 arquitecturas rechazadas antes de proponer nada) |
| **Voz del agente (TTS por OpenRouter)** | `80-voz-del-agente.md` (plan sin implementar 2026-09-17, D1 cerrada: se cobra del crédito IA del tenant. OpenRouter YA tiene TTS (endpoint `/api/v1/audio/speech`, catálogo Kokoro/Gemini/Grok) — la voz actual es `speechSynthesis` del navegador en UN archivo (`message-actions.tsx`) y el billing entra por el `billing-gate.ts` existente (gate fail-closed + débito best-effort), NUNCA un débito propio. D2-D6 propuestas: Kokoro default configurable en /admin, cobro por caracteres mapeado a tokens con `reason=tts`, fallback a la voz nativa con toast, v1 solo panel (el POS requeriría BFF propio con Bearer device), blob sin streaming) |
| **Lote compartido: costo de un lote repartido entre pedidos (lavandería + viandas)** | `81-lote-compartido-costo-por-pedido.md` (plan sin implementar 2026-09-17, D1-D8 PROPUESTAS sin OK — un lote atiende pedidos de N clientes, consume insumos UNA vez y reparte su costo. Se EXTIENDE `production_batch` (no entidad "lavado"); el proceso es un ítem tipo `proceso` con receta y SIN stock (consume sin ingreso — lo decide el ítem, D1 de context/70); indirectos v1 = `insumo_sin_stock` con costo estándar; `production_batch_order(basisqty, allocatedcost)` congelado al confirmar; receta en el proceso O en el servicio vendido, nunca en los dos (doble consumo); margen por pedido sale del lote, NO se reescribe `itemsold`. Hallazgo: servicio sin receta congela COGS 0 en vez de NULL. Ver §8 arquitecturas rechazadas antes de proponer nada) |
| **RAG de ayuda para Punto AI** | `82-base-de-conocimiento-punto-ai.md` (plan sin implementar 2026-09-17, D1-D4 CERRADAS por el owner: es para Punto AI de los COMERCIOS; UNA sola burbuja (Punto AI) y el CRM de soporte queda FUERA DE ALCANCE — es otro sistema con su RAG propio, no se integra todavía y NO se dejan ganchos preparados; mientras tanto el agente no nombra ningún canal de soporte al no encontrar un artículo; el RAG se carga SOLO desde /admin y NADA se indexa solo — `frontend/content/ayuda/` es la fuente de `docs.punto.la` y punto, el bot NO la indexa: el owner carga lo que quiere que el bot sepa (contexto general, conocimiento por RUBRO, casos raros) y, si quiere, IMPORTA artículos de ayuda a mano (queda la deriva declarada: si el artículo cambia en git, se avisa, nunca se reimporta solo); ojo: lo cargado no se publica pero SÍ lo puede leer cualquier comercio vía el bot; y la base es una Postgres+pgvector APARTE, exclusiva del RAG — la de los tenants no se toca. OJO: prod es `postgres:18-alpine`, no la 16 del docker-compose local. D5-D10 PROPUESTAS sin OK: embeddings por OpenRouter `/api/v1/embeddings` (existe; la cuenta OpenAI no hace falta) con modelo FIJO por índice — mezclar modelos da resultados sin sentido y sin error; solo el BACKEND habla con la base del RAG (el Front no tiene driver); la imagen del backend NO contiene `frontend/`, así que el Front manda un manifiesto al arrancar y el backend embebe solo lo que cambió por hash; búsqueda HÍBRIDA vector+tsvector con las `keywords` del frontmatter; tool `search_punto_help` que la caja recibe solo si se suma a su ALLOWLIST. La base del RAG no necesita backup: se reconstruye desde git. Ver §6 arquitecturas rechazadas antes de proponer nada) |
| **RRHH básico + marcación (quiosco facial)** | `83-rrhh-y-marcacion.md` (**plan CERRADO 2026-09-17, D1-D10 aprobadas por el owner; **D9 SUPERSEDED 2026-09-17: RRHH es CORE, no togglable** — regla general: módulo base de todos los rubros (facturación/compras/inventario/RRHH) nunca es activable, activable es lo específico de rubro (producción/ecommerce/espacios/órdenes/calendario). F0 legajo + F1 quiosco PIN+foto + F2 facial DEPLOYADAS 2026-09-17; F3 ausencias y F4 liquidación pendientes — legajo (`employee` entidad propia, vínculo OPCIONAL a `user`), asistencia, ausencias, adelantos y liquidación simple; esquemas de remuneración fijo/por hora/comisión COMBINABLES; las comisiones son un TARIFARIO patrón lista de precios (regla %/monto fijo por producto/categoría, override por empleado, la más específica gana) y se CONGELAN en la línea de venta como el IVA/COGS — la liquidación suma lo congelado, cambiar la regla no re-liquida; recibo de dinero del adelanto por plantilla de impresión y liquidación A4 react-pdf. La marcación INVIERTE el modelo legacy: dispositivo DEL COMERCIO en modo quiosco — QR y geolocalización mueren con el caso principal (existían para probar presencia). La cara IDENTIFICA (1:N on-device, embeddings server-side cacheados por sucursal), NUNCA bloquea: PIN de respaldo, foto del momento SIEMPRE, mismatch = flag, fail-open como la venta offline. Biometría = dato sensible: consentimiento en legajo, borrado al egreso, prohibido mandarla a terceros. Adelantos y pagos van por el ledger de Finanzas, nunca un libro paralelo. Resuelve el pendiente del roadmap 2026-08-30: `attendance` catalogado como available sin UI ni generador de QR — se completa acá y el token derivable muere. Ver §8 arquitecturas rechazadas antes de proponer nada) |
