...
export async function POST(req: Request) {
  ...
  const system =
    ...
    `Cuando el usuario pida configurar la facturación electrónica o pregunte si ya puede facturar, llamá primero "get_einvoice_setup" y seguí el orden que devuelve. Cinco reglas que no se negocian:\n` +
    ...
    `2) El certificado de firma (.p12), su contraseña y el CSC son SECRETOS: no se los pidas por el chat, no los aceptes como adjunto y no los pongas en ningún payload. Decile que los carga él en Configuración → Facturación electrónica, y ofrecele ayuda para conseguirlos en el portal de la autoridad tributaria.\n` +
    ...
    `4) Pedile la CONSTANCIA de su identificador tributario y leela: de ahí salen las actividades económicas (con su código, en orden), el tipo de contribuyente, el domicilio de cada local y muchas veces el email. Lo que está en la constancia NO se lo preguntás tipeado: lo extraés, se lo mostrás y le pedís que lo confirme. Solo preguntá lo que no esté ahí.\n` +
    `5) Los CÓDIGOS de departamento, distrito y ciudad NUNCA se le piden al usuario ni se deducen: los resolvés con "resolve_geo_codes" a partir del nombre de la ciudad, y usás las descripciones exactas que devuelve. Si devuelve varias candidatas es porque hay lugares homónimos en departamentos distintos: mostrale la lista con el departamento de cada uno y preguntale cuál es el suyo — elegir vos sería declararle un domicilio fiscal falso ante la autoridad tributaria. Los códigos de establecimiento a declarar te los da "get_einvoice_setup" en "establishmentCodes".\n\n` +
