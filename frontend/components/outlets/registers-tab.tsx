...
const EMPTY_FISCAL: RegisterFiscal = {
  ...
  // Serie SIFEN (mig 223): vacía por defecto. Nunca se precarga un valor.
  invoiceSerie: "",
  creditNoteSerie: "",
}

/**
 * Talonarios cuya serie cambia con este guardado. La serie SIFEN es IDENTIDAD
 * de la serie fiscal (mig 223): cambiarla abre una numeración nueva, así que el
...
function serieWithoutPrefix(fiscal: RegisterFiscal): boolean {
  const hasSerie = normalizeSerie(fiscal.invoiceSerie) !== "" || normalizeSerie(fiscal.creditNoteSerie) !== ""
  return hasSerie && fiscal.invoicePrefix.trim() === ""
}

/**
 * ¿Las series del form se pueden guardar? Dos letras o vacías, y nunca sin
 * punto de expedición: la serie es de un punto (mig 223).
 */
function seriesValid(fiscal: RegisterFiscal): boolean {
  return isValidSerie(fiscal.invoiceSerie) && isValidSerie(fiscal.creditNoteSerie) && !serieWithoutPrefix(fiscal)
}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="sm:max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button
              disabled={!newName.trim() || !seriesValid(newFiscal) || createRegister.isPending}
              onClick={() => {
...
/**
 * Serie SIFEN de un talonario. Se escribe en mayúsculas mientras se tipea y no
