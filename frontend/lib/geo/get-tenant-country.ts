 ...
 * `app/api/agent/chat/route.ts` (fetch a `/v1/settings` con el Bearer del
 * panel reenviado) — fail-open: si no hay credencial, el fetch falla o el owner
 ...
 *
 * `/v1/settings` solo acepta el realm `panel` (Bearer del panel), no el
 ...
 * crítico.
 */
export async function getTenantCountry(authHeader: string): Promise<string | null> {
  const apiUrl = process.env.API_URL ?? process.env.NEXT_PUBLIC_API_URL ?? ""
  if (!apiUrl || !authHeader) return null
  try {
