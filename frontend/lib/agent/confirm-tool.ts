...
const payloadSchema = z.object({
  ...
  establecimientos: z
    ...
    .describe(
      ...
      "El TELEFONO es OBLIGATORIO y de al menos 6 caracteres: termina en el XML como el teléfono del emisor (dTelEmi) y SIFEN rechaza el documento si viene vacío — no es opcional aunque nuestro motor deje crear el emisor sin él. Sacalo de get_outlets si la sucursal lo tiene, y si no, PEDISELO al usuario. " +
      "email y denominacion del establecimiento sí son opcionales: no se los pidas, mandá los que ya conozcas por get_outlets y dejá vacío el resto. numeroCasa también es opcional — si la dirección no tiene altura, omitilo y el backend declara la convención de SIFEN para 'sin número'. " +
