...
// Formateador único del correlativo (mig 158) — ver
...
export function buildTicketData({ payload, result, config }: BuildTicketDataInput): TicketData {
  ...
  return {
    ...
    //
    // mig 158: se pinta con los ceros a la izquierda del talonario
...
export function buildTicketDataFromTransaction(
  ...
  return {
    ...
    docType,
    // mig 158 — mismo formateador que la venta recién cobrada, para que el
...
export interface TicketableTxDetail {
  transaction: {
     ...
     * (`DocumentNumber::pad`, mig 158) y prefijo del timbrado. Opcionales
