...
if(validateHttp('action') == 'generalTable'){
	...
	if($result){
		...
		while (!$result->EOF) {
			...
			if($fields['id'] && $fields['id'] > 0){
				...
				$loyalty		= formatCurrentNumber($customer['loyalty']);

				$strBday 		= strtotime($customer['bday']);
				$strBday 		= ($strBday < strtotime('2018-01-01 00:00:00')) ? $strBday : false;

				$bday				= $strBday ? date('m-d',strtotime($strBday)) : '';
				$city				= $customer['city'];
				...
				$table .= 	'<tr class="clickrow pointer" data-id="' . enc($cId) . '">' .
									  ...
									  '	<td> '.$ruc.' </td>' .
										...
										'	<td> '.$city.' </td>' .
										'	<td> '.$tags.' </td>' .
										'	<td class="text-right" data-order="'.$loyalty.'"> '.$loyalty.' </td>' .
										'	<td class="text-right" data-order="'.$compras.'"> '.$compras.' </td>' .
	...
	if(validateHttp('part')){
		...
		$jsonResult['global'] 	= [
										'sales' 	=> $currency . formatCurrentNumber($tTotal), 
	...
	var xhr = ncmHelpers.load({
		...
		success 	: function(result){
			...
			var options = {
						...
						"sort" 			: 0,
						"footerSumCol" 	: [12,13,14,15,16,17],
						"currency" 		: "<?=CURRENCY?>",
						"decimal" 		: decimal,
						...
						"colsFilter"	: {
												name 		: 'reportCustomers5',
												menu 		:  [
																{"index":0,"name":'Razón Social',"visible":true},
																{"index":1,"name":'<?=TIN_NAME?>',"visible":false},
																...
																{"index":14,"name":'Uni. adquiridas',"visible":false},
																{"index":15,"name":'Descuentos',"visible":false},
																{"index":16,"name":'Subtotal',"visible":false},
																{"index":17,"name":'Total',"visible":true}
																]
											  },
