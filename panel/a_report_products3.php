...
if(in_array(validateHttp('action'), ['detailTable','generalTable'])){
	...
	$limits		= ($export || validateHttp('action') == 'generalTable') ? '' : $limits;
	...
	if(validateHttp('src')){
		...
		if(validateHttp('cusId')){
			...
			$result 	= ncmExecute($sql,[$startDate,$endDate,$contactUID],true,true);
			...
			$sql = 'SELECT 
						...
						b.userId as itemUser
					FROM transaction a USE INDEX(transactionType), itemSold b
					WHERE a.transactionType IN(0,3)
					AND b.itemId = ?
					' . $roc . '
					AND a.transactionId = b.transactionId
					ORDER BY a.transactionDate DESC';
...
if(validateHttp('action') == 'detailTable'){
	...
	if($result){		
		...
		while (!$result->EOF) {
			...
			$itm 		= $allItems[$fields['itemId']];//getItemData($id,true);
		    ...
		    //customer data
		    $customer 	= $allContacts[$fields['customer']];
			...
			$customerDatas 	= join(' › ', array_filter([$customer['contactPhone'],$customer['contactEmail']]));
			...
			$table .= 	'<tr data-load="/a_report_transactions?action=edit&id=' . enc($tId) . '&ro=1" class="clickrow pointer">' .
						'	<td>' . $outlet . '</td>' .
						'	<td>' . $register . '</td>' .
						...
						'	<td class="' . (($status < 1) ? 'text-l-t' : '') . '"> ' . $name . $typeName . ' </td>' .
						'	<td> ' . $sku . ' </td>' .
						'	<td> ' . $brand . ' </td>' .
						'	<td> ' . $category . ' </td>' .
						'	<td class="text-right bg-light lter" data-order="' . $uSold . '"> ' . formatQty($uSold) . ' </td>' .
						'	<td class="text-right bg-light lter" data-order="' . $earned . '" data-format="money"> ' . $earnedf . ' </td>' .
						...
						'	<td class="text-right bg-light lter" data-order="' . $tax . '" data-format="money"> ' . 
								$taxf .
						'	</td>' .
						'	<td class="text-right bg-light lter" data-order="' . $discount . '" data-format="money"> ' . $discountf . ' </td>' .
						'	<td class="text-right bg-light lter" data-order="' . $utility . '" data-format="money"> ' . $utilityf . ' </td>' .
...
if(validateHttp('action') == 'generalTable'){
	...
	if($resultB){
		...
		while (!$resultB->EOF) {
			...
			$comission 	= (float) $fields['itemSoldComission'];
			$tax 		= (float) $fields['itemSoldTax'];
			$cogs 		= (float) $fields['itemSoldCOGS'];
			...
			if($fields['itemSoldTotal'] > 0){
				$soldBack[$fields['itemId']] = $fields['itemSoldUnits'];
			}
	...
	if($result){
		...
		while (!$result->EOF) {
			...
			if($generalList[$fields['itemId']]){
				$generalList[$fields['itemId']]['itemSoldUnits'] 		+= $uSold;
				$generalList[$fields['itemId']]['itemSoldDiscount'] 	+= $discount;
				$generalList[$fields['itemId']]['itemSoldTotal'] 		+= $total;
				$generalList[$fields['itemId']]['itemSoldComission'] 	+= $comission;
				$generalList[$fields['itemId']]['itemSoldTax'] 			+= $tax;
				$generalList[$fields['itemId']]['itemSoldCOGS'] 		+= $cogs;
				$generalList[$fields['itemId']]['utility'] 				+= $utility;
			}else{
				$generalList[$fields['itemId']] = 	[
														'itemSoldUnits' 	=> $uSold,
														'itemSoldDiscount' 	=> $discount,
														'itemSoldTotal' 	=> $total,
		...
		$x 			= 0;
		foreach ($generalList as $id => $fields) {
			...
			if(!validateHttp('month')){
				...
				$name			= $meses[$month];
				$gname			= $name . ' Cant. ' . formatQty($fields['itemSoldUnits']) . ' ' . toUTF8($itm['itemOUM']);
				$nameSort 		= $month;
			...
			if(in_array($type, ['precombo','combo'])){//dinámico y predefinido
				//$ftotal 	= '-';
				$ftax 		= '-';
				$fdiscount	= '-';
				$futility 	= '-';
			...
			$table .= 	'<tr class="' . $parentId . ' clickrow pointer" data-url="' . $dataUrl . '">' .
						...
						'	<td> ' . $brandName . ' </td>' .
						'	<td> ' . $categoryName . ' </td>' .
						'	<td class="text-right bg-light lter" data-order="' . $price . '"> '.formatCurrentNumber($price).' </td>' .
						'	<td class="text-right bg-light lter" data-order="' . $uSold . '">' . 
								formatQty($uSold) . 
						' 	</td>' .
						'	<td class="text-right bg-light lter" data-order="' . $comission . '" data-format="money">' . 
								$fcomission . 
						'	</td>' .
						...
						'	</td>' .
						'	<td class="text-right bg-light lter" data-order="'.$discount.'" data-format="money">' . 
								$fdiscount . 
						'	</td>' .
						'	<td class="text-right bg-light lter" data-order="'.$utility.'" data-format="money"> ' . 
								$futility . 
						' 	</td>' .
						'	<td class="text-right bg-light lter" data-order="'.$total.'" data-format="money">' .
								$ftotal .
						' 	</td>' . 
						'</tr>';
...
$(document).ready(function(){
	...
	var loadUrl 	= rawUrl + "&itmId=<?=validateHttp('ii')?>&month=<?=validateHttp('m')?>&year=<?=validateHttp('y')?>&cusId=<?=validateHttp('ci')?>&usrId=<?=validateHttp('ui')?>";
	...
	$('#detailTabLink').on('shown.bs.tab', function (e) {
	    if(!detailTableOpen){
			...
			var loadUrl 	= rawUrl + "&itmId=<?=validateHttp('ii')?>&cusId=<?=validateHttp('ci')?>&usrId=<?=validateHttp('ui')?>";
