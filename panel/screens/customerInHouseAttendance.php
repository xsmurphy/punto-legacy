...
define('BALANCE', $companyData['balance']);
...
if(validateHttp('phone')){
  ...
  $contact  = ncmExecute('SELECT contactName as name, contactSecondName as secondName, contactId FROM contact WHERE (contactPhone = ? OR contactPhone2 = ? OR contactCI = ?) AND companyId = ? LIMIT 1',[$phone,$phone,$phone,COMPANY_ID]);
  ...
  if($contact){
    ...
    $expired  = ncmExecute('SELECT transactionId 
                            ...
                            DESC LIMIT 1',[TODAY,$contact['contactId'],COMPANY_ID]);
    ...
    if($expired){
      ...
      $toExpire  = ncmExecute('SELECT transactionDueDate
                                ...
                                AND companyId             = ? 
                                ORDER BY transactionDueDate 
                                DESC LIMIT 1',[$contact['contactId'],COMPANY_ID]);
      $msg       = '';
    ...
    $atendance = ncmExecute('SELECT transactionId FROM transaction WHERE customerId = ? AND transactionType = 13 AND transactionStatus IN(0,1) AND fromDate > ? AND toDate < ? AND companyId = ? AND outletId = ? LIMIT 1',[$contact['contactId'],TODAY_FROM,TODAY_TO,COMPANY_ID,OUTLET_ID]);
    ...
    if($atendance){
      ...
      //Inserto asistencia
      $db->AutoExecute('taxonomy', ['taxonomyName'=>$contact['contactId'],'taxonomyType'=>'customerAssistanceFromWidget','sourceId'=>$atendance['transactionId'],'companyId'=>COMPANY_ID], 'INSERT');
...
<?php
loadCDNFiles(['/assets/vendor/js/libphonenumber-1.6.8.min.js'],'js');
?>
