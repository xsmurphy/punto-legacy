/**
 * Front del reporte Ventas por Categorías (a_report_categories) — BFF de 3 niveles.
 *
 *   - config (currency/decimal/thousand/taxName) ← GET /bff/bootstrap.php
 *   - datos (filas por categoría + totales, CRUDOS) ← GET /bff/reports/categories.php
 *
 * El BFF manda datos crudos + el % por fila (REGLA RAÍZ 2); este JS formatea, arma KPIs +
 * treemap + tabla (con barra de %), y escapa los datos. Ver context/02-arquitectura.md.
 */
(function () {

	var BFF       = '/bff/reports/categories.php';
	...
	function buildTable(rows) {
		var head =
			...
			'<th class="text-center">Total</th><th class="text-center" style="max-width:15%">Porcentaje</th>' +
			'</tr></thead><tbody>';

		var body = '';
		$.each(rows, function (i, r) {
			var usold = parseFloat(r.usold) || 0, cogs = parseFloat(r.cogs) || 0,
			    tax = parseFloat(r.tax) || 0, disc = parseFloat(r.discount) || 0, total = parseFloat(r.total) || 0;
		...
		var foot = '</tbody><tfoot><tr><th>TOTALES</th>' +
			...
			'<th class="text-right"></th><th class="text-right"></th><th class="text-right">100%</th></tr></tfoot>';
	...
	function drawTreemap(rows) {
		var tree = [];
		$.each(rows, function (i, r) {
			tree.push({ title: r.name === 'None' ? 'Sin categoría' : r.name, total: parseFloat(r.usold) || 0 });
		});
		...
		var xhr = ncmHelpers.load({
			...
			success: function (res) {
				...
				ncmDataTables({
					...
					"offset": 0, "limit": 500, "nolimit": false, "noMoreBtn": true,
					"tableName": 'tableCategories', "fileTitle": 'Ranking de Categorías',
					"ncmTools": { left: '', right: '' }
				});
