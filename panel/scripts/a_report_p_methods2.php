...
if(validateHttp('action') == 'generalTable'){
	...
	$result   	= ncmExecute("SELECT transactionId,
								...
								BETWEEN ? 
								AND ? 
								" . $roc,[$startDate,$endDate],false,true);
	$table = '';
		...
		while (!$result->EOF) {
			...
			if($new){
				foreach($new as $method => $meth){
					...
					$pName 	= getPaymentMethodName($meth['type']);

					$table .= 	'<tr data-load="/a_report_transactions?action=edit&id=' . enc($fields['transactionId']) . '&ro=1" class="clickrow" data-type="' . $meth['type'] . '">' .
