...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (COMPANY_ID === 4456) {
		...
		if ($file) {
			...
			// $mail->setFrom(EMAIL_NOTIFICATION, 'Comprobante');
			// //Set who the message is to be sent to
			// $mail->addAddress(EMAIL_NOTIFICATION_TO);
			// //Set the subject line
			// $mail->Subject = toUTF8('Comprobante transferencia bancaria');
			// $mail->isHTML(true);
			// // Convert all message body line breaks to CRLF, makes quoted-printable encoding work much better
			// $mail->Body = toUTF8("RUC: $customerRuc<br>");
			// $mail->Body .= toUTF8("Cliente: $customerName<br>");

			// $docType = 'del comprobante: ';
			// if (in_array($result['transactionType'], [0, 3])) {
			// 	$mail->Body .= toUTF8("Factura Nro.: " . iftn($result['invoicePrefix'], $register['registerInvoicePrefix']) . str_pad($result['invoiceNo'], 7, "0", STR_PAD_LEFT) . "<br>");
			// 	$docType = 'de la factura: ';
			// } else {
			// 	$mail->Body .= toUTF8("Comprobante Nro.: " . ($result['invoiceNo'] ?? '') . "<br>");
			// }

			// $totalAmount = formatCurrentNumber($result['transactionTotal'] - $result['transactionDiscount']);

			// $mail->Body .= toUTF8("Monto $totalAmount<br>");

			// $mail->Body .= "<p>Datos De Transferencia Bancaria Adjuntos</p>";

			// $mail->addStringAttachment(file_get_contents($fileTmpPath), $fileName);
...
<body class="bg-light lter col-xs-12 no-padder">
	<section class="vbox" id="content">
		...
		<div class="wrapper text-center col-md-4 col-md-offset-4 col-sm-6 col-sm-offset-3 col-xs-12">
			...
			<img height="80" width="80" src="https://assets.encom.app/150-150/0/<?= enc(COMPANY_ID) ?>.jpg" class="img-circle animated bounceIn" id="logo" style="<?= $displayNone; ?>" />
			...
			<div class="animated fadeInUp speed-3x r-24x md-whiteframe-16dp col-xs-12 no-padder clear" style="<?= $displayNone; ?>" id="list">
				...
				?>
						...
						<a href="<?= PUBLIC_URL ?>/rewardsView?s=<?= base64_encode(enc(COMPANY_ID) . ',' . enc($contact['contactUID'])) ?>" class="col-xs-12 wrapper-xs bg-primary gradBgPurple animateBg hidden-print text-center" id="loyalty">
						...
						?>
							...
							if ($_modules['epos'] && $result['transactionType'] == 3) {
								...
								$url = ePOSLink([
									...
									'amount'    => $deudaTotal,
									'customer'  => enc($contact['contactUID']),
...
utm_campaign=<?= COMPANY_NAME ?>" class="block hidden-print">
	...
	<link rel="stylesheet" href="https://panel.encom.app/css/ncm.css?<?= date('d.H') ?>" type="text/css" />
	<script type="text/javascript">
