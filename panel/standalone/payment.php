...
if (isset($_GET['shop_process_id']) && isset($_GET['status'])) {
	...
	if ($STATUS == 'APPROVED') {
		...
		$result 			= curlContents(API_URL . '/add_vpayment', 'POST', $data);
		 ...
		 $updateQuery = "UPDATE setting SET settingBlocked = 0 WHERE companyId = ?";
		 $params = [$COMPANY_ID];
...
if (CUSTOMER) {
	$contact 					= ncmExecute('SELECT * FROM contact WHERE contactUID = ? AND companyId = ? LIMIT 1', [CUSTOMER, COMPANY_ID]);
...
<body class="bg-light lter col-xs-12 no-padder">
    <section class="vbox" id="content">
        <div class="wrapper text-center col-md-4 col-md-offset-4 col-sm-6 col-sm-offset-3 col-xs-12">
            ...
            <img height="80" width="80" src="https://assets.encom.app/150-150/0/<?= enc(COMPANY_ID) ?>.jpg" class="img-circle animated bounceIn" id="logo" style="" />
            ...
            <div class="m-b-md m-t-lg col-xs-12 animated bounceIn" id="encom">
                <a href="https://encom.app?utm_source=ENCOM_online_receipt&utm_medium=ENCOM_footer_icon&utm_campaign=<?= COMPANY_NAME ?>" class="block hidden-print">
