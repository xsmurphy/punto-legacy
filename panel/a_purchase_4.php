...
if(validateHttp('action') == 'insert'){
  ...
  if($discount > 0){
      $totalDis = $discount * 100 / $totalDis;
  }
  ...
  foreach($items as $key => $label){
    ...
    error_log("--> descuento = ".$discount);
    error_log("--> porcentaje = ".$totalDis);
    error_log("--> tax = ".$tax);
    error_log("--> totalDis * tax = ".($totalDis * $tax));
    error_log("--> resultado = ".(($totalDis * $tax) / 100));
    if($discount > 0){
      
      $tax    = (($totalDis * $tax) / 100);
      //error_log("--> tax nuevo = ".$tax);
    }
    $taxObj[]   = ['name' => $taxName, 'val' => $tax];
  
    $totalTax   += $tax;
    error_log("--> totalTax = ".$totalTax);
    $totalUnits += $units;
    $total      += $tPrice;
