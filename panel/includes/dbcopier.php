...
$NAMESPACE 			= 'incomepo_';
$ORIGINALDB 		= $NAMESPACE . '905';

$db->debug 			= true;

if(!isset($_GET['dbname']) || !isset($_GET['companyId'])){
	die('error');
}

$dbName = $db->Prepare($_GET['dbname']);
$dbName = $NAMESPACE . $dbName;
$COMPANY_ID = $db->Prepare($_GET['companyId']);
	...
	if($chkDb){
		if($chkDb->RecordCount()>0){
			...
			if($createDB !== false){
				...
				$db->Execute("
				CREATE TABLE IF NOT EXISTS `cpayments` (
				...
				$db->Execute("
				CREATE TABLE IF NOT EXISTS `cRecordField` (
				...
				$db->Execute("
				CREATE TABLE IF NOT EXISTS `cRecordValue` (
				...
				$db->Execute("
				CREATE TABLE IF NOT EXISTS `giftCardSold` (
				...
				$db->Execute("
				CREATE TABLE IF NOT EXISTS `inventorySold` (
				...
				$db->Execute("
				CREATE TABLE IF NOT EXISTS `stockTrigger` (
			...
			//$db->selectDb($ORIGINALDB);

			$insertInto 	= 'INSERT INTO ' . $dbName;
			$selectFrom 	= 'SELECT * FROM  ' . $ORIGINALDB;
			$whereCompanyId = 'companyId = ' . $COMPANY_ID;

			//ACTIVITYLOG
			$db->Execute($insertInto . '.activityLog ' . $selectFrom . '.activityLog WHERE ' . $whereCompanyId);

			//COMPANY
			$db->Execute($insertInto . '.company ' . $selectFrom . '.company WHERE ' . $whereCompanyId);

			//CONTACT
			$db->Execute($insertInto . '.contact ' . $selectFrom . '.contact WHERE ' . $whereCompanyId);

			//CPAYMENTS
			$db->Execute($insertInto . '.cpayments ' . $selectFrom . '.cpayments WHERE ' . $whereCompanyId);

			//CUSTOMERRECORD
			$db->Execute($insertInto . '.customerRecord ' . $selectFrom . '.customerRecord WHERE ' . $whereCompanyId);

			//DRAWER
			$db->Execute($insertInto . '.drawer ' . $selectFrom . '.drawer WHERE ' . $whereCompanyId);

			//EXPENSES
			$db->Execute($insertInto . '.expenses ' . $selectFrom . '.expenses WHERE ' . $whereCompanyId);

			//GIFTCARDSOLD
			$db->Execute($insertInto . '.giftCardSold ' . $selectFrom . '.giftCardSold WHERE ' . $whereCompanyId);

			//INVENTORY
			$db->Execute($insertInto . '.inventory ' . $selectFrom . '.inventory WHERE ' . $whereCompanyId);

			//INVENTORYCOUNT
			$db->Execute($insertInto . '.inventoryCount ' . $selectFrom . '.inventoryCount WHERE ' . $whereCompanyId);

			//INVENTORYHISTORY
			$db->Execute($insertInto . '.inventoryHistory ' . $selectFrom . '.inventoryHistory WHERE ' . $whereCompanyId);

			//INVENTORYTRANSFER
			$db->Execute($insertInto . '.inventoryTransfer ' . $selectFrom . '.inventoryTransfer WHERE ' . $whereCompanyId);

			//ITEM
			$db->Execute($insertInto . '.item ' . $selectFrom . '.item WHERE ' . $whereCompanyId);

			//NOTIFY
			$db->Execute($insertInto . '.notify ' . $selectFrom . '.notify WHERE ' . $whereCompanyId);

			//OUTLET
			$db->Execute($insertInto . '.outlet ' . $selectFrom . '.outlet WHERE ' . $whereCompanyId);

			//PRICELIST
			$db->Execute($insertInto . '.priceList ' . $selectFrom . '.priceList WHERE ' . $whereCompanyId);

			//PRODUCTION
			$db->Execute($insertInto . '.production ' . $selectFrom . '.production WHERE ' . $whereCompanyId);

			//RECURRING
			$db->Execute($insertInto . '.recurring ' . $selectFrom . '.recurring WHERE ' . $whereCompanyId);

			//REGISTER
			$db->Execute($insertInto . '.register ' . $selectFrom . '.register WHERE ' . $whereCompanyId);

			//SATISFACTION
			$db->Execute($insertInto . '.satisfaction ' . $selectFrom . '.satisfaction WHERE ' . $whereCompanyId);

			//SETTINGS
			$db->Execute($insertInto . '.setting ' . $selectFrom . '.setting WHERE ' . $whereCompanyId);

			//TAXONOMY
			$db->Execute($insertInto . '.taxonomy ' . $selectFrom . '.taxonomy WHERE ' . $whereCompanyId);

			//TRANSACTION
			$db->Execute($insertInto . '.transaction ' . $selectFrom . '.transaction WHERE ' . $whereCompanyId);

			//PLANS
			$db->Execute($insertInto . '.plans ' . $selectFrom . '.plans');

			//SELECTING AND INSERTING
			//INVENTORYSOLD, ITEMSOLD
			$item = $db->Execute('SELECT * FROM ' . $ORIGINALDB . '.item WHERE ' . $whereCompanyId);

			if($item){
				if($item->RecordCount()>0){
					while (!$item->EOF) {
						$db->Execute($insertInto . '.inventorySold ' . $selectFrom . '.inventorySold WHERE itemId = ' . $item->fields['itemId']);
						$db->Execute($insertInto . '.itemSold ' . $selectFrom . '.itemSold WHERE itemId = ' . $item->fields['itemId']);
						$db->Execute($insertInto . '.stockTrigger ' . $selectFrom . '.stockTrigger WHERE itemId = ' . $item->fields['itemId']);
						$db->Execute($insertInto . '.price ' . $selectFrom . '.price WHERE itemId = ' . $item->fields['itemId']);
						$item->MoveNext(); 
					}
					$item->Close();
				}
			...
			//SIN COMPANY ID, , cRecordValue, cRecordField, , , , , tableTags
