...
if(validateHttp('action') == 'sessions'){
	...
	//$sql = "SELECT a.*, b.*, c.* FROM transaction a, itemSold b, item c WHERE a.companyId = ? AND a.transactionDate BETWEEN ? AND ? AND a.transactionId = b.transactionId AND b.itemId = c.itemId AND c.itemSessions > 1 ORDER BY b.itemSoldDate DESC" . $limits;
	//$result = ncmExecute($sql,[COMPANY_ID,$startDate,$endDate],true,true);
	
	//obtengo todos los items con sesiones
	$allItems 	= [];
	$itmsIDs 	= [];
	$sqlItms 	= ncmExecute('SELECT * FROM item WHERE itemSessions > 1 AND itemStatus > 0 LIMIT 50000', [], false, true);

	if($sqlItms){
		while (!$sqlItms->EOF) {
			$fields 						= $sqlItms->fields;
			$itmsIDs[] 						= $fields['itemId'];
			$allItems[$fields['itemId']] 	=  [
	...
	//selecciono los IDs de las transacciones en el periodo asignado
	$transArr = [];
	$sqlTrans = ncmExecute('SELECT transactionId, customerId, invoiceNo FROM transaction WHERE companyId = ? AND transactionDate BETWEEN ? AND ? LIMIT 50000', [COMPANY_ID, $startDate, $endDate], false, true);
	
	if($sqlTrans){

		while (!$sqlTrans->EOF) {

			$fields 										= $sqlTrans->fields;
			$transArr['IDs'][] 								= $fields['transactionId'];
			$transArr['trans'][$fields['transactionId']] 	= ['customer' => $fields['customerId'], 'invoice' => $fields['invoiceNo']];
	...
	$sqlItmSold = ncmExecute('SELECT * FROM itemSold WHERE transactionId IN(' . implode(',', $transArr['IDs']) . ') AND itemId IN(' . implode(',', $itmsIDs) . ') ORDER BY itemSoldDate DESC LIMIT 50000', [], false, true);
	$itmSoldArr	= [];

	if($sqlItmSold){
		while (!$sqlItmSold->EOF) {

			$fields 								= $sqlItmSold->fields;
			$itmSoldArr[] 							= $fields['itemId'];

			$allItems[$fields['itemId']]['soldID'] 	= $fields['itemSoldId'];
			$allItems[$fields['itemId']]['date'] 	= $fields['itemSoldDate'];

			foreach ($transArr['trans'] as $transID => $data) {
				if($fields['transactionId'] == $transID){
					$allItems[$fields['itemId']]['customer'] 	= $data['customer'];
					$allItems[$fields['itemId']]['invoice'] 	= $data['invoice'];
	...
	if($allItems){
		foreach ($allItems as $itemID => $fields) {

			if(validInArray($fields,'soldID')){
				
				$contact 		= getContactData($fields['customer'],'uid',true);
				$contactName 	= getCustomerName($contact);
				$date       	= niceDate($fields['date']);
				$soldID 		= enc($fields['soldID']);

		        //obtengo sesiones

		        $sessions   	= ncmExecute("SELECT COUNT(transactionId) as dones FROM transaction WHERE companyId = ? AND transactionType = 13 AND packageId = ? AND transactionStatus = 6 LIMIT " . $fields['sessions'],[ COMPANY_ID, $fields['soldID'] ]);

				$table .= 	'<tr class="pointer clickrowSession" data-id="' . $soldID . '" data-load="' . $baseUrl . '?action=detail&id=' . $soldID . '&cid=' . enc($fields['customer']) . '">'.
							'	<td>' . $fields['invoice'] . '</td>'.
							'	<td data-sort="' . $fields['date'] . '">' . $date . '</td>'.
							'	<td>' . $fields['name'] . '</td>'.
							'	<td>' . $contactName . '</td>'. 
							'	<td class="text-right">' . $fields['sessions'] . '</td>'. 
							'	<td class="text-right">' . $sessions['dones'] . '</td>'.
							'	<td class="text-right">' . ($fields['sessions'] - $sessions['dones']) . '</td>'.
							'</tr>';
