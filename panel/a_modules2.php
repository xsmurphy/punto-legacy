...
if(validateHttp('action') == 'epos'){
	$record['eposData'] = json_encode([
										'idCommerce' 	=> validateHttp('idCommerce'),
										'hash' 			=> validateHttp('hash')
										]);
...
if(validateHttp('action') == 'crm'){
	...
	if($data){
		$modulesUpdt 			= ncmExecute('SELECT moduleData FROM module WHERE companyId = ? LIMIT 1',[COMPANY_ID]);
		$moduleUpdt				= json_decode($modulesUpdt['moduleData'],true);
		if(validInArray($moduleUpdt,'crm')){
			if(validInArray($moduleUpdt['crm'],'crmDontAutoSendDocsToCustomer')){
				$moduleUpdt['crm']['crmDontAutoSendDocsToCustomer'] = $data['crmDontAutoSendDocsToCustomer'];
			}else{
				$moduleUpdt['crm'] = ['crmDontAutoSendDocsToCustomer' => $data['crmDontAutoSendDocsToCustomer']];
			}
		}else{
		...
		$update = $db->AutoExecute('module', ['moduleData' => json_encode($moduleUpdt)], 'UPDATE', 'companyId = ' . COMPANY_ID);
...
if(validateHttp('action') == 'loadModal'){
	if($_GET['type'] == 'ecom'){
		...
		<div class="col-xs-12 wrapper bg-white">
            ...
            <div class="col-xs-7 no-padder">
            	<?=selectInputGenerator(['ccc'=>'Efectivo y Tarjetas','c'=>'Solo Efectivo','cc'=>'Solo Tarjetas'],['match'=>$ecomData['ecom_payments'],'class'=>'ecom_payments no-border b-b'])?>
            </div>
            ...
            <div class="col-xs-7 no-padder">
            	<?=selectInputOutlet(iftn($ecomData['outlet'],OUTLET_ID,dec($ecomData['outlet'])),false,'no-border b-b ecom_outlet','ecom_outlet',false,false,false,true)?>
            </div>
	        ...
	        <div class="col-sm-3 col-xs-12 no-padder">
            	<span class="font-bold text-u-c block m-t-sm">Envio 1</span>
            </div>
            <div class="col-sm-3 col-xs-12 no-padder">
            	<input type="text" name="tier1-km" placeholder="7" class="form-control units font-bold no-border b-b tier1-km" value="<?php echo $ecomData['tiers']['tier1']['km'];?>">
            </div>
            <div class="col-sm-6 col-xs-12 no-padder">
            	<select name="tier1-id" class="form-control bg-white no-border b-b tier1-id searchAjax" autocomplete="off">
            		<?php
            		if($ecomData['tiers']['tier1']['id']){
            			$itmData 	= getItemData(dec($ecomData['tiers']['tier1']['id']));
            			$name 		= $itmData['itemName'];
            			$price 		= $itmData['itemPrice'];
            			?>
            			<option value="<?=$ecomData['tiers']['tier1']['id']?>" data-price="<?=$price?>" selected><?=$name?></option>
            			<?php
            		}
            		?>
            	</select>
            ...
            <div class="col-sm-3 col-xs-12 no-padder">
            	<span class="font-bold text-u-c block m-t-sm">Envio 2</span>
            </div>
            <div class="col-sm-3 col-xs-12 no-padder">
            	<input type="text" name="tier2-km" placeholder="10" class="form-control units font-bold no-border b-b tier2-km" value="<?php echo $ecomData['tiers']['tier2']['km'];?>">
            </div>
            <div class="col-sm-6 col-xs-12 no-padder">
            	<select name="tier2-id" class="form-control bg-white no-border b-b tier2-id searchAjax" autocomplete="off">
            		<?php
            		if($ecomData['tiers']['tier2']['id']){
            			$itmData 	= getItemData(dec($ecomData['tiers']['tier2']['id']));
            			$name 		= $itmData['itemName'];
            			$price 		= $itmData['itemPrice'];
            			?>
            			<option value="<?=$ecomData['tiers']['tier2']['id']?>" data-price="<?=$price?>" selected><?=$name?></option>
            			<?php
            		}
            		?>
            	</select>
            ...
            <div class="col-sm-3 col-xs-12 no-padder">
            	<span class="font-bold text-u-c block m-t-sm">Envio 3</span>
            </div>
            <div class="col-sm-3 col-xs-12 no-padder">
            	<input type="text" name="tier3-km" placeholder="15" class="form-control units font-bold no-border b-b tier3-km" value="<?php echo $ecomData['tiers']['tier3']['km'];?>">
            </div>
            <div class="col-sm-6 col-xs-12 no-padder">
            	<select name="tier3-id" class="form-control bg-white no-border b-b tier3-id searchAjax" autocomplete="off">
            		<?php
            		if($ecomData['tiers']['tier3']['id']){
            			$itmData 	= getItemData(dec($ecomData['tiers']['tier3']['id']));
            			$name 		= $itmData['itemName'];
            			$price 		= $itmData['itemPrice'];
            			?>
            			<option value="<?=$ecomData['tiers']['tier3']['id']?>" data-price="<?=$price?>" selected><?=$name?></option>
            			<?php
            		}
            		?>
            	</select>
		...
		<div class="col-xs-12 wrapper bg-white">
			...
			$oarray = getAllOutlets();
			foreach($oarray as $name){
				$url = 'https://public.encom.app/qrGenerator?s=' . base64_encode( enc(COMPANY_ID) . ',' . enc($name['id']) . ',inHouseAttendance' );
				?>
				<tr>
					<td class="no-padder">
						<a class="block wrapper-md text-md" target="_blank" href="<?=$url?>"> 
		...
		<div class="col-xs-12 wrapper bg-white">
			...
			$oarray = getAllOutlets();
			foreach($oarray as $name){
				$url = 'https://public.encom.app/qrGenerator?s=' . base64_encode( enc(COMPANY_ID) . ',' . enc($name['id']) . ',userAttendance' );
				?>
				<tr>
					<td class="no-padder">
						<a class="block wrapper-md text-md" target="_blank" href="<?=$url?>"> 
		...
		<?php
			...
			$url = 'https://public.encom.app/priceChecker?s=' . base64_encode( enc(COMPANY_ID) );
		?>
		<div class="col-xs-12 wrapper bg-white">
		...
		<div class="col-xs-12 wrapper bg-white">
			...
			<section class="r-3x col-xs-12 no-padder clear">
				...
				<table class="table table-hover">
					...
					$oarray = getAllOutlets();
					foreach($oarray as $name){
						$url = 'https://public.encom.app/anonFeedback?s=' . base64_encode( enc(COMPANY_ID) . ',' . enc($name['id']) );
						?>
						<tr>
							<td>
		...
		<div class="col-xs-12 wrapper bg-white panel m-n">
			...
			$oarray = getAllOutlets();
			foreach($oarray as $name){
				$url = 'https://public.encom.app/kds?s=' . base64_encode( enc(COMPANY_ID) . ',' . enc($name['id']) );
				?>
				<tr>
					<td class="no-padder">
						<a class="block wrapper-md text-md" target="_blank" href="<?=$url?>"> 
		...
		<div class="col-xs-12 wrapper bg-white">
			...
			<div class="text-center">
				...
				<a href="#" class="btn btn-info btn-rounded btn-md font-bold m-t-sm" data-toggle="tooltip" title="Clic para copiar el enlace" id="copyOsWidget">
					<?='https://public.encom.app/osWidget?s=' . base64_encode(enc(COMPANY_ID)) ?>
				</a>
				<br>
	...
	<?php
	}else if($_GET['type'] == 'epos'){
		$eposData = json_decode($_modules['eposData'],true);
		?>
		<div class="col-xs-12 wrapper bg-white">
			<div class="font-bold h1 text-dark b-b m-b wrap-b">ePOS</div>
			<label class="font-bold text-u-c">ID de Comercio</label>
			<input type="tel" name="idCommerce" class="form-control input-lg rounded text-center font-bold ePOSIDCommerce" value="<?=$eposData['idCommerce'];?>">
			<label class="font-bold text-u-c">Hash</label>
			<input type="tel" name="hash" class="form-control input-lg rounded text-center font-bold ePOSHash" value="<?=$eposData['hash'];?>">
		</div>
	<?php
	}else if($_GET['type'] == 'ordersPanel'){
		...
		<div class="col-xs-12 wrapper bg-white">
			...
			<div class="col-xs-12 wrapper m-b">
				...
				<div class="wrapper-sm text-lg font-bold text-center bg-light rounded"><?=sha1($apis['accountId'])?></div>
			</div>
		</div>
		<?php
	}
...
?>
	...
	<div class="col-xs-12 no-padder">
		...
		if(COUNTRY == 'PY'){
		?>
		<?=modBlock(['active' => $_modules['epos'], 'id' => 'epos', 'title' => 'ePOS', 'description' => 'Olvidate del POS físico, usá el ePOS de ENCOM sin costo']);?>
		<?php
		}
		?>
		<!-- PAYMENT BTN -->
	...
	<div class="col-xs-12 no-padder">
		...
		<!-- NEWTON -->
		<?=modBlock(['hidden' => 1, 'blocked' => 1, 'active' => $_modules['newton'],'class' => 'hidden','class' => 'hidden', 'id' => 'newton', 'title' => '<img src="https://i.imgur.com/Mxy97Pp.png" height="38">', 'description' => 'Gestiona tu delivery de forma eficiente']);?>
		<!-- NEWTON -->
	</div>
	...
	<script>
		$(document).ready(function(){
			...
			switchit(function(tis, active){
				...
				if($.inArray(type, ['loyalty','calendar','tusfacturas','spotify','attendance','priceCheck','epos','dropbox','newton','ecom','ordersPanel','kds','tables','production','recurring','dunning','digitalInvoice','reminder','salesSummaryDaily','feedback','api','crm']) !== -1){
					if(active){
			...
			$('body').on('shown.bs.modal', '.modal', function () {
				...
				$('input.ePOSIDCommerce, input.ePOSHash').off('change').on('change',function(){
					var tis 			= $(this).val();
					var ePOSIDCommerce 	= $('input.ePOSIDCommerce').val();
					var ePOSHash 		= $('input.ePOSHash').val();

					$.get(baseUrl + '?action=epos&idCommerce=' + ePOSIDCommerce + '&hash=' + ePOSHash ,function(){
						message('Actualizado','success');
					});
				});
				...
				$('.modal input.ecomColorField, .modal input.ecomWelcome, .modal input.ecom_banner, .modal input.ecom_selfbanner, .modal select.ecom_handle, .modal input.ecomGA, select.ecom_payments, select.ecom_outlet, select.ecom_user, input.tier1-km, input.tier2-km, input.tier3-km, select.tier1-id, select.tier2-id, select.tier3-id,input.ecom_minimum').off('change keyup').on('change keyup',function(){
					processEcomData();
				});
			...
			function processEcomData(){
				...
				var tier1km 		= $('.modal input.tier1-km').val();
				var tier2km 		= $('.modal input.tier2-km').val();
				var tier3km 		= $('.modal input.tier3-km').val();

				var tier1id 		= $('.modal select.tier1-id').val();
				var tier2id 		= $('.modal select.tier2-id').val();
				var tier3id 		= $('.modal select.tier3-id').val();

				var tier1price 		= $('.modal select.tier1-id option:selected').data('price');
				var tier2price 		= $('.modal select.tier2-id option:selected').data('price');
				var tier3price 		= $('.modal select.tier3-id option:selected').data('price');

				var tier1 			= {id:tier1id,km:tier1km,price:tier1price};
				var tier2 			= {id:tier2id,km:tier2km,price:tier2price};
				var tier3 			= {id:tier3id,km:tier3km,price:tier3price};
