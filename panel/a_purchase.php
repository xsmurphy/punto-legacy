...
if(validateHttp('action') == 'insert'){
  ...
  if(validateHttp('typeOfEvent','post') == '1'){
    $isOrder        = true;
    $trStatus       = 0;
  }elseif(validateHttp('typeOfEvent','post') == '2'){
    $isReposition   = true;
    $trStatus       = 3;
  }elseif(validateHttp('typeOfEvent','post') == '3'){
    $isPurchase   = true;
    $isReturn     = true;
  }else{
    $isPurchase   = true;
  ...
  $authNo         = validateHttp('authNo','post');
  ...
  foreach($items as $key => $label){
    ...
    if($label['pack'] > 0){
      ...
      $price      = (DECIMAL == 'no') ? $label['packedPrice'] : (float) $label['packedPrice'];
    ...
    $taxName    = getTaxonomyName(dec($label['tax']));
    $tax        = formatNumberToInsertDB($label['taxvalue']);

    $taxObj[]   = ['name' => $taxName, 'val' => $tax];

    $totalTax   += $tax;
    $totalUnits += $units;
    $total      += $tPrice;

    if(isset($id) || isset($label['title'])){
      $details[] = [
                      'itemId'  => $id, 
                      'qty'     => formatCurrentNumber($units,'yes',false,3), 
                      'title'   => $label['title'] ?? "", 
                      'price'   => ((DECIMAL == 'no') ? (int) $tPrice : (float) $tPrice),
                      'tax'     => $tax,
                      'plan'    => $plan
                    ];
    }
  ...
  if($isOrder){
    ...
    $record['transactionTotal']       = -((float)$total - (float)$discount);
    $record['transactionDiscount']    = -((float)$discount);
    $record['transactionUnitsSold']   = -((int)$totalUnits);
    $record['transactionTax']         = -((float)$totalTax);
  ...
  if( $transID > 0 && $isPurchase ){

    foreach($items as $key => $itm){
      if(!isset($itm['itemId']) && !isset($itm['title'])){
        continue;
      }
      ...
      $expires    = (!empty($itm['expires']) ? $itm['expires'].' 00:00:00' : "");
      $uid        = $itm['uid'] ?? "";
      $title      = !empty($itm['title']) ? $itm['title'] : NULL;
      ...
      if( validity($id) && $isPurchase ){
        ...
        if($units > 0){
          ...
          //Si es nota de credito resto del stock
          $ops['type']        = "-";
          $ops['count']         = -$units;
          $stocked              = manageStock($ops);
        }
  ...
  if($insertTransaction === false || $itmInsert === false || $invUpdate == false){
    ...
    if($isPurchase){
      $taxObj = json_encode($taxObj); 
      ncmInsert(['records' => ['toTaxObjText' => $taxObj, 'transactionId' => $transID, 'companyId' => COMPANY_ID], 'table' => 'toTaxObj']);
    }
      ...
      <div class="col-md-3 col-sm-12 col-xs-12 matchCols no-padder bg-info gradBgBlue tutLeftColumn">
        ...
        <div class="col-xs-12 wrapper" style="min-height:450px;">
          ...
          <div class="col-xs-12 no-padder hidden" id="moreOpsPanel">
            ...
            <div class="col-xs-12 no-padder m-b visiblePurchase">
              <label class="font-bold text-xs text-u-c">Timbrado o Autorización</label>
              <input type="text" name="authNo" class="form-control m-b no-border b-b no-bg tabindex text-white" value="" autocomplete="off" placeholder="Timbrado o Autorización">
            </div>
  ...
  <?php
    ...
    if($id){
      ...
      if($isExtraction){
        if($result){
        ?>
            window.unOrderAction.extraction  = true;
            window.unOrderAction.description = "<?=toUTF8($result['expensesDescription'])?>";
            window.unOrderAction.amount      = "<?=$result['expensesAmount']?>";
        <?php
        }
      }else{
        if($result){
          ...
          window.unOrderAction.lines     = <?=json_encode($lines);?>;
          window.unOrderAction.unOrder   = true;
          ...
          $('textarea[name="note"]').val("Ref. Orden #<?=$result['invoiceNo']?> | <?=sanitizeForDB( $result['transactionNote']);?>");
    ...
    var ncmPurchase = {
      ...
      events : function(){
        ...
        onClickWrap('#totalPurchase',function(){
          console.log("Hola que tal, tanto tiempo");
...
<script src="scripts<?=$baseUrl?>.js?<?=date('d.i') . rand(22,9000)?>"></script>
