...
if(validateHttp('action') == 'migrateRecords'){
	if(COMPANY_ID != 10){
		die('no company allowed');
	}

	$result = ncmExecute('SELECT * FROM customerRecord WHERE companyId = ?',[COMPANY_ID],false,true);

	if($result){
		while (!$result->EOF) {
			$rSet = ncmExecute('SELECT * FROM customerRecord WHERE companyId = ?',[COMPANY_ID],false,true);
...
if(validateHttp('action') == 'update'){
	...
	$tin 				= iftn($_POST['tin'] ?? null,null);
	$person 		= iftn($_POST['person'] ?? null,null);
	...
	$note 			= iftn($_POST['note'] ?? null,null);
	$loyalty 		= formatNumberToInsertDB($_POST['loyalty'] ?? 0);
	$storeCredit= formatNumberToInsertDB($_POST['storeCredit'] ?? 0);
	$creditLine = formatNumberToInsertDB($_POST['storeCreditLine'] ?? 0);
	$loystatus 	= iftn($_POST['loyaltyStatus'] ?? null,null);
	$password 	= iftn($_POST['password2'] ?? null,null);
	$role 			= !empty($_POST['role']) ? dec($_POST['role']) : null;
	$lockPass 	= iftn($_POST['lockPass'] ?? null,null);
	$salt 			= iftn($_POST['salt'] ?? null,null);
	$status 		= iftn($_POST['status'] ?? '0','0');
	$outletId 	= iftn($_POST['outletId'] ?? null,null);
	$outletId 	= dec($outletId);
	$creditable	= iftn($_POST['creditable'] ?? null,null);
	$category		= iftn($_POST['category'] ?? null,null);
	$category		= dec($category);
	$color 			= iftn($_POST['color'] ?? null,null);
	$agendable 	= iftn($_POST['agendable'] ?? null,null);
	$trackeable = iftn($_POST['trackeable'] ?? null,null);
	$comission 	= iftn($_POST['comission'] ?? null,null);
	$schedulePosition = $_POST['schedulePosition'] ?? null;

	$hourSalary = formatNumberToInsertDB($_POST['hourSalary'] ?? 0);
...
if(validateHttp('action') == 'form'){
	...
	if(validateHttp('id')){// 
		...
		$rData 			= json_decode(!empty($result['data']) ? $result['data'] : "",true);
		...
		if($isUser){
			...
			$totalPurchased 			= iftn($totalPurNSold[0] ?? 0,0);
	    $totalComprado 			= iftn($totalPurNSold[1] ?? 0,0);
			...
			<div class="col-sm-5 col-xs-12 matchCols text-center" style="min-height:400px;">
				...
				<div class="text-left wrapper">
					...
					<div class="m-b-xs">
						...
						<?=iftn($address,'No posee dirección',$addressUrl)?>
					</div>
						...
						<table class="table">
							<tbody>
								...
								<tr>
									...
									<td class="text-right font-bold"><?=formatCurrentNumber($rData['hourSalary'] ?? 0)?></td>
								</tr>
			...
			<div class="col-sm-5 text-center" style="min-height:400px;">
	            ...
	            if(isset($main) && $email){
	              ...
	              echo '<input type="hidden" class="form-control hidden" name="email2" value="'.$email.'"/>';                
	            	...
	            	$disabled = (isset($itsId) && (USER_ID == $itsId)) ? 'disabled' : '';
			...
			<div class="col-sm-7 bg-white no-padder">
	            ...
	            <div class="col-md-6 wrapper" style="min-height:321px;">            	
	              ...
	              <?php
			        	...
			        	if(!empty($_fullSettings['creditLine'])){
			            ?>
			            	<div class="form-group">
			        ...
			        <div class="form-group">
		                ...
		                if($role){
			                ...
			                while (!$role->EOF) {
			                	$fields 		= $role->fields;
			                  $selected 	= '';
			                  $name 			= $fields['taxonomyName'];

			                  if($result['role'] == 6){
			                  	$result['role'] = 4;
			                  }

			                  if($result['role'] == $fields['taxonomyExtra']){
			                      $selected = 'selected';
			                  }

			                  if( $fields['taxonomyExtra'] != 6 ){
			              ?>

			                <option value="<?=enc($fields['taxonomyExtra']);?>" <?=$selected?>><?=$name;?></option>
              ...
              <div class="form-group">
                <span class="font-bold text-u-c text-xs">Salario por hora:</span>
                <input type="text" class="form-control input-lg b-b b-default no-border maskCurrency" name="hourSalary" value="<?=formatCurrentNumber($rData['hourSalary'] ?? 0)?>" autocomplete="off" />
              </div>
	            ...
	            <div class="col-xs-12 m-t bg-light lter wrapper">
	            	<?php if(dec($id) != USER_ID && dec($id)!= $userOwner['contactId']){ ?>
...
if(validateHttp('action') == 'file'){
	...
	foreach ($data as $key => $row) {
		...
		$name 			= preg_replace('/[^A-Za-z0-9]*$/', '', toUTF8($row['Nombre/Razon Social'] ?? ""));
		$ruc 				= preg_replace('/[^A-Za-z0-9_-]*$/', '', $row[TIN_NAME] ?? "");
		$fullname		= preg_replace('/[^A-Za-z0-9]*$/', '', toUTF8($row['Nombre y Apellido'] ?? ""));
		$phone 			= preg_replace('/[^\d]+/', '', $row['Telefono'] ?? "");
		$phone2			= preg_replace('/[^\d]+/', '', $row['Telefono 2'] ?? "");
		$email 			= strtolower(preg_replace('/[^a-zA-Z0-9.@_-]+/', '', $row['Email'] ?? ""));
		$address 		= preg_replace('/[^A-Za-z0-9]*$/', '',toUTF8($row['Direccion'] ?? ""));
		$address2 	= preg_replace('/[^A-Za-z0-9]*$/', '', toUTF8($row['Direccion 2'] ?? ""));
		$note 			= preg_replace('/[^A-Za-z0-9]*$/', '', toUTF8($row['Nota'] ?? ""));
		$role 			= strtolower(preg_replace('/[^A-Za-z]*$/', '', toUTF8($row['Rol'] ?? "")));
		$location		= strtoupper(preg_replace('/[^A-Za-z0-9]*$/', '', toUTF8($row['Localidad'] ?? "")));
		$city				= strtoupper(preg_replace('/[^A-Za-z0-9]*$/', '', toUTF8($row['Ciudad'] ?? "")));
		$ci					= preg_replace('/[^\d]+/', '', $row['Doc. de Identidad'] ?? "");
		$fechaNacimiento = null;
		if(!empty($row['Fecha de Nacimiento'])){
			if(is_numeric($row['Fecha de Nacimiento'])){
				$excelDate = round($row['Fecha de Nacimiento']);
				$fechaNacimiento	= date("Y-m-d",strtotime("+1 day",($excelDate - 25569) * 86400));
			}else{
				$excelDate = str_replace("/","-",$row['Fecha de Nacimiento']);
				$fechaNacimiento	= date("Y-m-d",strtotime($excelDate));
			}
		}
		...
		if($role == 'proveedor'){
			...
			$roleId = 3;
			$type 	= 0;
		}else if ($role == 'vendedor' || $role == 'delivery' || $role == 'recurso') {//recurso
			$roleId = 4;
			$type 	= 0;
		}
...
if(validateHttp('action') == 'rolePermissions'){
	...
	if($roles){
		while (!$roles->EOF) {
			...
			$data[$rolIndex] 		= !empty($rol['taxonomyExtra']) ? json_decode($rol['taxonomyExtra'],true) : $_ROLES_DATA[$rolIndex] ?? [];
		...
		<form action="<?=$baseUrl?>?action=saveRoles" method="post" id="rolesForm" name="rolesForm">
			<table class="table text-center">
				...
				<tbody>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][dashboard][view]',$data[2]['panel']['dashboard']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][dashboard][view]',$data[1]['panel']['dashboard']['view'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][items][delete]',$data[2]['panel']['items']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][items][delete]',$data[1]['panel']['items']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][contacts][edit]',$data[2]['panel']['contacts']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][contacts][edit]',$data[1]['panel']['contacts']['edit'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][contacts][delete]',$data[2]['panel']['contacts']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][contacts][delete]',$data[1]['panel']['contacts']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][reports][sales][view]',$data[2]['panel']['reports']['sales']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][reports][sales][view]',$data[1]['panel']['reports']['sales']['view'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][reports][sales][edit]',$data[2]['panel']['reports']['sales']['edit'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][reports][sales][delete]',$data[2]['panel']['reports']['sales']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][reports][expenses][edit]',$data[2]['panel']['reports']['expenses']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][reports][expenses][edit]',$data[1]['panel']['reports']['expenses']['edit'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][reports][expenses][delete]',$data[2]['panel']['reports']['expenses']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][reports][expenses][delete]',$data[1]['panel']['reports']['expenses']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][settings][view]',$data[2]['panel']['settings']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][settings][view]',$data[1]['panel']['settings']['view'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][settings][edit]',$data[2]['panel']['settings']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][settings][edit]',$data[1]['panel']['settings']['edit'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('2[panel][settings][delete]',$data[2]['panel']['settings']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[panel][settings][delete]',$data[1]['panel']['settings']['delete'] ?? false,'roles')?></td>
			...
			<table class="table text-center">
				...
				<tbody>
					...
					<tr>
						<td class="bg-light font-bold text-left">Editar</td>
						<td><?=switchIn('4[register][transactions][edit]',$data[4]['register']['transactions']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][transactions][edit]',$data[3]['register']['transactions']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][transactions][edit]',$data[2]['register']['transactions']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][transactions][edit]',$data[1]['register']['transactions']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][transactions][edit]',true,'disabled roles')?></td> 
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Anular/Cancelar</td>
						<td><?=switchIn('4[register][transactions][delete]',$data[4]['register']['transactions']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][transactions][delete]',$data[3]['register']['transactions']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][transactions][delete]',$data[2]['register']['transactions']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][transactions][delete]',$data[1]['register']['transactions']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						<td class="bg-light font-bold text-left">Crear</td>
						<td><?=switchIn('4[register][quotes][create]',$data[4]['register']['quotes']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][quotes][create]',$data[3]['register']['quotes']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][quotes][create]',$data[2]['register']['quotes']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][quotes][create]',$data[1]['register']['quotes']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][quotes][create]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Ver</td>
						<td><?=switchIn('4[register][quotes][view]',$data[4]['register']['quotes']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][quotes][view]',$data[3]['register']['quotes']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][quotes][view]',$data[2]['register']['quotes']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][quotes][view]',$data[1]['register']['quotes']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][quotes][view]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Editar</td>
						<td><?=switchIn('4[register][quotes][edit]',$data[4]['register']['quotes']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][quotes][edit]',$data[3]['register']['quotes']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][quotes][edit]',$data[2]['register']['quotes']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][quotes][edit]',$data[1]['register']['quotes']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][quotes][edit]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Anular/Cancelar</td>
						<td><?=switchIn('4[register][quotes][delete]',$data[4]['register']['quotes']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][quotes][delete]',$data[3]['register']['quotes']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][quotes][delete]',$data[2]['register']['quotes']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][quotes][delete]',$data[1]['register']['quotes']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						<td class="bg-light font-bold text-left">Crear</td>
						<td><?=switchIn('4[register][schedule][create]',$data[4]['register']['schedule']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][schedule][create]',$data[3]['register']['schedule']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][schedule][create]',$data[2]['register']['schedule']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][schedule][create]',$data[1]['register']['schedule']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][schedule][create]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Ver</td>
						<td><?=switchIn('4[register][schedule][view]',$data[4]['register']['schedule']['view'] ?? false ?? false,'roles')?></td>
						<td><?=switchIn('3[register][schedule][view]',$data[3]['register']['schedule']['view'] ?? false ?? false,'roles')?></td>
						<td><?=switchIn('2[register][schedule][view]',$data[2]['register']['schedule']['view'] ?? false ?? false,'roles')?></td>
						<td><?=switchIn('1[register][schedule][view]',$data[1]['register']['schedule']['view'] ?? false ?? false,'roles')?></td>
						<td><?=switchIn('0[register][schedule][view]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Editar</td>
						<td><?=switchIn('4[register][schedule][edit]',$data[4]['register']['schedule']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][schedule][edit]',$data[3]['register']['schedule']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][schedule][edit]',$data[2]['register']['schedule']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][schedule][edit]',$data[1]['register']['schedule']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][schedule][edit]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Anular/Cancelar</td>
						<td><?=switchIn('4[register][schedule][delete]',$data[4]['register']['schedule']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][schedule][delete]',$data[3]['register']['schedule']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][schedule][delete]',$data[2]['register']['schedule']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][schedule][delete]',$data[1]['register']['schedule']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						<td class="bg-light font-bold text-left">Crear</td>
						<td><?=switchIn('4[register][tables][create]',$data[4]['register']['tables']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][tables][create]',$data[3]['register']['tables']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][tables][create]',$data[2]['register']['tables']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][tables][create]',$data[1]['register']['tables']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][tables][create]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Ver</td>
						<td><?=switchIn('4[register][tables][view]',$data[4]['register']['tables']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][tables][view]',$data[3]['register']['tables']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][tables][view]',$data[2]['register']['tables']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][tables][view]',$data[1]['register']['tables']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][tables][view]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Editar</td>
						<td><?=switchIn('4[register][tables][edit]',$data[4]['register']['tables']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][tables][edit]',$data[3]['register']['tables']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][tables][edit]',$data[2]['register']['tables']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][tables][edit]',$data[1]['register']['tables']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][tables][edit]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Anular/Cancelar</td>
						<td><?=switchIn('4[register][tables][delete]',$data[4]['register']['tables']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][tables][delete]',$data[3]['register']['tables']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][tables][delete]',$data[2]['register']['tables']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][tables][delete]',$data[1]['register']['tables']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						<td class="bg-light font-bold text-left">Crear</td>
						<td><?=switchIn('4[register][orders][create]',$data[4]['register']['orders']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][orders][create]',$data[3]['register']['orders']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][orders][create]',$data[2]['register']['orders']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][orders][create]',$data[1]['register']['orders']['create'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][orders][create]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Ver</td>
						<td><?=switchIn('4[register][orders][view]',$data[4]['register']['orders']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][orders][view]',$data[3]['register']['orders']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][orders][view]',$data[2]['register']['orders']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][orders][view]',$data[1]['register']['orders']['view'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][orders][view]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Editar</td>
						<td><?=switchIn('4[register][orders][edit]',$data[4]['register']['orders']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][orders][edit]',$data[3]['register']['orders']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][orders][edit]',$data[2]['register']['orders']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][orders][edit]',$data[1]['register']['orders']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][orders][edit]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Anular/Cancelar</td>
						<td><?=switchIn('4[register][orders][delete]',$data[4]['register']['orders']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][orders][delete]',$data[3]['register']['orders']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][orders][delete]',$data[2]['register']['orders']['delete'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][orders][delete]',$data[1]['register']['orders']['delete'] ?? false,'roles')?></td>
					...
					<tr>
						...
						<td><?=switchIn('4[register][sales][discounts]',$data[4]['register']['sales']['discounts'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][sales][discounts]',$data[3]['register']['sales']['discounts'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][sales][discounts]',$data[2]['register']['sales']['discounts'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][sales][discounts]',$data[1]['register']['sales']['discounts'] ?? false,'roles')?></td>
						<td><?=switchIn('0[register][sales][discounts]',true,'disabled roles')?></td>
					</tr>
					<tr>
						<td class="bg-light font-bold text-left">Modificar precios</td>
						<td><?=switchIn('4[register][sales][price]',$data[4]['register']['sales']['price'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][sales][price]',$data[3]['register']['sales']['price'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][sales][price]',$data[2]['register']['sales']['price'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][sales][price]',$data[1]['register']['sales']['price'] ?? false,'roles')?></td>
					...
					<tr>
						<td class="bg-light font-bold text-left">Editar</td>
						<td><?=switchIn('4[register][settings][edit]',$data[4]['register']['settings']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('3[register][settings][edit]',$data[3]['register']['settings']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('2[register][settings][edit]',$data[2]['register']['settings']['edit'] ?? false,'roles')?></td>
						<td><?=switchIn('1[register][settings][edit]',$data[1]['register']['settings']['edit'] ?? false,'roles')?></td>
...
if(validateHttp('action') == 'recordList'){
	...
	if($record){
		...
		<section class="panel panel-default no-bg clear r-3x" id="<?=$id?>">
			<header class="panel-heading bg-light no-border text-u-c" data-id="<?=$id?>">
								...
								<tr id="rField<?=$fId?>" data-position="<?=$i;?>">
									...
									<td>
										<select id="type<?=$fId?>" data-id="<?=$fId?>" class="typeRecordField form-control no-border b-b no-bg">
										  ...
										  <option value="6" <?=($fType==6)?'selected':''?>>Título</option>
										</select>
									</td>
									<td>
...
if(validateHttp('action') == 'editRecordSort' && validateHttp('id')){
	...
	if($id && $value > -1){
		...
		$record['customerRecordSort'] 	= $value;
...
if(validateHttp('action') == 'generalTable'){
	...
	if($result){
		...
		while (!$result->EOF) {
			...
			if($_rol == 'customer'){
				...
				$distance					= coorsToKms($oLat, $oLng, $cAdd['lat'] ?? 0, $cAdd['lng'] ?? 0);
			}
			...
			if($allow){
				...
				if($_rol == 'customer'){
					...
					$table .= 	'<tr data-id="' . $itemId . '" class="clickrow ' . $itemId . '">' .
											...
											'	<td>' . ($field['contactBirthDay'] ?? '') . '</td>' .
											'	<td data-order="' . $field['contactDate'] . '"> ' . niceDate($field['contactDate'],true) . ' </td>' .
											'	<td data-order="' . $field['updated_at'] . '"> ' . niceDate($field['updated_at'],true) . ' </td>' .
											'	<td data-order="' . $lastDate . '"> ' . $lastDateF . ' </td>' .
											'	<td>' . $field['contactPhone'] . '</td>' .
											'	<td>' . $field['contactPhone2'] .'</td>' .
											'	<td>' . $field['contactEmail'] .'</td>' .
											'	<td>' . toUTF8($field['contactAddress']) .'</td>' .
											'	<td>' . toUTF8($field['contactLocation']) .'</td>' .
											'	<td>' . toUTF8($field['contactCity']) .'</td>' .
											'	<td>' . toUTF8($field['contactNote']) . '</td>' .
	...
	if($_rol == 'customer'){
		$foot = '</tbody><tfoot><tr><td colspan="19"></td></tr></tfoot>';
	}
...
<script>
	...
	var loadUrl 		= baseUrl + "?action=generalTable<?=!empty($_GET['rol']) ? '&rol=' . $_GET['rol'] : '' ?>";
	var _rol 				= '<?=$_rol?>';
