...
if (validateHttp('action') == 'generalTable') {
	...
	$allTransactions 	= [];
	$specialOutlet 		= (OUTLET_ID > 0) ? ' AND (c.outletId = ' . OUTLET_ID . ' OR c.outletId IS NULL OR c.outletId = 0)' : '';
	$result 			= ncmExecute('SELECT
									...
									JOIN transaction t ON
										i.transactionId = t.transactionId
									JOIN contact c ON
										i.userId = c.contactId
									WHERE
										t.transactionDate >= ?
										...
										' . $specialOutlet . '
									GROUP BY
										i.userId, c.contactName', [$startDate, $endDate, COMPANY_ID], false, true);
	...
	if ($result) {
		...
		while (!$result->EOF) { //aquí agrupo los IDS nuevos con los viejos
			...
			if (!empty($allTransactions[$id]) && validity($allTransactions[$id])) {
				$allTransactions[$id]['usold'] 		+= $fields['usold'] ?? 0;
				$allTransactions[$id]['total'] 		+= $fields['total'] ?? 0;
				$allTransactions[$id]['comission'] 	+= $fields['comission'] ?? 0;
				$allTransactions[$id]['count'] 		+= $fields['count'] ?? 0;
				$allTransactions[$id]['discount'] 	+= $fields['discount'] ?? 0;
			} else {
				$allTransactions[$id] = 	[
					'name' 		=> $fields['contactName'] ?? '',
					'usold' 	=> $fields['usold'] ?? 0,
					'total' 	=> $fields['total']	?? 0,
					'comission' => $fields['comission'] ?? 0,
	...
	// Consulta para seleccionar usuarios que no tuvieron movimientos en transaction e itemSold
	$allUsers = ncmExecute('SELECT
										c.contactId,
										c.contactName,
										0 as usold,
										0 as total,
	...
	// Procesar los usuarios sin actividad y agregarlos a $allTransactions
	if ($allUsers) {
		while (!$allUsers->EOF) {
			$fields = $allUsers->fields;
			$id = $fields['contactId'];

			if (empty($allTransactions[$id])) {
				$allTransactions[$id] = [
					'name'      => $fields['contactName'] ?? '',
					'usold'     => $fields['usold'] ?? 0,
					'total'     => $fields['total'] ?? 0,
	...
	if (validity($allTransactions, 'array')) {
		...
		foreach ($allTransactions as $id => $fields) {
			...
			if ($comission > 0) {
				$url 		= PUBLIC_URL . '/userItemsSold?s=' . base64_encode(enc(COMPANY_ID) . ',' . $userId . ',' . $startDate . ',' . $endDate);
				$urlColor	= 'text-info';
				$urlStatus	= '';
			}
