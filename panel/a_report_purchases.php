...
if(validateHttp('action') == 'update' && validateHttp('id','post')){
	...
	if(validateHttp('itemTrsId','post')){
		foreach (validateHttp('itemTrsId','post') as $itmSID) {
			...
			$record['itemSoldTotal']	= $itemPrice - ($record['itemSoldDiscount'] ?? 0);
...
if(validateHttp('action') == 'edit' && validateHttp('id')){
	...
	$tags 		= json_decode(is_null($result['tags']) ? "" : $result['tags']);
    ...
    <div class="modal-body no-padder clear r-24x" id="factura">
	    ...
	    <form action="<?=$baseUrl?>?action=update" method="POST" id="editSale" name="editSale">
	    	<div class="row equal padder">
	    		<div class="col-sm-3 col-xs-12 wrapper bg-info gradBgBlue hidden-print">
					...
					<input type="hidden" name="salestatus" value="<?=$isOrder ? 0 : 1 ?>">
	    		...
	    		<div class="col-sm-9 col-xs-12 no-padder bg-white">
...
if(validateHttp('action') == 'detailTable'){
	...
	if($result){		
		...
		while (!$result->EOF) {
			...
			$cogs 		= $fields['itemSoldTotal'] < 0 ? (ABS($fields['itemSoldTotal'])/$uSold) : divider($fields['itemSoldTotal'], $uSold); //lo hago de esta manera ya que si itemsoldtotal es negativo quiere decir que es una nota de crédito
			...
			if(isset($comission) && $comission > 0){
				...
				$commission	= $allItems[$id]['comission'] ?? 0;
				$earned 	= 0;
				if($commission > 0){
...
if(validateHttp('action') == 'rg90'){
	...
	*/
	...
	$saleDay 	= ncmExecute("	SELECT 
									t.transactionId, t.supplierId, t.invoiceNo, t.transactionStatus, t.transactionDetails, t.transactionTotal, t.invoicePrefix, t.transactionDate, t.transactionType, t.transactionDiscount
								FROM transaction t
								WHERE t.transactionType IN(1,4)
	...
	if($saleDay){
		...
		while (!$saleDay->EOF) {
			$fields 				= $saleDay->fields;
			$allItemsSold			= ncmExecute("
			SELECT * FROM itemSold itmS
			WHERE itmS.transactionId = ?",[$fields['transactionId']], false, false, true);
			if(empty($allItemsSold)){
				$allItemsSold = [];
			}
			$tStatus 				= $fields['transactionStatus'];
			$itemsDetails = json_decode($fields["transactionDetails"],true);
			...
			// }
			$customer["ruc"]	= unXss($allSuppliersArray[$fields['supplierId']]['contactTIN'] ?? "");
			$customer["name"] 	= unXss($allSuppliersArray[$fields['supplierId']]['contactName'] ?? "");

			foreach($allItemsSold as $item){
				if(empty($item['itemSoldTotal'])){
					$percentTax = 0;
				}else{
					$amountWithoutTax = $item['itemSoldTotal'] - $item['itemSoldTax'];
					if ($amountWithoutTax != 0) {
						$percentTax = round((floatval($item['itemSoldTax']) / $amountWithoutTax) * 100);
					} else {
						// Manejar el caso cuando $amountWithoutTax es cero
						$percentTax = 0; 
					}
				}
				if($percentTax == 10){
					$grav10 += $item['itemSoldTotal'];
				}
				if($percentTax == 5){
					$grav5 += $item['itemSoldTotal'];
				}
				if($percentTax == 0){
					$exentas += $item['itemSoldTotal'];
				}
			}
			// foreach ($getAllTaxNames as $tnK => $tnV) {
			...
			// 		if($tnV['name'] == '10'){
			// 			$grav10 	+= $fields['transactionTax'];
			// 		}else{
			// 			$grav10 	+= 0;
			// 		}

			// 		if($tnV['name'] == '0'){
			// 			$exentas 	+= $fields['transactionTax'];
			// 		}else{
			// 			$exentas 	+= 0;
			// 		}

			// 		if($tnV['name'] == '5'){
			// 			$grav5 		+= $fields['transactionTax'];
			// 		}else{
			// 			$grav5 		+= 0;
			// 		}
			// 	//}
			...
			$excellRow[]  = [
	                            ...
	                            $authNo, //NRO TIMBRADO
	                            $prefix. (substr($prefix,-1) == "-" ? " " : "-"). leadingZeros($fields['invoiceNo'], 7), //NRO FACTURA
	                            round($grav10), //GRAVADO 10%
	                            round($grav5), //GRAVADO 5%
	                            round($exentas), //EXENTAS
	                            round($tTotal - $fields['transactionDiscount']), //TOTAL FACTURA
...
if(validateHttp('action') == 'libro-compra'){
	ini_set('memory_limit', '256M');
	/*
	0 = Venta al contado  	    
	1 = Compra al contado 	    
	...
	*/
	...
	// $taxItemsArray = [];
	// if(!empty($taxItems) && is_object($taxItems)){
	...
	if($saleDay){
		...
		while (!$saleDay->EOF) {
			...
			if(isset($_GET['test'])){
				echo '<pre>';
				print_r(date('d/m/Y H:i:s', strtotime($fields['transactionDate'])));
				print_r($customer['name']);
				print_r(json_encode($taxItem)) ;
				echo '</pre>';
			}
			foreach($allItemsSold as $item){
				if(empty($item['itemSoldTotal'])){
				...
				if($percentTax == 10){
					$tax10 += $item['itemSoldTax'];
					$grav10 += $amountWithoutTax;
				}else{
					$tax10 += 0;
				}
	
				if($percentTax == 5){
					$tax5 += $item['itemSoldTax'];
					$grav5 += $amountWithoutTax;
				}else{
					$tax5 += 0;
				}
	
				if($percentTax == 0){
					$tax0 += $item['itemSoldTax'];
					$grav0 += $amountWithoutTax;
				}else{
					$tax0 		+= 0;
				}
			}

			if(!($cachedContact[$fields['supplierId']] ?? false)){
				$customer 				= getContactData($fields['supplierId'],'id',true);
				$cachedContact[$fields['supplierId']] = $customer;
			}else{
				$customer = $cachedContact[$fields['supplierId']];
			...
			//Sin nombre  = 15
			$TINType 				= 11;
			$customer['ruc'] 		= trim(is_null($customer['ruc'] ?? null) ? "" : $customer['ruc']);
			...
			$dataProviderInvoicing = explode(";",is_null($fields['invoicePrefix'])? "" :$fields['invoicePrefix']);
			$excellRow[]  = [
	                            date('d/m/Y', strtotime($fields['transactionDate'])), 		//FECHA
	                            // $TINType, 			//RUC, CI
	                            (empty($dataProviderInvoicing[1])? "" :$dataProviderInvoicing[1].((substr($dataProviderInvoicing[1],-1) == "-") ? "" : "-" ))  . leadingZeros($fields['invoiceNo'],7), //NRO FACTURA
	                            $customer['name'], 	//customer name
	                            $customer['ruc'], 	//customer TIN
	                            // $transType, 		//FACTURA o Nota de crédito
	                            $dataProviderInvoicing[0], 	//NRO TIMBRADO
	                            round($grav10), 	//GRAVADO 10%
								round($tax10),
	                            round($grav5), 	//GRAVADO 5%
								round($tax5),
	                            round($grav0), 	//GRAVADO 0%
								round($tax0),
	                            round($exentas), 	//EXENTAS
	                            round($tTotal), 	//TOTAL FACTURA
								round($total10),
								round($total5),
								round($total0)
...
$(document).ready(function(){
	...
	if(country == 'PY'){
		...
		libroCompra = '<a class="btn r-3x b b-light font-bold" id="libro-compra" href="#" data-toggle="tooltip" data-placement="bottom" title="Libro Compra">Libro Compra</a>';
	}
	...
	var loadTheTable = function(tableOps,oTable,_scope){
		...
		onClickWrap('#rg90',function(event,tis){
			var url = baseUrl + '?action=rg90&from=' + dateFrom + '&to=' + dateTo;
			window.open(url);
		});
		onClickWrap('#libro-compra',function(event,tis){
			var url = baseUrl + '?action=libro-compra&from=' + dateFrom + '&to=' + dateTo;
			window.open(url);
		});
		...
		onClickWrap('#reportDownload',function(event,tis){
			var url = tableOps.rawUrl + '&download-report=true&from='+startDate+'&to='+endDate;
			window.open(url);
		});
	};
