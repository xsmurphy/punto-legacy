...
if(isset($_GET['doit']) && $_GET['doit'] == 'beibe'){
	...
	$sql = "SELECT a.customerId as cliente, COUNT(a.*) as compras, SUM(b.itemSoldTotal) as total
			...
			ORDER BY compras DESC LIMIT 100;";
...
if($result){
		...
		while (!$result->EOF) {
			...
			$name 		= $contacts[$fields['cliente']]['contactName'];
			$loyalty 	= $contacts[$fields['cliente']]['contactLoyaltyAmount'];

			echo $name . ';' . $fields['compras'] . ';' . $fields['total'] . ';' . $loyalty . '<br>';
