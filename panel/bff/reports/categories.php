<?php
/**
 * BFF — Reporte de Ventas por Categorías.
 *
 *   GET /bff/reports/categories.php?from=&to=
 *       → { rows: [...+percent...], totals: {...} } CRUDO. El front formatea + arma
 *         tabla (con barra de %) + KPIs + treemap. Cero HTML, cero formateo.
 *
 * Gateway sobre la API (NO toca BD) + cálculo del % por fila y los totales.
...
$res = bffApiGet('v1/reports/categories.php', ['from' => $from, 'to' => $to], '_jwt_panel', ['base' => 'shared']);
if (!$res['ok']) {
    bffFailFromApi($res);
}
...
// allTotal para el % (cálculo del BFF).
$allTotal = 0;
foreach ($rows as $r) { $allTotal += (float) ($r['total'] ?? 0); }

$totals = ['usold' => 0, 'tax' => 0, 'discount' => 0, 'total' => 0];
foreach ($rows as &$r) {
    $t = (float) ($r['total'] ?? 0);
