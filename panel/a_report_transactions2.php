	...
	if(validateHttp('src')){
		...
		if(validateHttp('cusId')){
			...
			/*if($isSameDay){
				...
				$sql = "SELECT GROUP_CONCAT(transactionId) as ids
				FROM transaction 
				WHERE transactionType IN (0,3,6,7,8)
				AND transactionDate
				BETWEEN ? 
	...
	if($saleDay){
		...
		foreach ($saleDay as $key => $fields) {
			...
			if($fields['customerId']){
				if($custCache[$fields['customerId']]){
			...
			$invoicePaddedNo = str_pad($fields['invoiceNo'],  $registerData['registerDocsLeadingZeros'], "0", STR_PAD_LEFT);
			...
			if($customer['name'] || $customer['secondName']){
			...
			$table .= 	'<tr data-id="' . $itemId . '" class="clickrow ' . $isInternal . '">' .
						...
						'	<td data-filter="' . $custNameFltr . '">' . $customer['name'] . '</td>' .
						'	<td>' . $customerTin . '</td>' .
						'	<td>' . $userName . ' </td>' .
			...
			if(!$_fullSettings['taxPy'] && $fields['transactionDetails']){ //correccion del IVA mal calculado del 26 al 28
				$newTTax = 0;
				foreach(json_decode($fields['transactionDetails'],true) as $keyT => $valT){
					if(validity($valT['itemId'])){
						
						$newTTax += getTaxOfPrice($valT['tax'], ($valT['price'] * $valT['count']) );
					}
				}
...
if(validateHttp('action') == 'feTable'){
	...
	curl_setopt_array($curl, [
		CURLOPT_URL 			=> 'https://facturas.encom.com.py/api/companies/documents',
		...
		CURLOPT_POSTFIELDS 		=> '{
		    "ruc": "80131098-9",
		    "fecha_desde": "2023-07-15",
		    "fecha_hasta": "2023-07-27"
		}',
	...
	foreach ($result['documents'] as $key => $value) {
		...
		$body 	.=	'<tr>' .
					...
					'	<td> <a href="' . ($value['qr'] ? $value['qr'] : '#') . '" class="btn btn-default btn-sm" target="_blank" ' . (!$value['qr'] ? 'disabled' : '') . '><i class="material-icons">open_in_new</i></a> </td>' . 
					'</tr>';
	}
	    ...
	    <form action="<?=$baseUrl?>?action=update" method="POST" id="editSale" name="editSale">
	    	<div class="row equal padder">
		    	<div class="col-md-3 col-md-offset-0 col-sm-10 col-sm-offset-1 col-xs-12 wrapper bg-info gradBgBlue hidden-print">
	    			...
	    			$customerDT 	= ncmExecute('SELECT contactName, contactTIN FROM contact WHERE contactUID = ? LIMIT 1',[$tCustomer],true);
	    			...
	    			<div class="col-md-12 col-sm-4 col-xs-12 no-padder m-b">
		    			...
		    			<div>
							...
							if($result['transactionType'] == 9){
								?>
								<a href="https://public.encom.app/quoteView?s=<?=base64_encode(enc($result['transactionId']) . ',' . enc(COMPANY_ID))?>" class="hidden-print" target="_blank"> <i class="material-icons">launch</i></a>
								<?php
							}
							?>
		    			</div>
		    				...
		    				<table class="table" id="modalItemsTable">
				                <thead>
				                    <tr class="text-u-c">
				                        ...
				                        <th style="" class="text-left">Artículo</th>
				                        <th style="" class="text-left hidden-print">Usuario</th>
				                    	...
				                    	if(in_array($tSaleType,['2','7','9','11','12','13'])){
				                    		foreach(json_decode($result['transactionDetails'],true) as $key => $val){
				                    			...
				                    			if(validity($val['itemId']) || $isCustom){
						                            ...
						                            if($itemData['itemType'] == 'dynamic' || $isCustom){
					                        ...
					                        if($items){
					                        	while (!$items->EOF) {
					                        		...
					                        		if(validity($item['itemId'])){
						                        		...
						                        		if(in_array($itemData['itemType'],['combo','precombo','comboAddons','service','product','production','direct_production','giftcard','discount','dynamic',NULL])){
				                ...
				                <tfoot class="font-bold text-right">
				                	...
				                	<tr>
				                		<td class="hidden-print"></td>
				                		<td colspan="3">Subtotal (Base Imponible)</td>
				                		<td><?=formatCurrentNumber($result['transactionTotal'] - $result['transactionTax'])?></td>
				                	</tr>
				                	<tr>
				                		<td class="hidden-print"></td>
					                      ...
					                      ?>
					                        <tr>
					                          ...
					                          <td class="text-right">
					                            ...
					                            <a href="https://public.encom.app/receipt?s=<?=base64_encode(enc($credit->fields['transactionId']) . ',' . enc(COMPANY_ID))?>" class="hidden-print" target="_blank"><i class="material-icons text-info">receipt</i></a>
		    					...
		    					while (!$returns->EOF) {
		    						...
		    						?>
					                    ...
					                    <tr class="subModal pointer" data-id="<?=enc($retur['transactionId'])?>">
	    	    							...
	    	    							<td class="text-right font-bold"><?=formatCurrentNumber($retur['transactionTotal'])?></td>
			    	    				</tr>
...
if(validateHttp('ci')){
	...
	$reportsTitle = [
						...
						'maxDays' 		=> $MAX_DAYS_RANGE,
						'tutorial' 		=> 'https://docs.encom.app/panel-de-control/reportes/reporte-de-ventas-1/transacciones',
						'hideChart' 	=> true
					];
}
	...
	var xhr = ncmHelpers.load({
		...
		success 	: (result) => {
			...
			window.info2 = {
						...
						"colsFilter"		: {
												...
												menu 		:  [
																...
																{"index" : 7, "name" : tinName,"visible":false},
			...
			onClickWrap('#mcal',function(event,tis){
				var url = 'https://panel.encom.app/thirdparty/mcal/mcalSendSales?s=' + btoa( eCompanyID + ',' + eOutletID + ',' + endDate );
				$.get(url, (result) => {
