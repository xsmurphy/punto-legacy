...
<script type="text/html" id="lineTpl">
  <div class="col-xs-12 b-b no-padder oneRow animates fadeIn speed-4x" id="row{{index}}">
  ...
  $(document).ready(function(){
    ...
    var ncmTransfer = {
      ...
      load : function(){
        ...
        //guardo progreso
        //var progress = simpleStorage.get('bulk_transfer_default_progress_<?=enc(OUTLET_ID)?>');
        /*if(progress){
          $.each(progress,function(i,val){
            ncmTransfer.addNewLine(val.index,val.qty,val.itemId,val.itemName,true);
          });
          //$('#table').html(progress);
        }else{
          $('#table').html($('#empty').html());
        ...
        //ncmTransfer.index = $('#table .oneRow').length ? ($('#table .oneRow').length - 1) : -1;
      ...
      actions : function(){
        onClickWrap('#transferBtn,#orderBtn',function(event,tis){
          ...
          ncmDialogs.confirm('¿Realmente desea realizar la transferencia?','No podrá revertir esta operación','warning',function(e){
            if (e) {
              $('#addItem').submit();
              simpleStorage.set('bulk_transfer_default_progress_<?=enc(OUTLET_ID)?>',null);
              spinner('body', 'show');
            }
          });
        },false,true);
        ...
        onClickWrap(".remove",function(tis) {
          ...
          var prePro = simpleStorage.get('bulk_transfer_default_progress_<?=enc(OUTLET_ID)?>')
          prePro.pop();
          simpleStorage.set('bulk_transfer_default_progress_<?=enc(OUTLET_ID)?>',prePro);
